package typingsSlinky.uncontrollable

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object utilsMod {
  
  @JSImport("uncontrollable/cjs/utils", "canAcceptRef")
  @js.native
  def canAcceptRef(component: js.Any): js.Any = js.native
  
  @JSImport("uncontrollable/cjs/utils", "defaultKey")
  @js.native
  def defaultKey(key: String): String = js.native
  
  @JSImport("uncontrollable/cjs/utils", "isProp")
  @js.native
  def isProp[P](props: P, prop: /* keyof P */ String): Boolean = js.native
  
  @JSImport("uncontrollable/cjs/utils", "uncontrolledPropTypes")
  @js.native
  def uncontrolledPropTypes(controlledValues: js.Any, displayName: String): js.Object = js.native
}
