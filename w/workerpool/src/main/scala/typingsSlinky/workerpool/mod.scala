...
import typingsSlinky.workerpool.workerpoolStrings.browser
import typingsSlinky.workerpool.workerpoolStrings.max
import typingsSlinky.workerpool.workerpoolStrings.node
...
object mod {
  ...
  {[ M in keyof T ]: (args : std.Parameters<T[M]>): workerpool.workerpool.Promise<std.ReturnType<T[M]>, std.Error>}
    */ typingsSlinky.workerpool.workerpoolStrings.Proxy with TopLevel[js.Any]
