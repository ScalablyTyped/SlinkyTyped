...
object IStorageQueryResultBase {
  ...
  implicit class IStorageQueryResultBaseOps[Self <: IStorageQueryResultBase] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withFindStartIndexAsync(value: js.Any => IPromiseWithIAsyncOperation[Double]): Self = {
        val ret = this.duplicate
    ...
    @scala.inline
    def withGetItemCountAsync(value: () => IPromiseWithIAsyncOperation[Double]): Self = {
        val ret = this.duplicate
