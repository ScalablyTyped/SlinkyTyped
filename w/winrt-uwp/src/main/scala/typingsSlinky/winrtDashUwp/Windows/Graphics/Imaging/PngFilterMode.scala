package typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
sealed trait PngFilterMode extends js.Object

/** Specifies the filter used to optimize the image prior to image compression in PNG images. */
@JSGlobal("Windows.Graphics.Imaging.PngFilterMode")
@js.native
object PngFilterMode extends js.Object {
  /** The Adaptive type filter is applied. */
  @js.native
  sealed trait adaptive extends PngFilterMode
  
  /** The filter type will be chosen automatically. */
  @js.native
  sealed trait automatic extends PngFilterMode
  
  /** The Average type filter is applied. */
  @js.native
  sealed trait average extends PngFilterMode
  
  /** The image is unmodified. */
  @js.native
  sealed trait none extends PngFilterMode
  
  /** The Paeth type filter is applied. */
  @js.native
  sealed trait paeth extends PngFilterMode
  
  /** The Sub type filter is applied. */
  @js.native
  sealed trait sub extends PngFilterMode
  
  /** The Up filter type is applied. */
  @js.native
  sealed trait up extends PngFilterMode
  
  /* 6 */ val adaptive: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.adaptive with Double = js.native
  /* 0 */ val automatic: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.automatic with Double = js.native
  /* 4 */ val average: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.average with Double = js.native
  /* 1 */ val none: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.none with Double = js.native
  /* 5 */ val paeth: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.paeth with Double = js.native
  /* 2 */ val sub: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.sub with Double = js.native
  /* 3 */ val up: typingsSlinky.winrtDashUwp.Windows.Graphics.Imaging.PngFilterMode.up with Double = js.native
  @JSBracketAccess
  def apply(value: Double): js.UndefOr[PngFilterMode with Double] = js.native
}

