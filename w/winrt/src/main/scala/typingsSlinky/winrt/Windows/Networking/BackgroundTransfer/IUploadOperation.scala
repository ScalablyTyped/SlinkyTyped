...
object IUploadOperation {
  ...
  implicit class IUploadOperationOps[Self <: IUploadOperation] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withAttachAsync(value: () => IAsyncOperationWithProgress[UploadOperation, UploadOperation]): Self = {
    ...
    @scala.inline
    def withStartAsync(value: () => IAsyncOperationWithProgress[UploadOperation, UploadOperation]): Self = {
        val ret = this.duplicate
