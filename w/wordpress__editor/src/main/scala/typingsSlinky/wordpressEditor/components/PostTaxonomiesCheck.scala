...
import typingsSlinky.wordpressEditor.mod.PostTaxonomiesCheck.^
...
object PostTaxonomiesCheck {
  ...
  implicit def make(companion: PostTaxonomiesCheck.type): Default[tag.type, ^] = new Default[tag.type, ^](js.Array(this.component, js.Dictionary.empty))()
}
