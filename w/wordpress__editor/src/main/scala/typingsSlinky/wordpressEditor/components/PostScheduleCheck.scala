...
import typingsSlinky.wordpressEditor.mod.PostScheduleCheck.^
...
object PostScheduleCheck {
  ...
  implicit def make(companion: PostScheduleCheck.type): Default[tag.type, ^] = new Default[tag.type, ^](js.Array(this.component, js.Dictionary.empty))()
}
