...
import typingsSlinky.wordpressEditor.mod.PostVisibilityLabel.^
...
object PostVisibilityLabel {
  ...
  implicit def make(companion: PostVisibilityLabel.type): Default[tag.type, ^] = new Default[tag.type, ^](js.Array(this.component, js.Dictionary.empty))()
}
