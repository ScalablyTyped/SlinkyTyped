...
import typingsSlinky.wiiu.wiiu.ImageView_
import typingsSlinky.wiiu.wiiu.VideoPlayer_
...
object Typeofwiiu {
  ...
  implicit class TypeofwiiuOps[Self <: Typeofwiiu] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withImageview(value: ImageView_): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("imageview")(value.asInstanceOf[js.Any])
        ret
    }
    @scala.inline
    def withVideoplayer(value: VideoPlayer_): Self = {
        val ret = this.duplicate
