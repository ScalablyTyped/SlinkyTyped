...
object DocumentRangeSemanticTokensProvider {
  ...
  implicit class DocumentRangeSemanticTokensProviderOps[Self <: DocumentRangeSemanticTokensProvider] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withProvideDocumentRangeSemanticTokens(value: (TextDocument, Range, CancellationToken) => ProviderResult[SemanticTokens]): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("provideDocumentRangeSemanticTokens")(js.Any.fromFunction3(value))
        ret
    }
  }
