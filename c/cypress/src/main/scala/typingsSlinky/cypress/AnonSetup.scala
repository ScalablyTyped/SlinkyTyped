...
import typingsSlinky.cypress.JQuery_.EventHandler
...
object AnonSetup {
  ...
  @scala.inline
  implicit class AnonSetupOps[Self[ttarget, tdata] <: AnonSetup[ttarget, tdata], TTarget, TData] (val x: Self[TTarget, TData]) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def withSetup(value: (TData, String, EventHandler[TTarget, TData]) => Unit | `false`): Self[TTarget, TData] = {
