package typingsSlinky.cypress.typesLodashFpMod

import typingsSlinky.cypress.typesLodashMod.PropertyPath
import typingsSlinky.cypress.typesLodashMod.__
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait LodashGetOr4x4 extends js.Object {
  def apply(defaultValue: js.Any): LodashGetOr4x5 = js.native
  def apply(defaultValue: js.Any, path: PropertyPath): js.Any = js.native
  def apply(defaultValue: __, path: PropertyPath): LodashGetOr4x6 = js.native
}

