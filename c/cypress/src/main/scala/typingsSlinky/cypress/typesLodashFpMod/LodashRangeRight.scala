package typingsSlinky.cypress.typesLodashFpMod

import typingsSlinky.cypress.typesLodashMod.__
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait LodashRangeRight extends js.Object {
  def apply(start: Double): LodashRangeRight1x1 = js.native
  def apply(start: Double, end: Double): js.Array[Double] = js.native
  def apply(start: __, end: Double): LodashRangeRight1x2 = js.native
}

