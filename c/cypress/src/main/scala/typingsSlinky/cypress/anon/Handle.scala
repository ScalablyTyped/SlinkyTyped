...
@js.native
trait Handle[TTarget, TData] extends SpecialEventHook[TTarget, TData] {
  /**
...
object Handle {
  ...
  @scala.inline
  implicit class HandleOps[Self[ttarget, tdata] <: Handle[ttarget, tdata], TTarget, TData] (val x: Self[TTarget, TData]) extends AnyVal {
    @scala.inline
