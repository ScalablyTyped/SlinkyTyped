package typingsSlinky.contentfulManagement

import typingsSlinky.contentfulManagement.appDefinitionMod.AppLocation
import typingsSlinky.contentfulManagement.appDefinitionMod.Field
import typingsSlinky.contentfulManagement.appDefinitionMod.LinkType
import typingsSlinky.contentfulManagement.roleMod.ActionType
import typingsSlinky.contentfulManagement.roleMod.ConditionType
import typingsSlinky.contentfulManagement.usageMod.UsageMetricEnum
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object contentfulManagementStrings {
  
  @scala.inline
  def Array: Array = "Array".asInstanceOf[Array]
  
  @scala.inline
  def Asset: Asset = "Asset".asInstanceOf[Asset]
  
  @scala.inline
  def Boolean: Boolean = "Boolean".asInstanceOf[Boolean]
  
  @scala.inline
  def DELETE: DELETE = "DELETE".asInstanceOf[DELETE]
  
  @scala.inline
  def Date: Date = "Date".asInstanceOf[Date]
  
  @scala.inline
  def Entry: Entry = "Entry".asInstanceOf[Entry]
  
  @scala.inline
  def Environment: Environment = "Environment".asInstanceOf[Environment]
  
  @scala.inline
  def GET: GET = "GET".asInstanceOf[GET]
  
  @scala.inline
  def Integer: Integer = "Integer".asInstanceOf[Integer]
  
  @scala.inline
  def Link: Link = "Link".asInstanceOf[Link]
  
  @scala.inline
  def Location: Location = "Location".asInstanceOf[Location]
  
  @scala.inline
  def Number: Number = "Number".asInstanceOf[Number]
  
  @scala.inline
  def Object: Object = "Object".asInstanceOf[Object]
  
  @scala.inline
  def PATCH: PATCH = "PATCH".asInstanceOf[PATCH]
  
  @scala.inline
  def POST: POST = "POST".asInstanceOf[POST]
  
  @scala.inline
  def PUT: PUT = "PUT".asInstanceOf[PUT]
  
  @scala.inline
  def RichText: RichText = "RichText".asInstanceOf[RichText]
  
  @scala.inline
  def Symbol: Symbol = "Symbol".asInstanceOf[Symbol]
  
  @scala.inline
  def Tag: Tag = "Tag".asInstanceOf[Tag]
  
  @scala.inline
  def Text: Text = "Text".asInstanceOf[Text]
  
  @scala.inline
  def all: all = "all".asInstanceOf[all]
  
  @scala.inline
  def and: and = "and".asInstanceOf[and]
  
  @scala.inline
  def `app-config`: `app-config` = "app-config".asInstanceOf[`app-config`]
  
  @scala.inline
  def applicationSlashjson: applicationSlashjson = "application/json".asInstanceOf[applicationSlashjson]
  
  @scala.inline
  def `applicationSlashjsonSemicolon charsetEqualssignutf-8`: `applicationSlashjsonSemicolon charsetEqualssignutf-8` = ("application/json; charset=utf-8").asInstanceOf[`applicationSlashjsonSemicolon charsetEqualssignutf-8`]
  
  @scala.inline
  def applicationSlashvndDotcontentfulDotmanagementDotv1Plussignjson: applicationSlashvndDotcontentfulDotmanagementDotv1Plussignjson = "application/vnd.contentful.management.v1+json".asInstanceOf[applicationSlashvndDotcontentfulDotmanagementDotv1Plussignjson]
  
  @scala.inline
  def `applicationSlashvndDotcontentfulDotmanagementDotv1PlussignjsonSemicolon charsetEqualssignutf-8`: `applicationSlashvndDotcontentfulDotmanagementDotv1PlussignjsonSemicolon charsetEqualssignutf-8` = ("application/vnd.contentful.management.v1+json; charset=utf-8").asInstanceOf[`applicationSlashvndDotcontentfulDotmanagementDotv1PlussignjsonSemicolon charsetEqualssignutf-8`]
  
  @scala.inline
  def `applicationSlashx-www-form-urlencoded`: `applicationSlashx-www-form-urlencoded` = "application/x-www-form-urlencoded".asInstanceOf[`applicationSlashx-www-form-urlencoded`]
  
  @scala.inline
  def `applicationSlashx-www-form-urlencodedSemicolon charsetEqualssignutf-8`: `applicationSlashx-www-form-urlencodedSemicolon charsetEqualssignutf-8` = ("application/x-www-form-urlencoded; charset=utf-8").asInstanceOf[`applicationSlashx-www-form-urlencodedSemicolon charsetEqualssignutf-8`]
  
  @scala.inline
  def archive: archive = "archive".asInstanceOf[archive]
  
  @scala.inline
  def cda: cda = "cda".asInstanceOf[cda]
  
  @scala.inline
  def cma: cma = "cma".asInstanceOf[cma]
  
  @scala.inline
  def contentDeliveryApi: contentDeliveryApi = "contentDeliveryApi".asInstanceOf[contentDeliveryApi]
  
  @scala.inline
  def contentManagementApi: contentManagementApi = "contentManagementApi".asInstanceOf[contentManagementApi]
  
  @scala.inline
  def content_management_manage: content_management_manage = "content_management_manage".asInstanceOf[content_management_manage]
  
  @scala.inline
  def cpa: cpa = "cpa".asInstanceOf[cpa]
  
  @scala.inline
  def create: create = "create".asInstanceOf[create]
  
  @scala.inline
  def default: default = "default".asInstanceOf[default]
  
  @scala.inline
  def delete_ : delete_ = "delete".asInstanceOf[delete_]
  
  @scala.inline
  def description: description = "description".asInstanceOf[description]
  
  @scala.inline
  def dialog: dialog = "dialog".asInstanceOf[dialog]
  
  @scala.inline
  def displayField: displayField = "displayField".asInstanceOf[displayField]
  
  @scala.inline
  def `entry-editor`: `entry-editor` = "entry-editor".asInstanceOf[`entry-editor`]
  
  @scala.inline
  def `entry-field`: `entry-field` = "entry-field".asInstanceOf[`entry-field`]
  
  @scala.inline
  def `entry-sidebar`: `entry-sidebar` = "entry-sidebar".asInstanceOf[`entry-sidebar`]
  
  @scala.inline
  def equals: equals = "equals".asInstanceOf[equals]
  
  @scala.inline
  def gql: gql = "gql".asInstanceOf[gql]
  
  @scala.inline
  def not: not = "not".asInstanceOf[not]
  
  @scala.inline
  def optional: optional = "optional".asInstanceOf[optional]
  
  @scala.inline
  def or: or = "or".asInstanceOf[or]
  
  @scala.inline
  def page: page = "page".asInstanceOf[page]
  
  @scala.inline
  def publish: publish = "publish".asInstanceOf[publish]
  
  @scala.inline
  def read: read = "read".asInstanceOf[read]
  
  @scala.inline
  def sysDotcontentTypeDotsysDotid: sysDotcontentTypeDotsysDotid = "sys.contentType.sys.id".asInstanceOf[sysDotcontentTypeDotsysDotid]
  
  @scala.inline
  def sysDotenvironmentDotsysDotid: sysDotenvironmentDotsysDotid = "sys.environment.sys.id".asInstanceOf[sysDotenvironmentDotsysDotid]
  
  @scala.inline
  def sysDotid: sysDotid = "sys.id".asInstanceOf[sysDotid]
  
  @scala.inline
  def unarchive: unarchive = "unarchive".asInstanceOf[unarchive]
  
  @scala.inline
  def unpublish: unpublish = "unpublish".asInstanceOf[unpublish]
  
  @scala.inline
  def update: update = "update".asInstanceOf[update]
  
  @js.native
  sealed trait Array extends js.Object
  
  @js.native
  sealed trait Asset extends LinkType
  
  @js.native
  sealed trait Boolean extends Field
  
  @js.native
  sealed trait DELETE extends js.Object
  
  @js.native
  sealed trait Date extends Field
  
  @js.native
  sealed trait Entry extends LinkType
  
  @js.native
  sealed trait Environment extends js.Object
  
  @js.native
  sealed trait GET extends js.Object
  
  @js.native
  sealed trait Integer extends Field
  
  @js.native
  sealed trait Link extends js.Object
  
  @js.native
  sealed trait Location extends Field
  
  @js.native
  sealed trait Number extends Field
  
  @js.native
  sealed trait Object extends Field
  
  @js.native
  sealed trait PATCH extends js.Object
  
  @js.native
  sealed trait POST extends js.Object
  
  @js.native
  sealed trait PUT extends js.Object
  
  @js.native
  sealed trait RichText extends Field
  
  @js.native
  sealed trait Symbol extends Field
  
  @js.native
  sealed trait Tag extends js.Object
  
  @js.native
  sealed trait Text extends Field
  
  @js.native
  sealed trait all extends js.Object
  
  @js.native
  sealed trait and extends ConditionType
  
  @js.native
  sealed trait `app-config` extends AppLocation
  
  @js.native
  sealed trait applicationSlashjson extends js.Object
  
  @js.native
  sealed trait `applicationSlashjsonSemicolon charsetEqualssignutf-8` extends js.Object
  
  @js.native
  sealed trait applicationSlashvndDotcontentfulDotmanagementDotv1Plussignjson extends js.Object
  
  @js.native
  sealed trait `applicationSlashvndDotcontentfulDotmanagementDotv1PlussignjsonSemicolon charsetEqualssignutf-8` extends js.Object
  
  @js.native
  sealed trait `applicationSlashx-www-form-urlencoded` extends js.Object
  
  @js.native
  sealed trait `applicationSlashx-www-form-urlencodedSemicolon charsetEqualssignutf-8` extends js.Object
  
  @js.native
  sealed trait archive extends ActionType
  
  @js.native
  sealed trait cda extends UsageMetricEnum
  
  @js.native
  sealed trait cma extends UsageMetricEnum
  
  @js.native
  sealed trait contentDeliveryApi extends js.Object
  
  @js.native
  sealed trait contentManagementApi extends js.Object
  
  @js.native
  sealed trait content_management_manage extends js.Object
  
  @js.native
  sealed trait cpa extends UsageMetricEnum
  
  @js.native
  sealed trait create extends ActionType
  
  @js.native
  sealed trait default extends js.Object
  
  @js.native
  sealed trait delete_ extends ActionType
  
  @js.native
  sealed trait description extends js.Object
  
  @js.native
  sealed trait dialog extends AppLocation
  
  @js.native
  sealed trait displayField extends js.Object
  
  @js.native
  sealed trait `entry-editor` extends AppLocation
  
  @js.native
  sealed trait `entry-field` extends js.Object
  
  @js.native
  sealed trait `entry-sidebar` extends AppLocation
  
  @js.native
  sealed trait equals extends ConditionType
  
  @js.native
  sealed trait gql extends UsageMetricEnum
  
  @js.native
  sealed trait not extends ConditionType
  
  @js.native
  sealed trait optional extends js.Object
  
  @js.native
  sealed trait or extends ConditionType
  
  @js.native
  sealed trait page extends AppLocation
  
  @js.native
  sealed trait publish extends ActionType
  
  @js.native
  sealed trait read extends ActionType
  
  @js.native
  sealed trait sysDotcontentTypeDotsysDotid extends js.Object
  
  @js.native
  sealed trait sysDotenvironmentDotsysDotid extends js.Object
  
  @js.native
  sealed trait sysDotid extends js.Object
  
  @js.native
  sealed trait unarchive extends ActionType
  
  @js.native
  sealed trait unpublish extends ActionType
  
  @js.native
  sealed trait update extends ActionType
}
