package typingsSlinky.cadesplugin.CAdESCOM

import typingsSlinky.cadesplugin.cadespluginNumbers.`0`
import typingsSlinky.cadesplugin.cadespluginNumbers.`100`
import typingsSlinky.cadesplugin.cadespluginNumbers.`101`
import typingsSlinky.cadesplugin.cadespluginNumbers.`102`
import typingsSlinky.cadesplugin.cadespluginNumbers.`1`
import typingsSlinky.cadesplugin.cadespluginNumbers.`2`
import typingsSlinky.cadesplugin.cadespluginNumbers.`3`
import typingsSlinky.cadesplugin.cadespluginNumbers.`4`
import typingsSlinky.cadesplugin.cadespluginNumbers.`5`
import typingsSlinky.cadesplugin.cadespluginNumbers.`6`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait CADESCOM_HASH_ALGORITHM extends StObject {
  
  val CADESCOM_HASH_ALGORITHM_CP_GOST_3411: `100` = js.native
  
  val CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256: `101` = js.native
  
  val CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_512: `102` = js.native
  
  val CADESCOM_HASH_ALGORITHM_MD2: `1` = js.native
  
  val CADESCOM_HASH_ALGORITHM_MD4: `2` = js.native
  
  val CADESCOM_HASH_ALGORITHM_MD5: `3` = js.native
  
  val CADESCOM_HASH_ALGORITHM_SHA1: `0` = js.native
  
  val CADESCOM_HASH_ALGORITHM_SHA_256: `4` = js.native
  
  val CADESCOM_HASH_ALGORITHM_SHA_384: `5` = js.native
  
  val CADESCOM_HASH_ALGORITHM_SHA_512: `6` = js.native
}
object CADESCOM_HASH_ALGORITHM {
  
  @scala.inline
  def apply(
    CADESCOM_HASH_ALGORITHM_CP_GOST_3411: `100`,
    CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256: `101`,
    CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_512: `102`,
    CADESCOM_HASH_ALGORITHM_MD2: `1`,
    CADESCOM_HASH_ALGORITHM_MD4: `2`,
    CADESCOM_HASH_ALGORITHM_MD5: `3`,
    CADESCOM_HASH_ALGORITHM_SHA1: `0`,
    CADESCOM_HASH_ALGORITHM_SHA_256: `4`,
    CADESCOM_HASH_ALGORITHM_SHA_384: `5`,
    CADESCOM_HASH_ALGORITHM_SHA_512: `6`
  ): CADESCOM_HASH_ALGORITHM = {
    val __obj = js.Dynamic.literal(CADESCOM_HASH_ALGORITHM_CP_GOST_3411 = CADESCOM_HASH_ALGORITHM_CP_GOST_3411.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256 = CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_512 = CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_512.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_MD2 = CADESCOM_HASH_ALGORITHM_MD2.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_MD4 = CADESCOM_HASH_ALGORITHM_MD4.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_MD5 = CADESCOM_HASH_ALGORITHM_MD5.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_SHA1 = CADESCOM_HASH_ALGORITHM_SHA1.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_SHA_256 = CADESCOM_HASH_ALGORITHM_SHA_256.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_SHA_384 = CADESCOM_HASH_ALGORITHM_SHA_384.asInstanceOf[js.Any], CADESCOM_HASH_ALGORITHM_SHA_512 = CADESCOM_HASH_ALGORITHM_SHA_512.asInstanceOf[js.Any])
    __obj.asInstanceOf[CADESCOM_HASH_ALGORITHM]
  }
  
  @scala.inline
  implicit class CADESCOM_HASH_ALGORITHMMutableBuilder[Self <: CADESCOM_HASH_ALGORITHM] (val x: Self) extends AnyVal {
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_CP_GOST_3411(value: `100`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_CP_GOST_3411", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256(value: `101`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_256", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_512(value: `102`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_CP_GOST_3411_2012_512", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_MD2(value: `1`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_MD2", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_MD4(value: `2`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_MD4", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_MD5(value: `3`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_MD5", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_SHA1(value: `0`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_SHA1", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_SHA_256(value: `4`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_SHA_256", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_SHA_384(value: `5`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_SHA_384", value.asInstanceOf[js.Any])
    
    @scala.inline
    def setCADESCOM_HASH_ALGORITHM_SHA_512(value: `6`): Self = StObject.set(x, "CADESCOM_HASH_ALGORITHM_SHA_512", value.asInstanceOf[js.Any])
  }
}
