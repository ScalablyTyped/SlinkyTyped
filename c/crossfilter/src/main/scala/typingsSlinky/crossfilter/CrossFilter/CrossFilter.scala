...
trait CrossFilter[T] extends js.Object {
  def GroupAll(): typingsSlinky.crossfilter.CrossFilter.GroupAll[T, T]
  def add(records: js.Array[T]): typingsSlinky.crossfilter.CrossFilter.CrossFilter[T]
  ...
  def remove(): typingsSlinky.crossfilter.CrossFilter.CrossFilter[T]
  def size(): Double
}
