...
object ZoomMode {
  ...
  @scala.inline
  def `per-origin`: typingsSlinky.chromeDashApps.chromeDashAppsStrings.`per-origin` = this.cast("per-origin")
  @scala.inline
  def `per-view`: typingsSlinky.chromeDashApps.chromeDashAppsStrings.`per-view` = this.cast("per-view")
}
