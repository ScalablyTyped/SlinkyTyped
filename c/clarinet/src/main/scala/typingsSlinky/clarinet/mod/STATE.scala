package typingsSlinky.clarinet.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("clarinet", "STATE")
@js.native
object STATE extends js.Object {
  var `0`: String = js.native
  var `1`: String = js.native
  var `10`: String = js.native
  var `11`: String = js.native
  var `12`: String = js.native
  var `13`: String = js.native
  var `14`: String = js.native
  var `15`: String = js.native
  var `16`: String = js.native
  var `17`: String = js.native
  var `18`: String = js.native
  var `19`: String = js.native
  var `2`: String = js.native
  var `20`: String = js.native
  var `21`: String = js.native
  var `22`: String = js.native
  var `23`: String = js.native
  var `3`: String = js.native
  var `4`: String = js.native
  var `5`: String = js.native
  var `6`: String = js.native
  var `7`: String = js.native
  var `8`: String = js.native
  var `9`: String = js.native
  var BACKSLASH: Double = js.native
  var BEGIN: Double = js.native
  var CLOSE_ARRAY: Double = js.native
  var CLOSE_KEY: Double = js.native
  var CLOSE_OBJECT: Double = js.native
  var END: Double = js.native
  var FALSE: Double = js.native
  var FALSE2: Double = js.native
  var FALSE3: Double = js.native
  var FALSE4: Double = js.native
  var NULL: Double = js.native
  var NULL2: Double = js.native
  var NULL3: Double = js.native
  var NUMBER_DECIMAL_POINT: Double = js.native
  var NUMBER_DIGIT: Double = js.native
  var OPEN_ARRAY: Double = js.native
  var OPEN_KEY: Double = js.native
  var OPEN_OBJECT: Double = js.native
  var STRING: Double = js.native
  var TEXT_ESCAPE: Double = js.native
  var TRUE: Double = js.native
  var TRUE2: Double = js.native
  var TRUE3: Double = js.native
  var VALUE: Double = js.native
}

