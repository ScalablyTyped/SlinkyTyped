...
object mod {
  ...
  @js.native
  def createSubscription[S, T](config: SubscriptionConfig[S, T]): ReactComponentClass[SubscriptionProps[S, T]] = js.native
  ...
  object SubscriptionProps {
    ...
    implicit class SubscriptionPropsMutableBuilder[Self <: SubscriptionProps[_, _], S, T] (val x: Self with (SubscriptionProps[S, T])) extends AnyVal {
      ...
      @scala.inline
      def setChildren(value: T => ReactElement): Self = StObject.set(x, "children", js.Any.fromFunction1(value))
