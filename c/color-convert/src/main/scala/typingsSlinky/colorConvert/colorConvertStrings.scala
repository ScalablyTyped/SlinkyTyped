package typingsSlinky.colorConvert

import typingsSlinky.colorConvert.conversionsMod.KEYWORD_
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object colorConvertStrings {
  
  @scala.inline
  def aliceblue: aliceblue = "aliceblue".asInstanceOf[aliceblue]
  
  @scala.inline
  def ansi16: ansi16 = "ansi16".asInstanceOf[ansi16]
  
  @scala.inline
  def ansi256: ansi256 = "ansi256".asInstanceOf[ansi256]
  
  @scala.inline
  def antiquewhite: antiquewhite = "antiquewhite".asInstanceOf[antiquewhite]
  
  @scala.inline
  def apple: apple = "apple".asInstanceOf[apple]
  
  @scala.inline
  def aqua: aqua = "aqua".asInstanceOf[aqua]
  
  @scala.inline
  def aquamarine: aquamarine = "aquamarine".asInstanceOf[aquamarine]
  
  @scala.inline
  def azure: azure = "azure".asInstanceOf[azure]
  
  @scala.inline
  def b16: b16 = "b16".asInstanceOf[b16]
  
  @scala.inline
  def beige: beige = "beige".asInstanceOf[beige]
  
  @scala.inline
  def bisque: bisque = "bisque".asInstanceOf[bisque]
  
  @scala.inline
  def black: black = "black".asInstanceOf[black]
  
  @scala.inline
  def blanchedalmond: blanchedalmond = "blanchedalmond".asInstanceOf[blanchedalmond]
  
  @scala.inline
  def blue: blue = "blue".asInstanceOf[blue]
  
  @scala.inline
  def blueviolet: blueviolet = "blueviolet".asInstanceOf[blueviolet]
  
  @scala.inline
  def brown: brown = "brown".asInstanceOf[brown]
  
  @scala.inline
  def burlywood: burlywood = "burlywood".asInstanceOf[burlywood]
  
  @scala.inline
  def c: c = "c".asInstanceOf[c]
  
  @scala.inline
  def cadetblue: cadetblue = "cadetblue".asInstanceOf[cadetblue]
  
  @scala.inline
  def chartreuse: chartreuse = "chartreuse".asInstanceOf[chartreuse]
  
  @scala.inline
  def chocolate: chocolate = "chocolate".asInstanceOf[chocolate]
  
  @scala.inline
  def cmyk: cmyk = "cmyk".asInstanceOf[cmyk]
  
  @scala.inline
  def coral: coral = "coral".asInstanceOf[coral]
  
  @scala.inline
  def cornflowerblue: cornflowerblue = "cornflowerblue".asInstanceOf[cornflowerblue]
  
  @scala.inline
  def cornsilk: cornsilk = "cornsilk".asInstanceOf[cornsilk]
  
  @scala.inline
  def crimson: crimson = "crimson".asInstanceOf[crimson]
  
  @scala.inline
  def cyan: cyan = "cyan".asInstanceOf[cyan]
  
  @scala.inline
  def darkblue: darkblue = "darkblue".asInstanceOf[darkblue]
  
  @scala.inline
  def darkcyan: darkcyan = "darkcyan".asInstanceOf[darkcyan]
  
  @scala.inline
  def darkgoldenrod: darkgoldenrod = "darkgoldenrod".asInstanceOf[darkgoldenrod]
  
  @scala.inline
  def darkgray: darkgray = "darkgray".asInstanceOf[darkgray]
  
  @scala.inline
  def darkgreen: darkgreen = "darkgreen".asInstanceOf[darkgreen]
  
  @scala.inline
  def darkgrey: darkgrey = "darkgrey".asInstanceOf[darkgrey]
  
  @scala.inline
  def darkkhaki: darkkhaki = "darkkhaki".asInstanceOf[darkkhaki]
  
  @scala.inline
  def darkmagenta: darkmagenta = "darkmagenta".asInstanceOf[darkmagenta]
  
  @scala.inline
  def darkolivegreen: darkolivegreen = "darkolivegreen".asInstanceOf[darkolivegreen]
  
  @scala.inline
  def darkorange: darkorange = "darkorange".asInstanceOf[darkorange]
  
  @scala.inline
  def darkorchid: darkorchid = "darkorchid".asInstanceOf[darkorchid]
  
  @scala.inline
  def darkred: darkred = "darkred".asInstanceOf[darkred]
  
  @scala.inline
  def darksalmon: darksalmon = "darksalmon".asInstanceOf[darksalmon]
  
  @scala.inline
  def darkseagreen: darkseagreen = "darkseagreen".asInstanceOf[darkseagreen]
  
  @scala.inline
  def darkslateblue: darkslateblue = "darkslateblue".asInstanceOf[darkslateblue]
  
  @scala.inline
  def darkslategray: darkslategray = "darkslategray".asInstanceOf[darkslategray]
  
  @scala.inline
  def darkslategrey: darkslategrey = "darkslategrey".asInstanceOf[darkslategrey]
  
  @scala.inline
  def darkturquoise: darkturquoise = "darkturquoise".asInstanceOf[darkturquoise]
  
  @scala.inline
  def darkviolet: darkviolet = "darkviolet".asInstanceOf[darkviolet]
  
  @scala.inline
  def deeppink: deeppink = "deeppink".asInstanceOf[deeppink]
  
  @scala.inline
  def deepskyblue: deepskyblue = "deepskyblue".asInstanceOf[deepskyblue]
  
  @scala.inline
  def dimgray: dimgray = "dimgray".asInstanceOf[dimgray]
  
  @scala.inline
  def dimgrey: dimgrey = "dimgrey".asInstanceOf[dimgrey]
  
  @scala.inline
  def dodgerblue: dodgerblue = "dodgerblue".asInstanceOf[dodgerblue]
  
  @scala.inline
  def firebrick: firebrick = "firebrick".asInstanceOf[firebrick]
  
  @scala.inline
  def floralwhite: floralwhite = "floralwhite".asInstanceOf[floralwhite]
  
  @scala.inline
  def forestgreen: forestgreen = "forestgreen".asInstanceOf[forestgreen]
  
  @scala.inline
  def fuchsia: fuchsia = "fuchsia".asInstanceOf[fuchsia]
  
  @scala.inline
  def g: g = "g".asInstanceOf[g]
  
  @scala.inline
  def g16: g16 = "g16".asInstanceOf[g16]
  
  @scala.inline
  def gainsboro: gainsboro = "gainsboro".asInstanceOf[gainsboro]
  
  @scala.inline
  def ghostwhite: ghostwhite = "ghostwhite".asInstanceOf[ghostwhite]
  
  @scala.inline
  def gold: gold = "gold".asInstanceOf[gold]
  
  @scala.inline
  def goldenrod: goldenrod = "goldenrod".asInstanceOf[goldenrod]
  
  @scala.inline
  def gray: gray = "gray".asInstanceOf[gray]
  
  @scala.inline
  def green: green = "green".asInstanceOf[green]
  
  @scala.inline
  def greenyellow: greenyellow = "greenyellow".asInstanceOf[greenyellow]
  
  @scala.inline
  def grey: grey = "grey".asInstanceOf[grey]
  
  @scala.inline
  def h: h = "h".asInstanceOf[h]
  
  @scala.inline
  def hcg: hcg = "hcg".asInstanceOf[hcg]
  
  @scala.inline
  def hex: hex = "hex".asInstanceOf[hex]
  
  @scala.inline
  def honeydew: honeydew = "honeydew".asInstanceOf[honeydew]
  
  @scala.inline
  def hotpink: hotpink = "hotpink".asInstanceOf[hotpink]
  
  @scala.inline
  def hsl: hsl = "hsl".asInstanceOf[hsl]
  
  @scala.inline
  def hsv: hsv = "hsv".asInstanceOf[hsv]
  
  @scala.inline
  def hwb: hwb = "hwb".asInstanceOf[hwb]
  
  @scala.inline
  def indianred: indianred = "indianred".asInstanceOf[indianred]
  
  @scala.inline
  def indigo: indigo = "indigo".asInstanceOf[indigo]
  
  @scala.inline
  def ivory: ivory = "ivory".asInstanceOf[ivory]
  
  @scala.inline
  def keyword: keyword = "keyword".asInstanceOf[keyword]
  
  @scala.inline
  def khaki: khaki = "khaki".asInstanceOf[khaki]
  
  @scala.inline
  def lab: lab = "lab".asInstanceOf[lab]
  
  @scala.inline
  def lavender: lavender = "lavender".asInstanceOf[lavender]
  
  @scala.inline
  def lavenderblush: lavenderblush = "lavenderblush".asInstanceOf[lavenderblush]
  
  @scala.inline
  def lawngreen: lawngreen = "lawngreen".asInstanceOf[lawngreen]
  
  @scala.inline
  def lch: lch = "lch".asInstanceOf[lch]
  
  @scala.inline
  def lemonchiffon: lemonchiffon = "lemonchiffon".asInstanceOf[lemonchiffon]
  
  @scala.inline
  def lightblue: lightblue = "lightblue".asInstanceOf[lightblue]
  
  @scala.inline
  def lightcoral: lightcoral = "lightcoral".asInstanceOf[lightcoral]
  
  @scala.inline
  def lightcyan: lightcyan = "lightcyan".asInstanceOf[lightcyan]
  
  @scala.inline
  def lightgoldenrodyellow: lightgoldenrodyellow = "lightgoldenrodyellow".asInstanceOf[lightgoldenrodyellow]
  
  @scala.inline
  def lightgray: lightgray = "lightgray".asInstanceOf[lightgray]
  
  @scala.inline
  def lightgreen: lightgreen = "lightgreen".asInstanceOf[lightgreen]
  
  @scala.inline
  def lightgrey: lightgrey = "lightgrey".asInstanceOf[lightgrey]
  
  @scala.inline
  def lightpink: lightpink = "lightpink".asInstanceOf[lightpink]
  
  @scala.inline
  def lightsalmon: lightsalmon = "lightsalmon".asInstanceOf[lightsalmon]
  
  @scala.inline
  def lightseagreen: lightseagreen = "lightseagreen".asInstanceOf[lightseagreen]
  
  @scala.inline
  def lightskyblue: lightskyblue = "lightskyblue".asInstanceOf[lightskyblue]
  
  @scala.inline
  def lightslategray: lightslategray = "lightslategray".asInstanceOf[lightslategray]
  
  @scala.inline
  def lightslategrey: lightslategrey = "lightslategrey".asInstanceOf[lightslategrey]
  
  @scala.inline
  def lightsteelblue: lightsteelblue = "lightsteelblue".asInstanceOf[lightsteelblue]
  
  @scala.inline
  def lightyellow: lightyellow = "lightyellow".asInstanceOf[lightyellow]
  
  @scala.inline
  def lime: lime = "lime".asInstanceOf[lime]
  
  @scala.inline
  def limegreen: limegreen = "limegreen".asInstanceOf[limegreen]
  
  @scala.inline
  def linen: linen = "linen".asInstanceOf[linen]
  
  @scala.inline
  def magenta: magenta = "magenta".asInstanceOf[magenta]
  
  @scala.inline
  def maroon: maroon = "maroon".asInstanceOf[maroon]
  
  @scala.inline
  def mediumaquamarine: mediumaquamarine = "mediumaquamarine".asInstanceOf[mediumaquamarine]
  
  @scala.inline
  def mediumblue: mediumblue = "mediumblue".asInstanceOf[mediumblue]
  
  @scala.inline
  def mediumorchid: mediumorchid = "mediumorchid".asInstanceOf[mediumorchid]
  
  @scala.inline
  def mediumpurple: mediumpurple = "mediumpurple".asInstanceOf[mediumpurple]
  
  @scala.inline
  def mediumseagreen: mediumseagreen = "mediumseagreen".asInstanceOf[mediumseagreen]
  
  @scala.inline
  def mediumslateblue: mediumslateblue = "mediumslateblue".asInstanceOf[mediumslateblue]
  
  @scala.inline
  def mediumspringgreen: mediumspringgreen = "mediumspringgreen".asInstanceOf[mediumspringgreen]
  
  @scala.inline
  def mediumturquoise: mediumturquoise = "mediumturquoise".asInstanceOf[mediumturquoise]
  
  @scala.inline
  def mediumvioletred: mediumvioletred = "mediumvioletred".asInstanceOf[mediumvioletred]
  
  @scala.inline
  def midnightblue: midnightblue = "midnightblue".asInstanceOf[midnightblue]
  
  @scala.inline
  def mintcream: mintcream = "mintcream".asInstanceOf[mintcream]
  
  @scala.inline
  def mistyrose: mistyrose = "mistyrose".asInstanceOf[mistyrose]
  
  @scala.inline
  def moccasin: moccasin = "moccasin".asInstanceOf[moccasin]
  
  @scala.inline
  def navajowhite: navajowhite = "navajowhite".asInstanceOf[navajowhite]
  
  @scala.inline
  def navy: navy = "navy".asInstanceOf[navy]
  
  @scala.inline
  def oldlace: oldlace = "oldlace".asInstanceOf[oldlace]
  
  @scala.inline
  def olive: olive = "olive".asInstanceOf[olive]
  
  @scala.inline
  def olivedrab: olivedrab = "olivedrab".asInstanceOf[olivedrab]
  
  @scala.inline
  def orange: orange = "orange".asInstanceOf[orange]
  
  @scala.inline
  def orangered: orangered = "orangered".asInstanceOf[orangered]
  
  @scala.inline
  def orchid: orchid = "orchid".asInstanceOf[orchid]
  
  @scala.inline
  def palegoldenrod: palegoldenrod = "palegoldenrod".asInstanceOf[palegoldenrod]
  
  @scala.inline
  def palegreen: palegreen = "palegreen".asInstanceOf[palegreen]
  
  @scala.inline
  def paleturquoise: paleturquoise = "paleturquoise".asInstanceOf[paleturquoise]
  
  @scala.inline
  def palevioletred: palevioletred = "palevioletred".asInstanceOf[palevioletred]
  
  @scala.inline
  def papayawhip: papayawhip = "papayawhip".asInstanceOf[papayawhip]
  
  @scala.inline
  def peachpuff: peachpuff = "peachpuff".asInstanceOf[peachpuff]
  
  @scala.inline
  def peru: peru = "peru".asInstanceOf[peru]
  
  @scala.inline
  def pink: pink = "pink".asInstanceOf[pink]
  
  @scala.inline
  def plum: plum = "plum".asInstanceOf[plum]
  
  @scala.inline
  def powderblue: powderblue = "powderblue".asInstanceOf[powderblue]
  
  @scala.inline
  def purple: purple = "purple".asInstanceOf[purple]
  
  @scala.inline
  def r16: r16 = "r16".asInstanceOf[r16]
  
  @scala.inline
  def rebeccapurple: rebeccapurple = "rebeccapurple".asInstanceOf[rebeccapurple]
  
  @scala.inline
  def red: red = "red".asInstanceOf[red]
  
  @scala.inline
  def rgb: rgb = "rgb".asInstanceOf[rgb]
  
  @scala.inline
  def rosybrown: rosybrown = "rosybrown".asInstanceOf[rosybrown]
  
  @scala.inline
  def royalblue: royalblue = "royalblue".asInstanceOf[royalblue]
  
  @scala.inline
  def saddlebrown: saddlebrown = "saddlebrown".asInstanceOf[saddlebrown]
  
  @scala.inline
  def salmon: salmon = "salmon".asInstanceOf[salmon]
  
  @scala.inline
  def sandybrown: sandybrown = "sandybrown".asInstanceOf[sandybrown]
  
  @scala.inline
  def seagreen: seagreen = "seagreen".asInstanceOf[seagreen]
  
  @scala.inline
  def seashell: seashell = "seashell".asInstanceOf[seashell]
  
  @scala.inline
  def sienna: sienna = "sienna".asInstanceOf[sienna]
  
  @scala.inline
  def silver: silver = "silver".asInstanceOf[silver]
  
  @scala.inline
  def skyblue: skyblue = "skyblue".asInstanceOf[skyblue]
  
  @scala.inline
  def slateblue: slateblue = "slateblue".asInstanceOf[slateblue]
  
  @scala.inline
  def slategray: slategray = "slategray".asInstanceOf[slategray]
  
  @scala.inline
  def slategrey: slategrey = "slategrey".asInstanceOf[slategrey]
  
  @scala.inline
  def snow: snow = "snow".asInstanceOf[snow]
  
  @scala.inline
  def springgreen: springgreen = "springgreen".asInstanceOf[springgreen]
  
  @scala.inline
  def steelblue: steelblue = "steelblue".asInstanceOf[steelblue]
  
  @scala.inline
  def tan: tan = "tan".asInstanceOf[tan]
  
  @scala.inline
  def teal: teal = "teal".asInstanceOf[teal]
  
  @scala.inline
  def thistle: thistle = "thistle".asInstanceOf[thistle]
  
  @scala.inline
  def tomato: tomato = "tomato".asInstanceOf[tomato]
  
  @scala.inline
  def turquoise: turquoise = "turquoise".asInstanceOf[turquoise]
  
  @scala.inline
  def violet: violet = "violet".asInstanceOf[violet]
  
  @scala.inline
  def wheat: wheat = "wheat".asInstanceOf[wheat]
  
  @scala.inline
  def white: white = "white".asInstanceOf[white]
  
  @scala.inline
  def whitesmoke: whitesmoke = "whitesmoke".asInstanceOf[whitesmoke]
  
  @scala.inline
  def xyz: xyz = "xyz".asInstanceOf[xyz]
  
  @scala.inline
  def yellow: yellow = "yellow".asInstanceOf[yellow]
  
  @scala.inline
  def yellowgreen: yellowgreen = "yellowgreen".asInstanceOf[yellowgreen]
  
  @js.native
  sealed trait aliceblue extends KEYWORD_
  
  @js.native
  sealed trait ansi16 extends js.Object
  
  @js.native
  sealed trait ansi256 extends js.Object
  
  @js.native
  sealed trait antiquewhite extends KEYWORD_
  
  @js.native
  sealed trait apple extends js.Object
  
  @js.native
  sealed trait aqua extends KEYWORD_
  
  @js.native
  sealed trait aquamarine extends KEYWORD_
  
  @js.native
  sealed trait azure extends KEYWORD_
  
  @js.native
  sealed trait b16 extends js.Object
  
  @js.native
  sealed trait beige extends KEYWORD_
  
  @js.native
  sealed trait bisque extends KEYWORD_
  
  @js.native
  sealed trait black extends KEYWORD_
  
  @js.native
  sealed trait blanchedalmond extends KEYWORD_
  
  @js.native
  sealed trait blue extends KEYWORD_
  
  @js.native
  sealed trait blueviolet extends KEYWORD_
  
  @js.native
  sealed trait brown extends KEYWORD_
  
  @js.native
  sealed trait burlywood extends KEYWORD_
  
  @js.native
  sealed trait c extends js.Object
  
  @js.native
  sealed trait cadetblue extends KEYWORD_
  
  @js.native
  sealed trait chartreuse extends KEYWORD_
  
  @js.native
  sealed trait chocolate extends KEYWORD_
  
  @js.native
  sealed trait cmyk extends js.Object
  
  @js.native
  sealed trait coral extends KEYWORD_
  
  @js.native
  sealed trait cornflowerblue extends KEYWORD_
  
  @js.native
  sealed trait cornsilk extends KEYWORD_
  
  @js.native
  sealed trait crimson extends KEYWORD_
  
  @js.native
  sealed trait cyan extends KEYWORD_
  
  @js.native
  sealed trait darkblue extends KEYWORD_
  
  @js.native
  sealed trait darkcyan extends KEYWORD_
  
  @js.native
  sealed trait darkgoldenrod extends KEYWORD_
  
  @js.native
  sealed trait darkgray extends KEYWORD_
  
  @js.native
  sealed trait darkgreen extends KEYWORD_
  
  @js.native
  sealed trait darkgrey extends KEYWORD_
  
  @js.native
  sealed trait darkkhaki extends KEYWORD_
  
  @js.native
  sealed trait darkmagenta extends KEYWORD_
  
  @js.native
  sealed trait darkolivegreen extends KEYWORD_
  
  @js.native
  sealed trait darkorange extends KEYWORD_
  
  @js.native
  sealed trait darkorchid extends KEYWORD_
  
  @js.native
  sealed trait darkred extends KEYWORD_
  
  @js.native
  sealed trait darksalmon extends KEYWORD_
  
  @js.native
  sealed trait darkseagreen extends KEYWORD_
  
  @js.native
  sealed trait darkslateblue extends KEYWORD_
  
  @js.native
  sealed trait darkslategray extends KEYWORD_
  
  @js.native
  sealed trait darkslategrey extends KEYWORD_
  
  @js.native
  sealed trait darkturquoise extends KEYWORD_
  
  @js.native
  sealed trait darkviolet extends KEYWORD_
  
  @js.native
  sealed trait deeppink extends KEYWORD_
  
  @js.native
  sealed trait deepskyblue extends KEYWORD_
  
  @js.native
  sealed trait dimgray extends KEYWORD_
  
  @js.native
  sealed trait dimgrey extends KEYWORD_
  
  @js.native
  sealed trait dodgerblue extends KEYWORD_
  
  @js.native
  sealed trait firebrick extends KEYWORD_
  
  @js.native
  sealed trait floralwhite extends KEYWORD_
  
  @js.native
  sealed trait forestgreen extends KEYWORD_
  
  @js.native
  sealed trait fuchsia extends KEYWORD_
  
  @js.native
  sealed trait g extends js.Object
  
  @js.native
  sealed trait g16 extends js.Object
  
  @js.native
  sealed trait gainsboro extends KEYWORD_
  
  @js.native
  sealed trait ghostwhite extends KEYWORD_
  
  @js.native
  sealed trait gold extends KEYWORD_
  
  @js.native
  sealed trait goldenrod extends KEYWORD_
  
  @js.native
  sealed trait gray extends KEYWORD_
  
  @js.native
  sealed trait green extends KEYWORD_
  
  @js.native
  sealed trait greenyellow extends KEYWORD_
  
  @js.native
  sealed trait grey extends KEYWORD_
  
  @js.native
  sealed trait h extends js.Object
  
  @js.native
  sealed trait hcg extends js.Object
  
  @js.native
  sealed trait hex extends js.Object
  
  @js.native
  sealed trait honeydew extends KEYWORD_
  
  @js.native
  sealed trait hotpink extends KEYWORD_
  
  @js.native
  sealed trait hsl extends js.Object
  
  @js.native
  sealed trait hsv extends js.Object
  
  @js.native
  sealed trait hwb extends js.Object
  
  @js.native
  sealed trait indianred extends KEYWORD_
  
  @js.native
  sealed trait indigo extends KEYWORD_
  
  @js.native
  sealed trait ivory extends KEYWORD_
  
  @js.native
  sealed trait keyword extends js.Object
  
  @js.native
  sealed trait khaki extends KEYWORD_
  
  @js.native
  sealed trait lab extends js.Object
  
  @js.native
  sealed trait lavender extends KEYWORD_
  
  @js.native
  sealed trait lavenderblush extends KEYWORD_
  
  @js.native
  sealed trait lawngreen extends KEYWORD_
  
  @js.native
  sealed trait lch extends js.Object
  
  @js.native
  sealed trait lemonchiffon extends KEYWORD_
  
  @js.native
  sealed trait lightblue extends KEYWORD_
  
  @js.native
  sealed trait lightcoral extends KEYWORD_
  
  @js.native
  sealed trait lightcyan extends KEYWORD_
  
  @js.native
  sealed trait lightgoldenrodyellow extends KEYWORD_
  
  @js.native
  sealed trait lightgray extends KEYWORD_
  
  @js.native
  sealed trait lightgreen extends KEYWORD_
  
  @js.native
  sealed trait lightgrey extends KEYWORD_
  
  @js.native
  sealed trait lightpink extends KEYWORD_
  
  @js.native
  sealed trait lightsalmon extends KEYWORD_
  
  @js.native
  sealed trait lightseagreen extends KEYWORD_
  
  @js.native
  sealed trait lightskyblue extends KEYWORD_
  
  @js.native
  sealed trait lightslategray extends KEYWORD_
  
  @js.native
  sealed trait lightslategrey extends KEYWORD_
  
  @js.native
  sealed trait lightsteelblue extends KEYWORD_
  
  @js.native
  sealed trait lightyellow extends KEYWORD_
  
  @js.native
  sealed trait lime extends KEYWORD_
  
  @js.native
  sealed trait limegreen extends KEYWORD_
  
  @js.native
  sealed trait linen extends KEYWORD_
  
  @js.native
  sealed trait magenta extends KEYWORD_
  
  @js.native
  sealed trait maroon extends KEYWORD_
  
  @js.native
  sealed trait mediumaquamarine extends KEYWORD_
  
  @js.native
  sealed trait mediumblue extends KEYWORD_
  
  @js.native
  sealed trait mediumorchid extends KEYWORD_
  
  @js.native
  sealed trait mediumpurple extends KEYWORD_
  
  @js.native
  sealed trait mediumseagreen extends KEYWORD_
  
  @js.native
  sealed trait mediumslateblue extends KEYWORD_
  
  @js.native
  sealed trait mediumspringgreen extends KEYWORD_
  
  @js.native
  sealed trait mediumturquoise extends KEYWORD_
  
  @js.native
  sealed trait mediumvioletred extends KEYWORD_
  
  @js.native
  sealed trait midnightblue extends KEYWORD_
  
  @js.native
  sealed trait mintcream extends KEYWORD_
  
  @js.native
  sealed trait mistyrose extends KEYWORD_
  
  @js.native
  sealed trait moccasin extends KEYWORD_
  
  @js.native
  sealed trait navajowhite extends KEYWORD_
  
  @js.native
  sealed trait navy extends KEYWORD_
  
  @js.native
  sealed trait oldlace extends KEYWORD_
  
  @js.native
  sealed trait olive extends KEYWORD_
  
  @js.native
  sealed trait olivedrab extends KEYWORD_
  
  @js.native
  sealed trait orange extends KEYWORD_
  
  @js.native
  sealed trait orangered extends KEYWORD_
  
  @js.native
  sealed trait orchid extends KEYWORD_
  
  @js.native
  sealed trait palegoldenrod extends KEYWORD_
  
  @js.native
  sealed trait palegreen extends KEYWORD_
  
  @js.native
  sealed trait paleturquoise extends KEYWORD_
  
  @js.native
  sealed trait palevioletred extends KEYWORD_
  
  @js.native
  sealed trait papayawhip extends KEYWORD_
  
  @js.native
  sealed trait peachpuff extends KEYWORD_
  
  @js.native
  sealed trait peru extends KEYWORD_
  
  @js.native
  sealed trait pink extends KEYWORD_
  
  @js.native
  sealed trait plum extends KEYWORD_
  
  @js.native
  sealed trait powderblue extends KEYWORD_
  
  @js.native
  sealed trait purple extends KEYWORD_
  
  @js.native
  sealed trait r16 extends js.Object
  
  @js.native
  sealed trait rebeccapurple extends KEYWORD_
  
  @js.native
  sealed trait red extends KEYWORD_
  
  @js.native
  sealed trait rgb extends js.Object
  
  @js.native
  sealed trait rosybrown extends KEYWORD_
  
  @js.native
  sealed trait royalblue extends KEYWORD_
  
  @js.native
  sealed trait saddlebrown extends KEYWORD_
  
  @js.native
  sealed trait salmon extends KEYWORD_
  
  @js.native
  sealed trait sandybrown extends KEYWORD_
  
  @js.native
  sealed trait seagreen extends KEYWORD_
  
  @js.native
  sealed trait seashell extends KEYWORD_
  
  @js.native
  sealed trait sienna extends KEYWORD_
  
  @js.native
  sealed trait silver extends KEYWORD_
  
  @js.native
  sealed trait skyblue extends KEYWORD_
  
  @js.native
  sealed trait slateblue extends KEYWORD_
  
  @js.native
  sealed trait slategray extends KEYWORD_
  
  @js.native
  sealed trait slategrey extends KEYWORD_
  
  @js.native
  sealed trait snow extends KEYWORD_
  
  @js.native
  sealed trait springgreen extends KEYWORD_
  
  @js.native
  sealed trait steelblue extends KEYWORD_
  
  @js.native
  sealed trait tan extends KEYWORD_
  
  @js.native
  sealed trait teal extends KEYWORD_
  
  @js.native
  sealed trait thistle extends KEYWORD_
  
  @js.native
  sealed trait tomato extends KEYWORD_
  
  @js.native
  sealed trait turquoise extends KEYWORD_
  
  @js.native
  sealed trait violet extends KEYWORD_
  
  @js.native
  sealed trait wheat extends KEYWORD_
  
  @js.native
  sealed trait white extends KEYWORD_
  
  @js.native
  sealed trait whitesmoke extends KEYWORD_
  
  @js.native
  sealed trait xyz extends js.Object
  
  @js.native
  sealed trait yellow extends KEYWORD_
  
  @js.native
  sealed trait yellowgreen extends KEYWORD_
}
