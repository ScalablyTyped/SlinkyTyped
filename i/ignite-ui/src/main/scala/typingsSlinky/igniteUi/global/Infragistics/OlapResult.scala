package typingsSlinky.igniteUi.global.Infragistics

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("Infragistics.OlapResult")
@js.native
class OlapResult ()
  extends typingsSlinky.igniteUi.Infragistics.OlapResult {
  /**
    * Returns an array of $.ig.OlapResultAxis objects this result is built on.
    *
    * @param value
    */
  /* CompleteClass */
  override def axes(value: js.Object): js.Array[_] = js.native
  /**
    * Returns an array of $.ig.OlapResultCell objects which hold the result data.
    *
    * @param value
    */
  /* CompleteClass */
  override def cells(value: js.Object): js.Array[_] = js.native
  /**
    * Returns a value indicating whether the result object contains any data.
    *
    * @param value
    */
  /* CompleteClass */
  override def isEmpty(value: js.Object): Boolean = js.native
}

