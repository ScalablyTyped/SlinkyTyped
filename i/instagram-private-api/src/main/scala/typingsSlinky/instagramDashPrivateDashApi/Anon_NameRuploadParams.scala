package typingsSlinky.instagramDashPrivateDashApi

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_NameRuploadParams extends js.Object {
  var name: js.Any
  var ruploadParams: js.Any
  var waterfallId: js.Any
}

object Anon_NameRuploadParams {
  @scala.inline
  def apply(name: js.Any, ruploadParams: js.Any, waterfallId: js.Any): Anon_NameRuploadParams = {
    val __obj = js.Dynamic.literal(name = name.asInstanceOf[js.Any], ruploadParams = ruploadParams.asInstanceOf[js.Any], waterfallId = waterfallId.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon_NameRuploadParams]
  }
}

