...
object mod {
  ...
  trait Visitor extends StObject {
    ...
    def exit(path: String): typingsSlinky.istanbulLibInstrument.anon.FileCoverage = js.native
  }
  object Visitor {
    
    @scala.inline
    def apply(enter: String => Unit, exit: String => typingsSlinky.istanbulLibInstrument.anon.FileCoverage): Visitor = {
    ...
    implicit class VisitorMutableBuilder[Self <: Visitor] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setExit(value: String => typingsSlinky.istanbulLibInstrument.anon.FileCoverage): Self = StObject.set(x, "exit", js.Any.fromFunction1(value))
    }
  }
