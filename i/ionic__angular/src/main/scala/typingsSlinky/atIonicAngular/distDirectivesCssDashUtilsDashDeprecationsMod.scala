package typingsSlinky.atIonicAngular

import typingsSlinky.atAngularCore.atAngularCoreMod.ElementRef
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@ionic/angular/dist/directives/css-utils-deprecations", JSImport.Namespace)
@js.native
object distDirectivesCssDashUtilsDashDeprecationsMod extends js.Object {
  @js.native
  class CssUtilsDeprecations protected () extends js.Object {
    def this(ref: ElementRef[_]) = this()
  }
  
}

