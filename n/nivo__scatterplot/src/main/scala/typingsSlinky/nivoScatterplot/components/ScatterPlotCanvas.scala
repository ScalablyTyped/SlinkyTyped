...
object ScatterPlotCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoScatterplot.mod.ScatterPlotCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoScatterplot.mod.ScatterPlotCanvas] = {
  ...
  type Props = ScatterPlotCanvasProps with Dimensions
}
