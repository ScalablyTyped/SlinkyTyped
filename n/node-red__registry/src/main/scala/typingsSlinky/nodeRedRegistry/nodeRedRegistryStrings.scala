package typingsSlinky.nodeRedRegistry

import typingsSlinky.nodeRedRegistry.mod.NodeStatusFill
import typingsSlinky.nodeRedRegistry.mod.NodeStatusShape
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nodeRedRegistryStrings {
  
  @scala.inline
  def NodeCredentials: NodeCredentials = "NodeCredentials".asInstanceOf[NodeCredentials]
  
  @scala.inline
  def NodeSettings: NodeSettings = "NodeSettings".asInstanceOf[NodeSettings]
  
  @scala.inline
  def blue: blue = "blue".asInstanceOf[blue]
  
  @scala.inline
  def close: close = "close".asInstanceOf[close]
  
  @scala.inline
  def dot: dot = "dot".asInstanceOf[dot]
  
  @scala.inline
  def green: green = "green".asInstanceOf[green]
  
  @scala.inline
  def grey: grey = "grey".asInstanceOf[grey]
  
  @scala.inline
  def init: init = "init".asInstanceOf[init]
  
  @scala.inline
  def input: input = "input".asInstanceOf[input]
  
  @scala.inline
  def password: password = "password".asInstanceOf[password]
  
  @scala.inline
  def red: red = "red".asInstanceOf[red]
  
  @scala.inline
  def ring: ring = "ring".asInstanceOf[ring]
  
  @scala.inline
  def text: text = "text".asInstanceOf[text]
  
  @scala.inline
  def yellow: yellow = "yellow".asInstanceOf[yellow]
  
  @js.native
  sealed trait NodeCredentials extends js.Object
  
  @js.native
  sealed trait NodeSettings extends js.Object
  
  @js.native
  sealed trait blue extends NodeStatusFill
  
  @js.native
  sealed trait close extends js.Object
  
  @js.native
  sealed trait dot extends NodeStatusShape
  
  @js.native
  sealed trait green extends NodeStatusFill
  
  @js.native
  sealed trait grey extends NodeStatusFill
  
  @js.native
  sealed trait init extends js.Object
  
  @js.native
  sealed trait input extends js.Object
  
  @js.native
  sealed trait password extends js.Object
  
  @js.native
  sealed trait red extends NodeStatusFill
  
  @js.native
  sealed trait ring extends NodeStatusShape
  
  @js.native
  sealed trait text extends js.Object
  
  @js.native
  sealed trait yellow extends NodeStatusFill
}
