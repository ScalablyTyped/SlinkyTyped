...
object Radar
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoRadar.mod.Radar] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoRadar.mod.Radar] = {
  ...
  type Props = RadarProps with Dimensions
}
