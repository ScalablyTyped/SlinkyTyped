...
object GetDetails {
  ...
  implicit class GetDetailsMutableBuilder[Self <: GetDetails] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setResourceIdentifier(value: typingsSlinky.chrome.chrome.contentSettings.ResourceIdentifier): Self = StObject.set(x, "resourceIdentifier", value.asInstanceOf[js.Any])
