...
object mailComposerMod {
  ...
  object MailComposer {
    ...
    implicit class MailComposerMutableBuilder[Self <: MailComposer] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setCompile(value: () => typingsSlinky.nodemailer.mimeNodeMod.^): Self = StObject.set(x, "compile", js.Any.fromFunction0(value))
      ...
      @scala.inline
      def setMessage(value: typingsSlinky.nodemailer.mimeNodeMod.^ | `false`): Self = StObject.set(x, "message", value.asInstanceOf[js.Any])
    }
  }
}
