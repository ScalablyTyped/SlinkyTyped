...
object Sankey
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoSankey.mod.Sankey] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoSankey.mod.Sankey] = {
  ...
  type Props = Data with SankeyProps with Dimensions
}
