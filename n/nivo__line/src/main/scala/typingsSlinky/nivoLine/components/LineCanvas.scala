...
object LineCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoLine.mod.LineCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoLine.mod.LineCanvas] = {
  ...
  type Props = LineCanvasProps with Dimensions
}
