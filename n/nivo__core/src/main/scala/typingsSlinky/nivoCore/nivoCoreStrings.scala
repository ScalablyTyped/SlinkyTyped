package typingsSlinky.nivoCore

import typingsSlinky.nivoCore.mod.AreaCurve
import typingsSlinky.nivoCore.mod.BoxAlign
import typingsSlinky.nivoCore.mod.CssMixBlendMode
import typingsSlinky.nivoCore.mod.StackOffset
import typingsSlinky.nivoCore.mod.StackOrder
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nivoCoreStrings {
  
  @scala.inline
  def Asterisk: Asterisk = "*".asInstanceOf[Asterisk]
  
  @scala.inline
  def ascending: ascending = "ascending".asInstanceOf[ascending]
  
  @scala.inline
  def basis: basis = "basis".asInstanceOf[basis]
  
  @scala.inline
  def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @scala.inline
  def `bottom-left`: `bottom-left` = "bottom-left".asInstanceOf[`bottom-left`]
  
  @scala.inline
  def `bottom-right`: `bottom-right` = "bottom-right".asInstanceOf[`bottom-right`]
  
  @scala.inline
  def cardinal: cardinal = "cardinal".asInstanceOf[cardinal]
  
  @scala.inline
  def catmullRom: catmullRom = "catmullRom".asInstanceOf[catmullRom]
  
  @scala.inline
  def center: center = "center".asInstanceOf[center]
  
  @scala.inline
  def color: color = "color".asInstanceOf[color]
  
  @scala.inline
  def `color-burn`: `color-burn` = "color-burn".asInstanceOf[`color-burn`]
  
  @scala.inline
  def `color-dodge`: `color-dodge` = "color-dodge".asInstanceOf[`color-dodge`]
  
  @scala.inline
  def darken: darken = "darken".asInstanceOf[darken]
  
  @scala.inline
  def descending: descending = "descending".asInstanceOf[descending]
  
  @scala.inline
  def difference: difference = "difference".asInstanceOf[difference]
  
  @scala.inline
  def diverging: diverging = "diverging".asInstanceOf[diverging]
  
  @scala.inline
  def exclusion: exclusion = "exclusion".asInstanceOf[exclusion]
  
  @scala.inline
  def expand: expand = "expand".asInstanceOf[expand]
  
  @scala.inline
  def `hard-light`: `hard-light` = "hard-light".asInstanceOf[`hard-light`]
  
  @scala.inline
  def hue: hue = "hue".asInstanceOf[hue]
  
  @scala.inline
  def insideOut: insideOut = "insideOut".asInstanceOf[insideOut]
  
  @scala.inline
  def left: left = "left".asInstanceOf[left]
  
  @scala.inline
  def lighten: lighten = "lighten".asInstanceOf[lighten]
  
  @scala.inline
  def linear: linear = "linear".asInstanceOf[linear]
  
  @scala.inline
  def luminosity: luminosity = "luminosity".asInstanceOf[luminosity]
  
  @scala.inline
  def monotoneX: monotoneX = "monotoneX".asInstanceOf[monotoneX]
  
  @scala.inline
  def monotoneY: monotoneY = "monotoneY".asInstanceOf[monotoneY]
  
  @scala.inline
  def multiply: multiply = "multiply".asInstanceOf[multiply]
  
  @scala.inline
  def natural: natural = "natural".asInstanceOf[natural]
  
  @scala.inline
  def none: none = "none".asInstanceOf[none]
  
  @scala.inline
  def normal: normal = "normal".asInstanceOf[normal]
  
  @scala.inline
  def overlay: overlay = "overlay".asInstanceOf[overlay]
  
  @scala.inline
  def reverse: reverse = "reverse".asInstanceOf[reverse]
  
  @scala.inline
  def right: right = "right".asInstanceOf[right]
  
  @scala.inline
  def saturation: saturation = "saturation".asInstanceOf[saturation]
  
  @scala.inline
  def screen: screen = "screen".asInstanceOf[screen]
  
  @scala.inline
  def silhouette: silhouette = "silhouette".asInstanceOf[silhouette]
  
  @scala.inline
  def `soft-light`: `soft-light` = "soft-light".asInstanceOf[`soft-light`]
  
  @scala.inline
  def step: step = "step".asInstanceOf[step]
  
  @scala.inline
  def stepAfter: stepAfter = "stepAfter".asInstanceOf[stepAfter]
  
  @scala.inline
  def stepBefore: stepBefore = "stepBefore".asInstanceOf[stepBefore]
  
  @scala.inline
  def top: top = "top".asInstanceOf[top]
  
  @scala.inline
  def `top-left`: `top-left` = "top-left".asInstanceOf[`top-left`]
  
  @scala.inline
  def `top-right`: `top-right` = "top-right".asInstanceOf[`top-right`]
  
  @scala.inline
  def wiggle: wiggle = "wiggle".asInstanceOf[wiggle]
  
  @scala.inline
  def x: x = "x".asInstanceOf[x]
  
  @scala.inline
  def y: y = "y".asInstanceOf[y]
  
  @js.native
  sealed trait Asterisk extends js.Object
  
  @js.native
  sealed trait ascending extends StackOrder
  
  @js.native
  sealed trait basis extends AreaCurve
  
  @js.native
  sealed trait bottom extends BoxAlign
  
  @js.native
  sealed trait `bottom-left` extends BoxAlign
  
  @js.native
  sealed trait `bottom-right` extends BoxAlign
  
  @js.native
  sealed trait cardinal extends AreaCurve
  
  @js.native
  sealed trait catmullRom extends AreaCurve
  
  @js.native
  sealed trait center extends BoxAlign
  
  @js.native
  sealed trait color extends CssMixBlendMode
  
  @js.native
  sealed trait `color-burn` extends CssMixBlendMode
  
  @js.native
  sealed trait `color-dodge` extends CssMixBlendMode
  
  @js.native
  sealed trait darken extends CssMixBlendMode
  
  @js.native
  sealed trait descending extends StackOrder
  
  @js.native
  sealed trait difference extends CssMixBlendMode
  
  @js.native
  sealed trait diverging extends StackOffset
  
  @js.native
  sealed trait exclusion extends CssMixBlendMode
  
  @js.native
  sealed trait expand extends StackOffset
  
  @js.native
  sealed trait `hard-light` extends CssMixBlendMode
  
  @js.native
  sealed trait hue extends CssMixBlendMode
  
  @js.native
  sealed trait insideOut extends StackOrder
  
  @js.native
  sealed trait left extends BoxAlign
  
  @js.native
  sealed trait lighten extends CssMixBlendMode
  
  @js.native
  sealed trait linear extends AreaCurve
  
  @js.native
  sealed trait luminosity extends CssMixBlendMode
  
  @js.native
  sealed trait monotoneX extends AreaCurve
  
  @js.native
  sealed trait monotoneY extends AreaCurve
  
  @js.native
  sealed trait multiply extends CssMixBlendMode
  
  @js.native
  sealed trait natural extends AreaCurve
  
  @js.native
  sealed trait none
    extends StackOffset
       with StackOrder
  
  @js.native
  sealed trait normal extends CssMixBlendMode
  
  @js.native
  sealed trait overlay extends CssMixBlendMode
  
  @js.native
  sealed trait reverse extends StackOrder
  
  @js.native
  sealed trait right extends BoxAlign
  
  @js.native
  sealed trait saturation extends CssMixBlendMode
  
  @js.native
  sealed trait screen extends CssMixBlendMode
  
  @js.native
  sealed trait silhouette extends StackOffset
  
  @js.native
  sealed trait `soft-light` extends CssMixBlendMode
  
  @js.native
  sealed trait step extends AreaCurve
  
  @js.native
  sealed trait stepAfter extends AreaCurve
  
  @js.native
  sealed trait stepBefore extends AreaCurve
  
  @js.native
  sealed trait top extends BoxAlign
  
  @js.native
  sealed trait `top-left` extends BoxAlign
  
  @js.native
  sealed trait `top-right` extends BoxAlign
  
  @js.native
  sealed trait wiggle extends StackOffset
  
  @js.native
  sealed trait x extends js.Object
  
  @js.native
  sealed trait y extends js.Object
}
