...
object ChordCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoChord.mod.ChordCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoChord.mod.ChordCanvas] = {
  ...
  type Props = ChordCanvasProps with Dimensions
}
