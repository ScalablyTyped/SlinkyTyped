package typingsSlinky.nodegit.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("nodegit", "Libgit2")
@js.native
class Libgit2 ()
  extends typingsSlinky.nodegit.libGit2Mod.Libgit2
/* static members */
object Libgit2 {
  
  @JSImport("nodegit", "Libgit2.features")
  @js.native
  def features(): Double = js.native
  
  @JSImport("nodegit", "Libgit2.init")
  @js.native
  def init(): Double = js.native
  
  @JSImport("nodegit", "Libgit2.opts")
  @js.native
  def opts(option: Double): Double = js.native
  
  @JSImport("nodegit", "Libgit2.shutdown")
  @js.native
  def shutdown(): Double = js.native
  
  @JSImport("nodegit", "Libgit2.version")
  @js.native
  def version(major: Double, minor: Double, rev: Double): Unit = js.native
}
