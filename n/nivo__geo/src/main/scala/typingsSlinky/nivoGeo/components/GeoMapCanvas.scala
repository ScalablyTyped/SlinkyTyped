...
object GeoMapCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoGeo.mod.GeoMapCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoGeo.mod.GeoMapCanvas] = {
  ...
  type Props = GeoMapCanvasProps with Dimensions
}
