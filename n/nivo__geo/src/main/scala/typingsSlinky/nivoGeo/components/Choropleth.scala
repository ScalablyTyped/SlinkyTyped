...
object Choropleth
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoGeo.mod.Choropleth] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoGeo.mod.Choropleth] = {
  ...
  type Props = ChoroplethProps with Dimensions
}
