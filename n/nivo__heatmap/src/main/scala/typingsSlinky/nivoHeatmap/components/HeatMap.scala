...
object HeatMap
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoHeatmap.mod.HeatMap] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoHeatmap.mod.HeatMap] = {
  ...
  type Props = HeatMapSvgProps with Dimensions
}
