...
object HeatMapCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoHeatmap.mod.HeatMapCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoHeatmap.mod.HeatMapCanvas] = {
  ...
  type Props = HeatMapCanvasProps with Dimensions
}
