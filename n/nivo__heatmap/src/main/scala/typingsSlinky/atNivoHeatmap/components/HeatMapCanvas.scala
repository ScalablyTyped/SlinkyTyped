...
object HeatMapCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.atNivoHeatmap.atNivoHeatmapMod.HeatMapCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.atNivoHeatmap.atNivoHeatmapMod.HeatMapCanvas] = {
  ...
  type Props = HeatMapCanvasPropsDimensions
}
