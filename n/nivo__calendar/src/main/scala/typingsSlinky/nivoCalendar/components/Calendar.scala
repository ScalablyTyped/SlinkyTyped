...
object Calendar
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoCalendar.mod.Calendar] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoCalendar.mod.Calendar] = {
  ...
  type Props = CalendarSvgProps with Dimensions
}
