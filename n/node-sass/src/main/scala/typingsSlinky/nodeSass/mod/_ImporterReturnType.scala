package typingsSlinky.nodeSass.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait _ImporterReturnType extends StObject
object _ImporterReturnType {
  
  @scala.inline
  def Contents(contents: String): typingsSlinky.nodeSass.anon.Contents = {
    val __obj = js.Dynamic.literal(contents = contents.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsSlinky.nodeSass.anon.Contents]
  }
  
  @scala.inline
  def Error(___SASS_ERROR___ : js.Symbol): typingsSlinky.nodeSass.mod.types.Error = {
    val __obj = js.Dynamic.literal(___SASS_ERROR___ = ___SASS_ERROR___.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsSlinky.nodeSass.mod.types.Error]
  }
  
  @scala.inline
  def File(file: String): typingsSlinky.nodeSass.anon.File = {
    val __obj = js.Dynamic.literal(file = file.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsSlinky.nodeSass.anon.File]
  }
  
  @scala.inline
  def Null(___NULL___ : js.Symbol): typingsSlinky.nodeSass.mod.types.Null = {
    val __obj = js.Dynamic.literal(___NULL___ = ___NULL___.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsSlinky.nodeSass.mod.types.Null]
  }
}
