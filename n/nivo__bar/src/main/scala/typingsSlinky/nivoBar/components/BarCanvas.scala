...
object BarCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoBar.mod.BarCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoBar.mod.BarCanvas] = {
  ...
  type Props = BarCanvasProps with Dimensions
}
