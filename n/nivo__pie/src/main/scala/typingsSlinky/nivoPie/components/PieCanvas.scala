...
object PieCanvas
  extends ExternalComponentWithAttributesWithRefType[tag.type, typingsSlinky.nivoPie.mod.PieCanvas] {
  ...
  ): BuildingComponent[tag.type, typingsSlinky.nivoPie.mod.PieCanvas] = {
  ...
  type Props = PieCanvasProps with Dimensions
}
