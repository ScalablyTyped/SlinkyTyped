...
import typingsSlinky.knockout.knockoutStrings.added
import typingsSlinky.knockout.knockoutStrings.deleted
import typingsSlinky.knockout.knockoutStrings.retained
...
object ArrayChange {
  ...
  implicit class ArrayChangeOps[Self <: ArrayChange[_], T] (val x: Self with ArrayChange[T]) extends AnyVal {
    ...
    @scala.inline
    def setStatus(value: added | deleted | retained): Self = this.set("status", value.asInstanceOf[js.Any])
    @scala.inline
