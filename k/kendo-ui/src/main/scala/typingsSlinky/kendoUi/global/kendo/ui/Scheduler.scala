package typingsSlinky.kendoUi.global.kendo.ui

import org.scalajs.dom.raw.Element
import typingsSlinky.kendoUi.kendo.ui.SchedulerOptions
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("kendo.ui.Scheduler")
@js.native
class Scheduler protected ()
  extends typingsSlinky.kendoUi.kendo.ui.Scheduler {
  def this(element: Element) = this()
  def this(element: Element, options: SchedulerOptions) = this()
}
object Scheduler {
  
  @JSGlobal("kendo.ui.Scheduler")
  @js.native
  val ^ : js.Any = js.native
  
  /* static member */
  @JSGlobal("kendo.ui.Scheduler.extend")
  @js.native
  def extend(proto: js.Object): typingsSlinky.kendoUi.kendo.ui.Scheduler = js.native
  
  /* static member */
  @JSGlobal("kendo.ui.Scheduler.fn")
  @js.native
  def fn: typingsSlinky.kendoUi.kendo.ui.Scheduler = js.native
  @scala.inline
  def fn_=(x: typingsSlinky.kendoUi.kendo.ui.Scheduler): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("fn")(x.asInstanceOf[js.Any])
}
