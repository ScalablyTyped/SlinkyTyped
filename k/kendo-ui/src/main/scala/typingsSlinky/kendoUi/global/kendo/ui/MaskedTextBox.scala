package typingsSlinky.kendoUi.global.kendo.ui

import org.scalajs.dom.raw.Element
import typingsSlinky.kendoUi.kendo.ui.MaskedTextBoxOptions
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("kendo.ui.MaskedTextBox")
@js.native
class MaskedTextBox protected ()
  extends typingsSlinky.kendoUi.kendo.ui.MaskedTextBox {
  def this(element: Element) = this()
  def this(element: Element, options: MaskedTextBoxOptions) = this()
}
object MaskedTextBox {
  
  @JSGlobal("kendo.ui.MaskedTextBox")
  @js.native
  val ^ : js.Any = js.native
  
  /* static member */
  @JSGlobal("kendo.ui.MaskedTextBox.extend")
  @js.native
  def extend(proto: js.Object): typingsSlinky.kendoUi.kendo.ui.MaskedTextBox = js.native
  
  /* static member */
  @JSGlobal("kendo.ui.MaskedTextBox.fn")
  @js.native
  def fn: typingsSlinky.kendoUi.kendo.ui.MaskedTextBox = js.native
  @scala.inline
  def fn_=(x: typingsSlinky.kendoUi.kendo.ui.MaskedTextBox): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("fn")(x.asInstanceOf[js.Any])
}
