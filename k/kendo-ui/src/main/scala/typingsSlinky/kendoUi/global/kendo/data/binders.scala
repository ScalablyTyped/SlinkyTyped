package typingsSlinky.kendoUi.global.kendo.data

import typingsSlinky.kendoUi.kendo.data.DataSourceBinder
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object binders {
  
  @JSGlobal("kendo.data.binders.attr")
  @js.native
  class attr ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.css")
  @js.native
  class css ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.dataSourceBinding")
  @js.native
  def dataSourceBinding(bindingName: String, fieldName: String, setter: String): DataSourceBinder = js.native
  
  @JSGlobal("kendo.data.binders.disabled")
  @js.native
  class disabled ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.enabled")
  @js.native
  class enabled ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.events")
  @js.native
  class events ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.html")
  @js.native
  class html ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  object input {
    
    @JSGlobal("kendo.data.binders.input.checked")
    @js.native
    class checked ()
      extends typingsSlinky.kendoUi.kendo.data.binders.input.checked
  }
  
  @JSGlobal("kendo.data.binders.invisible")
  @js.native
  class invisible ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.readonly")
  @js.native
  class readonly ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  object select {
    
    @JSGlobal("kendo.data.binders.select.source")
    @js.native
    class source ()
      extends typingsSlinky.kendoUi.kendo.data.binders.source
    
    @JSGlobal("kendo.data.binders.select.value")
    @js.native
    class value ()
      extends typingsSlinky.kendoUi.kendo.data.binders.select.value
  }
  
  @JSGlobal("kendo.data.binders.source")
  @js.native
  class source ()
    extends typingsSlinky.kendoUi.kendo.data.binders.source
  
  @JSGlobal("kendo.data.binders.style")
  @js.native
  class style ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.text")
  @js.native
  class text ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  @JSGlobal("kendo.data.binders.value")
  @js.native
  class value ()
    extends typingsSlinky.kendoUi.kendo.data.binders.value
  
  @JSGlobal("kendo.data.binders.visible")
  @js.native
  class visible ()
    extends typingsSlinky.kendoUi.kendo.data.Binder
  
  object widget {
    
    @JSGlobal("kendo.data.binders.widget.checked")
    @js.native
    class checked ()
      extends typingsSlinky.kendoUi.kendo.data.binders.widget.checked
    
    @JSGlobal("kendo.data.binders.widget.disabled")
    @js.native
    class disabled ()
      extends typingsSlinky.kendoUi.kendo.data.Binder
    
    object dropdowntree {
      
      @JSGlobal("kendo.data.binders.widget.dropdowntree.value")
      @js.native
      class value ()
        extends typingsSlinky.kendoUi.kendo.data.binders.widget.dropdowntree.value
    }
    
    @JSGlobal("kendo.data.binders.widget.enabled")
    @js.native
    class enabled ()
      extends typingsSlinky.kendoUi.kendo.data.Binder
    
    @JSGlobal("kendo.data.binders.widget.end")
    @js.native
    class end ()
      extends typingsSlinky.kendoUi.kendo.data.binders.widget.end
    
    @JSGlobal("kendo.data.binders.widget.events")
    @js.native
    class events ()
      extends typingsSlinky.kendoUi.kendo.data.Binder
    
    object gantt {
      
      @JSGlobal("kendo.data.binders.widget.gantt.dependencies")
      @js.native
      def dependencies(bindingName: String, fieldName: String, setter: String): DataSourceBinder = js.native
    }
    
    object grid {
      
      @JSGlobal("kendo.data.binders.widget.grid.source")
      @js.native
      def source(bindingName: String, fieldName: String, setter: String): DataSourceBinder = js.native
    }
    
    @JSGlobal("kendo.data.binders.widget.invisible")
    @js.native
    class invisible ()
      extends typingsSlinky.kendoUi.kendo.data.Binder
    
    object multiselect {
      
      @JSGlobal("kendo.data.binders.widget.multiselect.value")
      @js.native
      class value ()
        extends typingsSlinky.kendoUi.kendo.data.binders.widget.multiselect.value
    }
    
    object scheduler {
      
      @JSGlobal("kendo.data.binders.widget.scheduler.source")
      @js.native
      def source(bindingName: String, fieldName: String, setter: String): DataSourceBinder = js.native
    }
    
    @JSGlobal("kendo.data.binders.widget.source")
    @js.native
    def source(bindingName: String, fieldName: String, setter: String): DataSourceBinder = js.native
    
    @JSGlobal("kendo.data.binders.widget.start")
    @js.native
    class start ()
      extends typingsSlinky.kendoUi.kendo.data.binders.widget.start
    
    @JSGlobal("kendo.data.binders.widget.value")
    @js.native
    class value ()
      extends typingsSlinky.kendoUi.kendo.data.binders.widget.value
    
    @JSGlobal("kendo.data.binders.widget.visible")
    @js.native
    class visible ()
      extends typingsSlinky.kendoUi.kendo.data.Binder
  }
}
