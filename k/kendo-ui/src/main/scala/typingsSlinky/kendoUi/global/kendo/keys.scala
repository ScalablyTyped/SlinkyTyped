package typingsSlinky.kendoUi.global.kendo

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object keys {
  
  @JSGlobal("kendo.keys")
  @js.native
  val ^ : js.Any = js.native
  
  @JSGlobal("kendo.keys.BACKSPACE")
  @js.native
  def BACKSPACE: Double = js.native
  @scala.inline
  def BACKSPACE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("BACKSPACE")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.DELETE")
  @js.native
  def DELETE: Double = js.native
  @scala.inline
  def DELETE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("DELETE")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.DOWN")
  @js.native
  def DOWN: Double = js.native
  @scala.inline
  def DOWN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("DOWN")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.END")
  @js.native
  def END: Double = js.native
  @scala.inline
  def END_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("END")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.ENTER")
  @js.native
  def ENTER: Double = js.native
  @scala.inline
  def ENTER_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ENTER")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.ESC")
  @js.native
  def ESC: Double = js.native
  @scala.inline
  def ESC_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ESC")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.F10")
  @js.native
  def F10: Double = js.native
  @scala.inline
  def F10_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F10")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.F12")
  @js.native
  def F12: Double = js.native
  @scala.inline
  def F12_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F12")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.F2")
  @js.native
  def F2: Double = js.native
  @scala.inline
  def F2_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F2")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.HOME")
  @js.native
  def HOME: Double = js.native
  @scala.inline
  def HOME_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("HOME")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.INSERT")
  @js.native
  def INSERT: Double = js.native
  @scala.inline
  def INSERT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("INSERT")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.LEFT")
  @js.native
  def LEFT: Double = js.native
  @scala.inline
  def LEFT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEFT")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.NUMPAD_DOT")
  @js.native
  def NUMPAD_DOT: Double = js.native
  @scala.inline
  def NUMPAD_DOT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUMPAD_DOT")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.NUMPAD_MINUS")
  @js.native
  def NUMPAD_MINUS: Double = js.native
  @scala.inline
  def NUMPAD_MINUS_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUMPAD_MINUS")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.NUMPAD_PLUS")
  @js.native
  def NUMPAD_PLUS: Double = js.native
  @scala.inline
  def NUMPAD_PLUS_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUMPAD_PLUS")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.PAGEDOWN")
  @js.native
  def PAGEDOWN: Double = js.native
  @scala.inline
  def PAGEDOWN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("PAGEDOWN")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.PAGEUP")
  @js.native
  def PAGEUP: Double = js.native
  @scala.inline
  def PAGEUP_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("PAGEUP")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.RIGHT")
  @js.native
  def RIGHT: Double = js.native
  @scala.inline
  def RIGHT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("RIGHT")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.SPACEBAR")
  @js.native
  def SPACEBAR: Double = js.native
  @scala.inline
  def SPACEBAR_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("SPACEBAR")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.TAB")
  @js.native
  def TAB: Double = js.native
  @scala.inline
  def TAB_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("TAB")(x.asInstanceOf[js.Any])
  
  @JSGlobal("kendo.keys.UP")
  @js.native
  def UP: Double = js.native
  @scala.inline
  def UP_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("UP")(x.asInstanceOf[js.Any])
}
