package typingsSlinky.zapierPlatformCore.anon

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait CustomStoreKey extends js.Object {
  var customStoreKey: js.UndefOr[String] = js.native
}

object CustomStoreKey {
  @scala.inline
  def apply(): CustomStoreKey = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[CustomStoreKey]
  }
  @scala.inline
  implicit class CustomStoreKeyOps[Self <: CustomStoreKey] (val x: Self) extends AnyVal {
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    @scala.inline
    def withCustomStoreKey(value: String): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("customStoreKey")(value.asInstanceOf[js.Any])
        ret
    }
    @scala.inline
    def withoutCustomStoreKey: Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("customStoreKey")(js.undefined)
        ret
    }
  }
  
}

