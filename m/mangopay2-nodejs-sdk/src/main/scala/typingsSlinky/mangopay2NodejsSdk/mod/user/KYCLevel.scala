package typingsSlinky.mangopay2NodejsSdk.mod.user

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typingsSlinky.mangopay2NodejsSdk.mangopay2NodejsSdkStrings.LIGHT
  - typingsSlinky.mangopay2NodejsSdk.mangopay2NodejsSdkStrings.REGULAR
*/
trait KYCLevel extends StObject
object KYCLevel {
  
  @scala.inline
  def LIGHT: typingsSlinky.mangopay2NodejsSdk.mangopay2NodejsSdkStrings.LIGHT = "LIGHT".asInstanceOf[typingsSlinky.mangopay2NodejsSdk.mangopay2NodejsSdkStrings.LIGHT]
  
  @scala.inline
  def REGULAR: typingsSlinky.mangopay2NodejsSdk.mangopay2NodejsSdkStrings.REGULAR = "REGULAR".asInstanceOf[typingsSlinky.mangopay2NodejsSdk.mangopay2NodejsSdkStrings.REGULAR]
}
