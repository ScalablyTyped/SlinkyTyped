package typingsSlinky.mysql2

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object mysql2Strings {
  
  @scala.inline
  def DATE: DATE = "DATE".asInstanceOf[DATE]
  
  @scala.inline
  def DATETIME: DATETIME = "DATETIME".asInstanceOf[DATETIME]
  
  @scala.inline
  def Field: Field = "Field".asInstanceOf[Field]
  
  @scala.inline
  def FieldPacket: FieldPacket = "FieldPacket".asInstanceOf[FieldPacket]
  
  @scala.inline
  def OkPacket: OkPacket = "OkPacket".asInstanceOf[OkPacket]
  
  @scala.inline
  def ResultSetHeader: ResultSetHeader = "ResultSetHeader".asInstanceOf[ResultSetHeader]
  
  @scala.inline
  def RowDataPacket: RowDataPacket = "RowDataPacket".asInstanceOf[RowDataPacket]
  
  @scala.inline
  def TIMESTAMP: TIMESTAMP = "TIMESTAMP".asInstanceOf[TIMESTAMP]
  
  @scala.inline
  def acquire: acquire = "acquire".asInstanceOf[acquire]
  
  @scala.inline
  def connection: connection = "connection".asInstanceOf[connection]
  
  @scala.inline
  def end: end = "end".asInstanceOf[end]
  
  @scala.inline
  def enqueue: enqueue = "enqueue".asInstanceOf[enqueue]
  
  @scala.inline
  def error: error = "error".asInstanceOf[error]
  
  @scala.inline
  def fields: fields = "fields".asInstanceOf[fields]
  
  @scala.inline
  def local: local = "local".asInstanceOf[local]
  
  @scala.inline
  def release: release = "release".asInstanceOf[release]
  
  @scala.inline
  def remove: remove = "remove".asInstanceOf[remove]
  
  @scala.inline
  def result: result = "result".asInstanceOf[result]
  
  @js.native
  sealed trait DATE extends js.Object
  
  @js.native
  sealed trait DATETIME extends js.Object
  
  @js.native
  sealed trait Field extends js.Object
  
  @js.native
  sealed trait FieldPacket extends js.Object
  
  @js.native
  sealed trait OkPacket extends js.Object
  
  @js.native
  sealed trait ResultSetHeader extends js.Object
  
  @js.native
  sealed trait RowDataPacket extends js.Object
  
  @js.native
  sealed trait TIMESTAMP extends js.Object
  
  @js.native
  sealed trait acquire extends js.Object
  
  @js.native
  sealed trait connection extends js.Object
  
  @js.native
  sealed trait end extends js.Object
  
  @js.native
  sealed trait enqueue extends js.Object
  
  @js.native
  sealed trait error extends js.Object
  
  @js.native
  sealed trait fields extends js.Object
  
  @js.native
  sealed trait local extends js.Object
  
  @js.native
  sealed trait release extends js.Object
  
  @js.native
  sealed trait remove extends js.Object
  
  @js.native
  sealed trait result extends js.Object
}
