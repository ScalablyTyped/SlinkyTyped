package typingsSlinky.mjmlReact.mod

import typingsSlinky.react.mod.Component
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("mjml-react", "MjmlCarousel")
@js.native
class MjmlCarousel protected ()
  extends Component[RequiredChildrenProps with MjmlCarouselProps with ClassNameProps, js.Object, js.Any] {
  def this(props: RequiredChildrenProps with MjmlCarouselProps with ClassNameProps) = this()
  /**
    * @deprecated
    * @see https://reactjs.org/docs/legacy-context.html
    */
  def this(props: RequiredChildrenProps with MjmlCarouselProps with ClassNameProps, context: js.Any) = this()
}
