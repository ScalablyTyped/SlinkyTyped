package typingsSlinky.atMaterialDashUiLab

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Inlined std.Partial<@material-ui/core.@material-ui/core/styles/withStyles.ClassNameMap<@material-ui/lab.@material-ui/lab/Autocomplete/Autocomplete.AutocompleteClassKey>> */
trait PartialClassNameMapAutocompleteClassKey extends js.Object {
  var clearIndicator: js.UndefOr[String] = js.undefined
  var clearIndicatorDirty: js.UndefOr[String] = js.undefined
  var focused: js.UndefOr[String] = js.undefined
  var groupLabel: js.UndefOr[String] = js.undefined
  var groupUl: js.UndefOr[String] = js.undefined
  var input: js.UndefOr[String] = js.undefined
  var inputFocused: js.UndefOr[String] = js.undefined
  var inputRoot: js.UndefOr[String] = js.undefined
  var listbox: js.UndefOr[String] = js.undefined
  var loading: js.UndefOr[String] = js.undefined
  var noOptions: js.UndefOr[String] = js.undefined
  var option: js.UndefOr[String] = js.undefined
  var paper: js.UndefOr[String] = js.undefined
  var popper: js.UndefOr[String] = js.undefined
  var popperDisablePortal: js.UndefOr[String] = js.undefined
  var popupIndicator: js.UndefOr[String] = js.undefined
  var popupIndicatorOpen: js.UndefOr[String] = js.undefined
  var root: js.UndefOr[String] = js.undefined
  var tag: js.UndefOr[String] = js.undefined
}

object PartialClassNameMapAutocompleteClassKey {
  @scala.inline
  def apply(
    clearIndicator: String = null,
    clearIndicatorDirty: String = null,
    focused: String = null,
    groupLabel: String = null,
    groupUl: String = null,
    input: String = null,
    inputFocused: String = null,
    inputRoot: String = null,
    listbox: String = null,
    loading: String = null,
    noOptions: String = null,
    option: String = null,
    paper: String = null,
    popper: String = null,
    popperDisablePortal: String = null,
    popupIndicator: String = null,
    popupIndicatorOpen: String = null,
    root: String = null,
    tag: String = null
  ): PartialClassNameMapAutocompleteClassKey = {
    val __obj = js.Dynamic.literal()
    if (clearIndicator != null) __obj.updateDynamic("clearIndicator")(clearIndicator.asInstanceOf[js.Any])
    if (clearIndicatorDirty != null) __obj.updateDynamic("clearIndicatorDirty")(clearIndicatorDirty.asInstanceOf[js.Any])
    if (focused != null) __obj.updateDynamic("focused")(focused.asInstanceOf[js.Any])
    if (groupLabel != null) __obj.updateDynamic("groupLabel")(groupLabel.asInstanceOf[js.Any])
    if (groupUl != null) __obj.updateDynamic("groupUl")(groupUl.asInstanceOf[js.Any])
    if (input != null) __obj.updateDynamic("input")(input.asInstanceOf[js.Any])
    if (inputFocused != null) __obj.updateDynamic("inputFocused")(inputFocused.asInstanceOf[js.Any])
    if (inputRoot != null) __obj.updateDynamic("inputRoot")(inputRoot.asInstanceOf[js.Any])
    if (listbox != null) __obj.updateDynamic("listbox")(listbox.asInstanceOf[js.Any])
    if (loading != null) __obj.updateDynamic("loading")(loading.asInstanceOf[js.Any])
    if (noOptions != null) __obj.updateDynamic("noOptions")(noOptions.asInstanceOf[js.Any])
    if (option != null) __obj.updateDynamic("option")(option.asInstanceOf[js.Any])
    if (paper != null) __obj.updateDynamic("paper")(paper.asInstanceOf[js.Any])
    if (popper != null) __obj.updateDynamic("popper")(popper.asInstanceOf[js.Any])
    if (popperDisablePortal != null) __obj.updateDynamic("popperDisablePortal")(popperDisablePortal.asInstanceOf[js.Any])
    if (popupIndicator != null) __obj.updateDynamic("popupIndicator")(popupIndicator.asInstanceOf[js.Any])
    if (popupIndicatorOpen != null) __obj.updateDynamic("popupIndicatorOpen")(popupIndicatorOpen.asInstanceOf[js.Any])
    if (root != null) __obj.updateDynamic("root")(root.asInstanceOf[js.Any])
    if (tag != null) __obj.updateDynamic("tag")(tag.asInstanceOf[js.Any])
    __obj.asInstanceOf[PartialClassNameMapAutocompleteClassKey]
  }
}

