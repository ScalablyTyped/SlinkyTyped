...
object UpdateQuery {
  ...
  implicit class UpdateQueryOps[Self <: UpdateQuery[_], TSchema] (val x: Self with UpdateQuery[TSchema]) extends AnyVal {
    ...
    @scala.inline
    def set$bit(value: StringDictionary[typingsSlinky.mongodb.mongodbStrings.UpdateQuery with TopLevel[js.Any]]): Self = this.set("$bit", value.asInstanceOf[js.Any])
    @scala.inline
    def delete$bit: Self = this.set("$bit", js.undefined)
    @scala.inline
    def set$currentDate(value: OnlyFieldsOfType[TSchema, js.Date, `true` | Type]): Self = this.set("$currentDate", value.asInstanceOf[js.Any])
    @scala.inline
