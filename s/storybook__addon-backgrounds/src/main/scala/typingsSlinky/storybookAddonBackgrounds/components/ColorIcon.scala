...
object ColorIcon {
  ...
  def withProps(
    p: (DetailedHTMLProps[HTMLAttributes[HTMLSpanElement], HTMLSpanElement]) with Background with ThemeTheme[Theme]
  ): Builder = new Builder(js.Array(this.component, p.asInstanceOf[js.Any]))
  @scala.inline
  def apply(background: String): Builder = {
    val __props = js.Dynamic.literal(background = background.asInstanceOf[js.Any])
    new Builder(js.Array(this.component, __props.asInstanceOf[(DetailedHTMLProps[HTMLAttributes[HTMLSpanElement], HTMLSpanElement]) with Background with ThemeTheme[Theme]]))
  }
}
