...
import typingsSlinky.sharp.Anon_Brightness
import typingsSlinky.sharp.Anon_Data
import typingsSlinky.sharp.Anon_False
import typingsSlinky.sharp.Anon_Raw
import typingsSlinky.sharp.Anon_ResolveWithObject
...
trait Sharp extends Duplex {
  ...
  def modulate(options: Anon_Brightness): Sharp = js.native
  /**
  ...
  def toBuffer(options: Anon_False): js.Promise[Buffer] = js.native
  /**
    * Write output to a Buffer. JPEG, PNG, WebP, TIFF and RAW output are supported.
    * By default, the format will match the input image, except GIF and SVG input which become PNG output.
    * @param options resolve options
    ...
    */
  def toBuffer(options: Anon_ResolveWithObject): js.Promise[Anon_Data] = js.native
  /**
