...
import typingsSlinky.storybookComponents.buttonMod.IconButtonProps
...
object IconButton {
  @JSImport("@storybook/components", "IconButton")
  @js.native
  ...
  def withProps(
    p: PropsWithChildren[
      (DetailedHTMLProps[ButtonHTMLAttributes[HTMLButtonElement], HTMLButtonElement]) with (Omit[IconButtonProps, theme]) with `0`[Theme]
    ]
