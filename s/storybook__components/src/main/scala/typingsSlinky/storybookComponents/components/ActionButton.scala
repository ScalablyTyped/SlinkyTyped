...
object ActionButton {
  ...
  @scala.inline
  def apply(disabled: Boolean): Builder = {
    val __props = js.Dynamic.literal(disabled = disabled.asInstanceOf[js.Any])
    new Builder(js.Array(this.component, __props.asInstanceOf[(DetailedHTMLProps[ButtonHTMLAttributes[HTMLButtonElement], HTMLButtonElement]) with (Omit[Disabled, theme]) with `0`[Theme]]))
  }
  ...
  def withProps(
    p: (DetailedHTMLProps[ButtonHTMLAttributes[HTMLButtonElement], HTMLButtonElement]) with (Omit[Disabled, theme]) with `0`[Theme]
