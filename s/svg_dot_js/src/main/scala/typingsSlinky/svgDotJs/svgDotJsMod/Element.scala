...
import typingsSlinky.svgDotJs.Anon_Color
import typingsSlinky.svgDotJs.Anon_Delay
...
trait Element
  ...
  def event(): Event | CustomEvent = js.native
  ...
  def fire(event: typingsSlinky.std.Event): this.type = js.native
