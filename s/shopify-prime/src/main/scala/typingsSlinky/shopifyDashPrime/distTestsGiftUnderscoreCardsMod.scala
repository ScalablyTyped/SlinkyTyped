package typingsSlinky.shopifyDashPrime

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("shopify-prime/dist/tests/gift_cards", JSImport.Namespace)
@js.native
object distTestsGiftUnderscoreCardsMod extends js.Object {
  @js.native
  class GiftCardTests () extends js.Object {
    def Test1(): js.Promise[Unit] = js.native
    def Test2(): js.Promise[Unit] = js.native
    def Test3(): js.Promise[Unit] = js.native
    def Test4(): js.Promise[Unit] = js.native
    def Test5(): js.Promise[Unit] = js.native
    def Test6(): js.Promise[Unit] = js.native
  }
  
}

