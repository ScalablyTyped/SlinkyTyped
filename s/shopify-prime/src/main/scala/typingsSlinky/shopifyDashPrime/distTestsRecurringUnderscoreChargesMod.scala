package typingsSlinky.shopifyDashPrime

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("shopify-prime/dist/tests/recurring_charges", JSImport.Namespace)
@js.native
object distTestsRecurringUnderscoreChargesMod extends js.Object {
  @js.native
  class RecurringChargeTests () extends js.Object {
    def Test1(): js.Promise[Unit] = js.native
    def Test2(): js.Promise[Unit] = js.native
    def Test3(): js.Promise[Unit] = js.native
    def Test4(): js.Promise[Unit] = js.native
  }
  
}

