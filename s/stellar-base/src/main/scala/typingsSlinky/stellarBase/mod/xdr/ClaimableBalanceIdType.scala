package typingsSlinky.stellarBase.mod.xdr

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("stellar-base", "xdr.ClaimableBalanceIdType")
@js.native
class ClaimableBalanceIdType ()
  extends typingsSlinky.stellarBase.xdrMod.default.ClaimableBalanceIdType
object ClaimableBalanceIdType {
  
  /* static member */
  @JSImport("stellar-base", "xdr.ClaimableBalanceIdType.claimableBalanceIdTypeV0")
  @js.native
  def claimableBalanceIdTypeV0(): typingsSlinky.stellarBase.xdrMod.xdr.ClaimableBalanceIdType = js.native
}
