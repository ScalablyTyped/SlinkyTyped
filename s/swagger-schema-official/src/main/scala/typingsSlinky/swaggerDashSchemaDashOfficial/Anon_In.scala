package typingsSlinky.swaggerDashSchemaDashOfficial

import typingsSlinky.swaggerDashSchemaDashOfficial.swaggerDashSchemaDashOfficialBooleans.`true`
import typingsSlinky.swaggerDashSchemaDashOfficial.swaggerDashSchemaDashOfficialStrings.path
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_In extends js.Object {
  var in: path
  var required: `true`
}

object Anon_In {
  @scala.inline
  def apply(in: path, required: `true`): Anon_In = {
    val __obj = js.Dynamic.literal(in = in.asInstanceOf[js.Any], required = required.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon_In]
  }
}

