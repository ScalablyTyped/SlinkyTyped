package typingsSlinky.streamingIterables.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("streaming-iterables", "pipeline")
@js.native
object pipeline extends js.Object {
  
  def apply[T0](firstFn: js.Function0[T0]): T0 = js.native
  def apply[T0, T1](a0: js.Function0[T0], a1: js.Function1[/* a */ T0, T1]): T1 = js.native
  def apply[T0, T1, T2](a0: js.Function0[T0], a1: js.Function1[/* a */ T0, T1], a2: js.Function1[/* a */ T1, T2]): T2 = js.native
  def apply[T0, T1, T2, T3](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3]
  ): T3 = js.native
  def apply[T0, T1, T2, T3, T4](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3],
    a4: js.Function1[/* a */ T3, T4]
  ): T4 = js.native
  def apply[T0, T1, T2, T3, T4, T5](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3],
    a4: js.Function1[/* a */ T3, T4],
    a5: js.Function1[/* a */ T4, T5]
  ): T5 = js.native
  def apply[T0, T1, T2, T3, T4, T5, T6](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3],
    a4: js.Function1[/* a */ T3, T4],
    a5: js.Function1[/* a */ T4, T5],
    a6: js.Function1[/* a */ T5, T6]
  ): T6 = js.native
  def apply[T0, T1, T2, T3, T4, T5, T6, T7](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3],
    a4: js.Function1[/* a */ T3, T4],
    a5: js.Function1[/* a */ T4, T5],
    a6: js.Function1[/* a */ T5, T6],
    a7: js.Function1[/* a */ T6, T7]
  ): T7 = js.native
  def apply[T0, T1, T2, T3, T4, T5, T6, T7, T8](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3],
    a4: js.Function1[/* a */ T3, T4],
    a5: js.Function1[/* a */ T4, T5],
    a6: js.Function1[/* a */ T5, T6],
    a7: js.Function1[/* a */ T6, T7],
    a8: js.Function1[/* a */ T7, T8]
  ): T8 = js.native
  def apply[T0, T1, T2, T3, T4, T5, T6, T7, T8, T9](
    a0: js.Function0[T0],
    a1: js.Function1[/* a */ T0, T1],
    a2: js.Function1[/* a */ T1, T2],
    a3: js.Function1[/* a */ T2, T3],
    a4: js.Function1[/* a */ T3, T4],
    a5: js.Function1[/* a */ T4, T5],
    a6: js.Function1[/* a */ T5, T6],
    a7: js.Function1[/* a */ T6, T7],
    a8: js.Function1[/* a */ T7, T8],
    a9: js.Function1[/* a */ T8, T9]
  ): T9 = js.native
}
