...
object CollapseIcon {
  ...
  @scala.inline
  def apply(isExpanded: Boolean): Builder = {
    val __props = js.Dynamic.literal(isExpanded = isExpanded.asInstanceOf[js.Any])
    new Builder(js.Array(this.component, __props.asInstanceOf[(DetailedHTMLProps[HTMLAttributes[HTMLSpanElement], HTMLSpanElement]) with (Omit[IsExpanded, theme]) with `0`[Theme]]))
  }
  ...
  def withProps(
    p: (DetailedHTMLProps[HTMLAttributes[HTMLSpanElement], HTMLSpanElement]) with (Omit[IsExpanded, theme]) with `0`[Theme]
