...
import typingsSlinky.storybookUi.anon.IsFullscreenIsToolshown
...
object typesMod {
  ...
  trait Wrapper extends StObject {
    ...
    var render: ReactComponentClass[WrapperProps] = js.native
  }
  object Wrapper {
    
    @scala.inline
    def apply(render: ReactComponentClass[WrapperProps]): Wrapper = {
    ...
    implicit class WrapperMutableBuilder[Self <: Wrapper] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setRender(value: ReactComponentClass[WrapperProps]): Self = StObject.set(x, "render", value.asInstanceOf[js.Any])
    }
  }
