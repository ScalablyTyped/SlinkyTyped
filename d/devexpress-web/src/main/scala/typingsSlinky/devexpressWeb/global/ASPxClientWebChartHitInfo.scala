package typingsSlinky.devexpressWeb.global

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/**
  * Contains information about a specific test point within a chart control, on the client side.
  */
@JSGlobal("ASPxClientWebChartHitInfo")
@js.native
class ASPxClientWebChartHitInfo ()
  extends typingsSlinky.devexpressWeb.ASPxClientWebChartHitInfo {
  /**
    * Gets the client-side annotation instance from under the test point.
    */
  /* CompleteClass */
  override var annotation: typingsSlinky.devexpressWeb.ASPxClientAnnotation = js.native
  /**
    * Gets the client-side axis instance from under the test point.
    */
  /* CompleteClass */
  override var axis: typingsSlinky.devexpressWeb.ASPxClientAxisBase = js.native
  /**
    * Gets the client-side axis label item instance from under the test point.
    */
  /* CompleteClass */
  override var axisLabelItem: typingsSlinky.devexpressWeb.ASPxClientAxisLabelItem = js.native
  /**
    * Gets the client-side axis title instance from under the test point.
    */
  /* CompleteClass */
  override var axisTitle: typingsSlinky.devexpressWeb.ASPxClientAxisTitle = js.native
  /**
    * Gets the client-side chart instance from under the test point.
    */
  /* CompleteClass */
  override var chart: typingsSlinky.devexpressWeb.ASPxClientWebChart = js.native
  /**
    * Gets the client-side chart title instance from under the test point.
    */
  /* CompleteClass */
  override var chartTitle: typingsSlinky.devexpressWeb.ASPxClientChartTitle = js.native
  /**
    * Gets the client-side constant line instance from under the test point.
    */
  /* CompleteClass */
  override var constantLine: typingsSlinky.devexpressWeb.ASPxClientConstantLine = js.native
  /**
    * Gets a custom legend item which is located under the test point.
    */
  /* CompleteClass */
  override var customLegendItem: typingsSlinky.devexpressWeb.ASPxClientCustomLegendItem = js.native
  /**
    * Gets the client-side diagram instance from under the test point.
    */
  /* CompleteClass */
  override var diagram: typingsSlinky.devexpressWeb.ASPxClientXYDiagramBase = js.native
  /** @deprecated This property is now obsolete. Use the indicator property instead. */
  /**
    * Gets the client-side Fibonacci indicator instance from under the test point.
    */
  /* CompleteClass */
  override var fibonacciIndicator: typingsSlinky.devexpressWeb.ASPxClientFibonacciIndicator = js.native
  /**
    * Returns a hyperlink which is located under the test point.
    */
  /* CompleteClass */
  override var hyperlink: String = js.native
  /**
    * Gets a value indicating whether the test point is within an annotation.
    */
  /* CompleteClass */
  override var inAnnotation: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the axis.
    */
  /* CompleteClass */
  override var inAxis: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the axis label item.
    */
  /* CompleteClass */
  override var inAxisLabelItem: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the axis title.
    */
  /* CompleteClass */
  override var inAxisTitle: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the chart.
    */
  /* CompleteClass */
  override var inChart: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the chart title.
    */
  /* CompleteClass */
  override var inChartTitle: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the constant line.
    */
  /* CompleteClass */
  override var inConstantLine: Boolean = js.native
  /**
    * Gets the value indicating whether or not the test point is within a custom legend item.
    */
  /* CompleteClass */
  override var inCustomLegendItem: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the diagram.
    */
  /* CompleteClass */
  override var inDiagram: Boolean = js.native
  /** @deprecated This property is now obsolete. Use the inIndicator property instead. */
  /**
    * Gets a value indicating whether the test point is within the Fibonacci Indicator.
    */
  /* CompleteClass */
  override var inFibonacciIndicator: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within a hyperlink.
    */
  /* CompleteClass */
  override var inHyperlink: Boolean = js.native
  /**
    * Gets a value specifying whether the test point is within an indicator.
    */
  /* CompleteClass */
  override var inIndicator: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the legend.
    */
  /* CompleteClass */
  override var inLegend: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the non-default pane.
    */
  /* CompleteClass */
  override var inNonDefaultPane: Boolean = js.native
  /**
    * Returns the value that indicates whether the test point is within the pane title.
    */
  /* CompleteClass */
  override var inPaneTitle: Boolean = js.native
  /** @deprecated This property is now obsolete. Use the inIndicator property instead. */
  /**
    * Gets a value indicating whether the test point is within the regression line.
    */
  /* CompleteClass */
  override var inRegressionLine: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the series.
    */
  /* CompleteClass */
  override var inSeries: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the series label.
    */
  /* CompleteClass */
  override var inSeriesLabel: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the series point.
    */
  /* CompleteClass */
  override var inSeriesPoint: Boolean = js.native
  /**
    * Gets a value indicating whether the test point is within the series title.
    */
  /* CompleteClass */
  override var inSeriesTitle: Boolean = js.native
  /** @deprecated This property is now obsolete. Use the inIndicator property instead. */
  /**
    * Gets a value indicating whether the test point is within the trendline.
    */
  /* CompleteClass */
  override var inTrendLine: Boolean = js.native
  /**
    * Gets the client-side indicator instance from under the test point.
    */
  /* CompleteClass */
  override var indicator: typingsSlinky.devexpressWeb.ASPxClientIndicator = js.native
  /**
    * Gets the client-side legend instance from under the test point.
    */
  /* CompleteClass */
  override var legend: typingsSlinky.devexpressWeb.ASPxClientLegend = js.native
  /**
    * Gets the client-side non-default pane instance from under the test point.
    */
  /* CompleteClass */
  override var nonDefaultPane: typingsSlinky.devexpressWeb.ASPxClientXYDiagramPane = js.native
  /**
    * Gets the client-side pane title instance from under the test point.
    */
  /* CompleteClass */
  override var paneTitle: typingsSlinky.devexpressWeb.ASPxClientPaneTitle = js.native
  /** @deprecated This property is now obsolete. Use the indicator property instead. */
  /**
    * Gets the client-side regression line instance from under the test point.
    */
  /* CompleteClass */
  override var regressionLine: typingsSlinky.devexpressWeb.ASPxClientRegressionLine = js.native
  /**
    * Gets the client-side series instance from under the test point.
    */
  /* CompleteClass */
  override var series: typingsSlinky.devexpressWeb.ASPxClientSeries = js.native
  /**
    * Gets the client-side series label instance from under the test point.
    */
  /* CompleteClass */
  override var seriesLabel: typingsSlinky.devexpressWeb.ASPxClientSeriesLabel = js.native
  /**
    * Gets the client-side series point instance from under the test point.
    */
  /* CompleteClass */
  override var seriesPoint: typingsSlinky.devexpressWeb.ASPxClientSeriesPoint = js.native
  /**
    * Gets the client-side series title instance from under the test point.
    */
  /* CompleteClass */
  override var seriesTitle: typingsSlinky.devexpressWeb.ASPxClientSeriesTitle = js.native
  /** @deprecated This property is now obsolete. Use the indicator property instead. */
  /**
    * Gets the client-side trendline instance from under the test point.
    */
  /* CompleteClass */
  override var trendLine: typingsSlinky.devexpressWeb.ASPxClientTrendLine = js.native
}

