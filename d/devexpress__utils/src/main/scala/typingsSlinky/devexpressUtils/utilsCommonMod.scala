package typingsSlinky.devexpressUtils

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object utilsCommonMod {
  
  @JSImport("@devexpress/utils/lib/utils/common", "boolToInt")
  @js.native
  def boolToInt(value: Boolean): Double = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "boolToString")
  @js.native
  def boolToString(value: Boolean): String = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "isDefined")
  @js.native
  def isDefined[T](): /* is T */ Boolean = js.native
  @JSImport("@devexpress/utils/lib/utils/common", "isDefined")
  @js.native
  def isDefined[T](value: T): /* is T */ Boolean = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "isEven")
  @js.native
  def isEven(num: Double): Boolean = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "isNonNullString")
  @js.native
  def isNonNullString(): /* is string */ Boolean = js.native
  @JSImport("@devexpress/utils/lib/utils/common", "isNonNullString")
  @js.native
  def isNonNullString(str: String): /* is string */ Boolean = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "isNumber")
  @js.native
  def isNumber(obj: js.Any): /* is number */ Boolean = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "isOdd")
  @js.native
  def isOdd(num: Double): Boolean = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "isString")
  @js.native
  def isString(obj: js.Any): /* is string */ Boolean = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "numberToStringBin")
  @js.native
  def numberToStringBin(num: Double): String = js.native
  @JSImport("@devexpress/utils/lib/utils/common", "numberToStringBin")
  @js.native
  def numberToStringBin(num: Double, minLength: Double): String = js.native
  
  @JSImport("@devexpress/utils/lib/utils/common", "numberToStringHex")
  @js.native
  def numberToStringHex(num: Double): String = js.native
  @JSImport("@devexpress/utils/lib/utils/common", "numberToStringHex")
  @js.native
  def numberToStringHex(num: Double, minLength: Double): String = js.native
}
