package typingsSlinky.devextreme

import typingsSlinky.devextreme.mod.DevExpress.ui._format
import typingsSlinky.devextreme.mod.DevExpress.viz._VizTimeInterval
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object devextremeStrings {
  
  @js.native
  sealed trait default extends StObject
  @scala.inline
  def default: default = "default".asInstanceOf[default]
  
  @js.native
  sealed trait A extends StObject
  @scala.inline
  def A: A = "A".asInstanceOf[A]
  
  @js.native
  sealed trait B extends StObject
  @scala.inline
  def B: B = "B".asInstanceOf[B]
  
  @js.native
  sealed trait Boolean extends StObject
  @scala.inline
  def Boolean: Boolean = "Boolean".asInstanceOf[Boolean]
  
  @js.native
  sealed trait Bright extends StObject
  @scala.inline
  def Bright: Bright = "Bright".asInstanceOf[Bright]
  
  @js.native
  sealed trait C extends StObject
  @scala.inline
  def C: C = "C".asInstanceOf[C]
  
  @js.native
  sealed trait Carmine extends StObject
  @scala.inline
  def Carmine: Carmine = "Carmine".asInstanceOf[Carmine]
  
  @js.native
  sealed trait D extends StObject
  @scala.inline
  def D: D = "D".asInstanceOf[D]
  
  @js.native
  sealed trait `Dark Moon` extends StObject
  @scala.inline
  def `Dark Moon`: `Dark Moon` = ("Dark Moon").asInstanceOf[`Dark Moon`]
  
  @js.native
  sealed trait `Dark Violet` extends StObject
  @scala.inline
  def `Dark Violet`: `Dark Violet` = ("Dark Violet").asInstanceOf[`Dark Violet`]
  
  @js.native
  sealed trait Decimal extends StObject
  @scala.inline
  def Decimal: Decimal = "Decimal".asInstanceOf[Decimal]
  
  @js.native
  sealed trait Equalssign extends StObject
  @scala.inline
  def Equalssign: Equalssign = "=".asInstanceOf[Equalssign]
  
  @js.native
  sealed trait EqualssignEqualssign extends StObject
  @scala.inline
  def EqualssignEqualssign: EqualssignEqualssign = "==".asInstanceOf[EqualssignEqualssign]
  
  @js.native
  sealed trait EqualssignEqualssignEqualssign extends StObject
  @scala.inline
  def EqualssignEqualssignEqualssign: EqualssignEqualssignEqualssign = "===".asInstanceOf[EqualssignEqualssignEqualssign]
  
  @js.native
  sealed trait ExclamationmarkEqualssign extends StObject
  @scala.inline
  def ExclamationmarkEqualssign: ExclamationmarkEqualssign = "!=".asInstanceOf[ExclamationmarkEqualssign]
  
  @js.native
  sealed trait ExclamationmarkEqualssignEqualssign extends StObject
  @scala.inline
  def ExclamationmarkEqualssignEqualssign: ExclamationmarkEqualssignEqualssign = "!==".asInstanceOf[ExclamationmarkEqualssignEqualssign]
  
  @js.native
  sealed trait GIF extends StObject
  @scala.inline
  def GIF: GIF = "GIF".asInstanceOf[GIF]
  
  @js.native
  sealed trait GT extends StObject
  @scala.inline
  def GT: GT = "GT".asInstanceOf[GT]
  
  @js.native
  sealed trait Greaterthansign extends StObject
  @scala.inline
  def Greaterthansign: Greaterthansign = ">".asInstanceOf[Greaterthansign]
  
  @js.native
  sealed trait GreaterthansignEqualssign extends StObject
  @scala.inline
  def GreaterthansignEqualssign: GreaterthansignEqualssign = ">=".asInstanceOf[GreaterthansignEqualssign]
  
  @js.native
  sealed trait `Green Mist` extends StObject
  @scala.inline
  def `Green Mist`: `Green Mist` = ("Green Mist").asInstanceOf[`Green Mist`]
  
  @js.native
  sealed trait Guid extends StObject
  @scala.inline
  def Guid: Guid = "Guid".asInstanceOf[Guid]
  
  @js.native
  sealed trait `Harmony Light` extends StObject
  @scala.inline
  def `Harmony Light`: `Harmony Light` = ("Harmony Light").asInstanceOf[`Harmony Light`]
  
  @js.native
  sealed trait Int32 extends StObject
  @scala.inline
  def Int32: Int32 = "Int32".asInstanceOf[Int32]
  
  @js.native
  sealed trait Int64 extends StObject
  @scala.inline
  def Int64: Int64 = "Int64".asInstanceOf[Int64]
  
  @js.native
  sealed trait JPEG extends StObject
  @scala.inline
  def JPEG: JPEG = "JPEG".asInstanceOf[JPEG]
  
  @js.native
  sealed trait Lessthansign extends StObject
  @scala.inline
  def Lessthansign: Lessthansign = "<".asInstanceOf[Lessthansign]
  
  @js.native
  sealed trait LessthansignEqualssign extends StObject
  @scala.inline
  def LessthansignEqualssign: LessthansignEqualssign = "<=".asInstanceOf[LessthansignEqualssign]
  
  @js.native
  sealed trait LessthansignGreaterthansign extends StObject
  @scala.inline
  def LessthansignGreaterthansign: LessthansignGreaterthansign = "<>".asInstanceOf[LessthansignGreaterthansign]
  
  @js.native
  sealed trait Material extends StObject
  @scala.inline
  def Material: Material = "Material".asInstanceOf[Material]
  
  @js.native
  sealed trait Ocean extends StObject
  @scala.inline
  def Ocean: Ocean = "Ocean".asInstanceOf[Ocean]
  
  @js.native
  sealed trait Office extends StObject
  @scala.inline
  def Office: Office = "Office".asInstanceOf[Office]
  
  @js.native
  sealed trait PDF extends StObject
  @scala.inline
  def PDF: PDF = "PDF".asInstanceOf[PDF]
  
  @js.native
  sealed trait PNG extends StObject
  @scala.inline
  def PNG: PNG = "PNG".asInstanceOf[PNG]
  
  @js.native
  sealed trait POST extends StObject
  @scala.inline
  def POST: POST = "POST".asInstanceOf[POST]
  
  @js.native
  sealed trait PUT extends StObject
  @scala.inline
  def PUT: PUT = "PUT".asInstanceOf[PUT]
  
  @js.native
  sealed trait Pastel extends StObject
  @scala.inline
  def Pastel: Pastel = "Pastel".asInstanceOf[Pastel]
  
  @js.native
  sealed trait SVG extends StObject
  @scala.inline
  def SVG: SVG = "SVG".asInstanceOf[SVG]
  
  @js.native
  sealed trait Single extends StObject
  @scala.inline
  def Single: Single = "Single".asInstanceOf[Single]
  
  @js.native
  sealed trait Soft extends StObject
  @scala.inline
  def Soft: Soft = "Soft".asInstanceOf[Soft]
  
  @js.native
  sealed trait `Soft Blue` extends StObject
  @scala.inline
  def `Soft Blue`: `Soft Blue` = ("Soft Blue").asInstanceOf[`Soft Blue`]
  
  @js.native
  sealed trait `Soft Pastel` extends StObject
  @scala.inline
  def `Soft Pastel`: `Soft Pastel` = ("Soft Pastel").asInstanceOf[`Soft Pastel`]
  
  @js.native
  sealed trait String extends StObject
  @scala.inline
  def String: String = "String".asInstanceOf[String]
  
  @js.native
  sealed trait T extends StObject
  @scala.inline
  def T: T = "T".asInstanceOf[T]
  
  @js.native
  sealed trait Vintage extends StObject
  @scala.inline
  def Vintage: Vintage = "Vintage".asInstanceOf[Vintage]
  
  @js.native
  sealed trait Violet extends StObject
  @scala.inline
  def Violet: Violet = "Violet".asInstanceOf[Violet]
  
  @js.native
  sealed trait absoluteVariation extends StObject
  @scala.inline
  def absoluteVariation: absoluteVariation = "absoluteVariation".asInstanceOf[absoluteVariation]
  
  @js.native
  sealed trait adaptive extends StObject
  @scala.inline
  def adaptive: adaptive = "adaptive".asInstanceOf[adaptive]
  
  @js.native
  sealed trait add extends StObject
  @scala.inline
  def add: add = "add".asInstanceOf[add]
  
  @js.native
  sealed trait addShape extends StObject
  @scala.inline
  def addShape: addShape = "addShape".asInstanceOf[addShape]
  
  @js.native
  sealed trait addShapeFromToolbox extends StObject
  @scala.inline
  def addShapeFromToolbox: addShapeFromToolbox = "addShapeFromToolbox".asInstanceOf[addShapeFromToolbox]
  
  @js.native
  sealed trait addTask extends StObject
  @scala.inline
  def addTask: addTask = "addTask".asInstanceOf[addTask]
  
  @js.native
  sealed trait after extends StObject
  @scala.inline
  def after: after = "after".asInstanceOf[after]
  
  @js.native
  sealed trait agenda extends StObject
  @scala.inline
  def agenda: agenda = "agenda".asInstanceOf[agenda]
  
  @js.native
  sealed trait align extends StObject
  @scala.inline
  def align: align = "align".asInstanceOf[align]
  
  @js.native
  sealed trait alignCenter extends StObject
  @scala.inline
  def alignCenter: alignCenter = "alignCenter".asInstanceOf[alignCenter]
  
  @js.native
  sealed trait alignJustify extends StObject
  @scala.inline
  def alignJustify: alignJustify = "alignJustify".asInstanceOf[alignJustify]
  
  @js.native
  sealed trait alignLeft extends StObject
  @scala.inline
  def alignLeft: alignLeft = "alignLeft".asInstanceOf[alignLeft]
  
  @js.native
  sealed trait alignRight extends StObject
  @scala.inline
  def alignRight: alignRight = "alignRight".asInstanceOf[alignRight]
  
  @js.native
  sealed trait all extends StObject
  @scala.inline
  def all: all = "all".asInstanceOf[all]
  
  @js.native
  sealed trait allArgumentPoints extends StObject
  @scala.inline
  def allArgumentPoints: allArgumentPoints = "allArgumentPoints".asInstanceOf[allArgumentPoints]
  
  @js.native
  sealed trait allPages extends StObject
  @scala.inline
  def allPages: allPages = "allPages".asInstanceOf[allPages]
  
  @js.native
  sealed trait allSeriesPoints extends StObject
  @scala.inline
  def allSeriesPoints: allSeriesPoints = "allSeriesPoints".asInstanceOf[allSeriesPoints]
  
  @js.native
  sealed trait alt extends StObject
  @scala.inline
  def alt: alt = "alt".asInstanceOf[alt]
  
  @js.native
  sealed trait alternate extends StObject
  @scala.inline
  def alternate: alternate = "alternate".asInstanceOf[alternate]
  
  @js.native
  sealed trait always extends StObject
  @scala.inline
  def always: always = "always".asInstanceOf[always]
  
  @js.native
  sealed trait and extends StObject
  @scala.inline
  def and: and = "and".asInstanceOf[and]
  
  @js.native
  sealed trait android extends StObject
  @scala.inline
  def android: android = "android".asInstanceOf[android]
  
  @js.native
  sealed trait anticlockwise extends StObject
  @scala.inline
  def anticlockwise: anticlockwise = "anticlockwise".asInstanceOf[anticlockwise]
  
  @js.native
  sealed trait anyof extends StObject
  @scala.inline
  def anyof: anyof = "anyof".asInstanceOf[anyof]
  
  @js.native
  sealed trait area extends StObject
  @scala.inline
  def area: area = "area".asInstanceOf[area]
  
  @js.native
  sealed trait array extends StObject
  @scala.inline
  def array: array = "array".asInstanceOf[array]
  
  @js.native
  sealed trait arrow extends StObject
  @scala.inline
  def arrow: arrow = "arrow".asInstanceOf[arrow]
  
  @js.native
  sealed trait arrowBottom extends StObject
  @scala.inline
  def arrowBottom: arrowBottom = "arrowBottom".asInstanceOf[arrowBottom]
  
  @js.native
  sealed trait arrowEastWest extends StObject
  @scala.inline
  def arrowEastWest: arrowEastWest = "arrowEastWest".asInstanceOf[arrowEastWest]
  
  @js.native
  sealed trait arrowLeft extends StObject
  @scala.inline
  def arrowLeft: arrowLeft = "arrowLeft".asInstanceOf[arrowLeft]
  
  @js.native
  sealed trait arrowNorthSouth extends StObject
  @scala.inline
  def arrowNorthSouth: arrowNorthSouth = "arrowNorthSouth".asInstanceOf[arrowNorthSouth]
  
  @js.native
  sealed trait arrowRight extends StObject
  @scala.inline
  def arrowRight: arrowRight = "arrowRight".asInstanceOf[arrowRight]
  
  @js.native
  sealed trait arrowTop extends StObject
  @scala.inline
  def arrowTop: arrowTop = "arrowTop".asInstanceOf[arrowTop]
  
  @js.native
  sealed trait asc extends StObject
  @scala.inline
  def asc: asc = "asc".asInstanceOf[asc]
  
  @js.native
  sealed trait async extends StObject
  @scala.inline
  def async: async = "async".asInstanceOf[async]
  
  @js.native
  sealed trait auto extends StObject
  @scala.inline
  def auto: auto = "auto".asInstanceOf[auto]
  
  @js.native
  sealed trait avg extends StObject
  @scala.inline
  def avg: avg = "avg".asInstanceOf[avg]
  
  @js.native
  sealed trait back extends StObject
  @scala.inline
  def back: back = "back".asInstanceOf[back]
  
  @js.native
  sealed trait background extends StObject
  @scala.inline
  def background: background = "background".asInstanceOf[background]
  
  @js.native
  sealed trait bar extends StObject
  @scala.inline
  def bar: bar = "bar".asInstanceOf[bar]
  
  @js.native
  sealed trait batch extends StObject
  @scala.inline
  def batch: batch = "batch".asInstanceOf[batch]
  
  @js.native
  sealed trait before extends StObject
  @scala.inline
  def before: before = "before".asInstanceOf[before]
  
  @js.native
  sealed trait beforeChangeConnectorText extends StObject
  @scala.inline
  def beforeChangeConnectorText: beforeChangeConnectorText = "beforeChangeConnectorText".asInstanceOf[beforeChangeConnectorText]
  
  @js.native
  sealed trait beforeChangeShapeText extends StObject
  @scala.inline
  def beforeChangeShapeText: beforeChangeShapeText = "beforeChangeShapeText".asInstanceOf[beforeChangeShapeText]
  
  @js.native
  sealed trait between extends StObject
  @scala.inline
  def between: between = "between".asInstanceOf[between]
  
  @js.native
  sealed trait betweenLabels extends StObject
  @scala.inline
  def betweenLabels: betweenLabels = "betweenLabels".asInstanceOf[betweenLabels]
  
  @js.native
  sealed trait billions extends _format
  @scala.inline
  def billions: billions = "billions".asInstanceOf[billions]
  
  @js.native
  sealed trait bing extends StObject
  @scala.inline
  def bing: bing = "bing".asInstanceOf[bing]
  
  @js.native
  sealed trait blend extends StObject
  @scala.inline
  def blend: blend = "blend".asInstanceOf[blend]
  
  @js.native
  sealed trait blockquote extends StObject
  @scala.inline
  def blockquote: blockquote = "blockquote".asInstanceOf[blockquote]
  
  @js.native
  sealed trait bold extends StObject
  @scala.inline
  def bold: bold = "bold".asInstanceOf[bold]
  
  @js.native
  sealed trait boolean_ extends StObject
  @scala.inline
  def boolean_ : boolean_ = "boolean".asInstanceOf[boolean_]
  
  @js.native
  sealed trait both extends StObject
  @scala.inline
  def both: both = "both".asInstanceOf[both]
  
  @js.native
  sealed trait bottom extends StObject
  @scala.inline
  def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait breakWord extends StObject
  @scala.inline
  def breakWord: breakWord = "breakWord".asInstanceOf[breakWord]
  
  @js.native
  sealed trait bringToFront extends StObject
  @scala.inline
  def bringToFront: bringToFront = "bringToFront".asInstanceOf[bringToFront]
  
  @js.native
  sealed trait bubble extends StObject
  @scala.inline
  def bubble: bubble = "bubble".asInstanceOf[bubble]
  
  @js.native
  sealed trait bulletList extends StObject
  @scala.inline
  def bulletList: bulletList = "bulletList".asInstanceOf[bulletList]
  
  @js.native
  sealed trait button extends StObject
  @scala.inline
  def button: button = "button".asInstanceOf[button]
  
  @js.native
  sealed trait buttonClick extends StObject
  @scala.inline
  def buttonClick: buttonClick = "buttonClick".asInstanceOf[buttonClick]
  
  @js.native
  sealed trait buttons extends StObject
  @scala.inline
  def buttons: buttons = "buttons".asInstanceOf[buttons]
  
  @js.native
  sealed trait calendar extends StObject
  @scala.inline
  def calendar: calendar = "calendar".asInstanceOf[calendar]
  
  @js.native
  sealed trait cancel extends StObject
  @scala.inline
  def cancel: cancel = "cancel".asInstanceOf[cancel]
  
  @js.native
  sealed trait candlestick extends StObject
  @scala.inline
  def candlestick: candlestick = "candlestick".asInstanceOf[candlestick]
  
  @js.native
  sealed trait cardWithImageOnLeft extends StObject
  @scala.inline
  def cardWithImageOnLeft: cardWithImageOnLeft = "cardWithImageOnLeft".asInstanceOf[cardWithImageOnLeft]
  
  @js.native
  sealed trait cardWithImageOnRight extends StObject
  @scala.inline
  def cardWithImageOnRight: cardWithImageOnRight = "cardWithImageOnRight".asInstanceOf[cardWithImageOnRight]
  
  @js.native
  sealed trait cardWithImageOnTop extends StObject
  @scala.inline
  def cardWithImageOnTop: cardWithImageOnTop = "cardWithImageOnTop".asInstanceOf[cardWithImageOnTop]
  
  @js.native
  sealed trait cell extends StObject
  @scala.inline
  def cell: cell = "cell".asInstanceOf[cell]
  
  @js.native
  sealed trait center extends StObject
  @scala.inline
  def center: center = "center".asInstanceOf[center]
  
  @js.native
  sealed trait centerBottom extends StObject
  @scala.inline
  def centerBottom: centerBottom = "centerBottom".asInstanceOf[centerBottom]
  
  @js.native
  sealed trait centerContinuous extends StObject
  @scala.inline
  def centerContinuous: centerContinuous = "centerContinuous".asInstanceOf[centerContinuous]
  
  @js.native
  sealed trait centerTop extends StObject
  @scala.inline
  def centerTop: centerTop = "centerTop".asInstanceOf[centerTop]
  
  @js.native
  sealed trait century extends StObject
  @scala.inline
  def century: century = "century".asInstanceOf[century]
  
  @js.native
  sealed trait changeConnection extends StObject
  @scala.inline
  def changeConnection: changeConnection = "changeConnection".asInstanceOf[changeConnection]
  
  @js.native
  sealed trait changeConnectorPoints extends StObject
  @scala.inline
  def changeConnectorPoints: changeConnectorPoints = "changeConnectorPoints".asInstanceOf[changeConnectorPoints]
  
  @js.native
  sealed trait changeConnectorText extends StObject
  @scala.inline
  def changeConnectorText: changeConnectorText = "changeConnectorText".asInstanceOf[changeConnectorText]
  
  @js.native
  sealed trait changeShapeText extends StObject
  @scala.inline
  def changeShapeText: changeShapeText = "changeShapeText".asInstanceOf[changeShapeText]
  
  @js.native
  sealed trait circle extends StObject
  @scala.inline
  def circle: circle = "circle".asInstanceOf[circle]
  
  @js.native
  sealed trait clear extends StObject
  @scala.inline
  def clear: clear = "clear".asInstanceOf[clear]
  
  @js.native
  sealed trait clearSelection extends StObject
  @scala.inline
  def clearSelection: clearSelection = "clearSelection".asInstanceOf[clearSelection]
  
  @js.native
  sealed trait click extends StObject
  @scala.inline
  def click: click = "click".asInstanceOf[click]
  
  @js.native
  sealed trait clockwise extends StObject
  @scala.inline
  def clockwise: clockwise = "clockwise".asInstanceOf[clockwise]
  
  @js.native
  sealed trait close extends StObject
  @scala.inline
  def close: close = "close".asInstanceOf[close]
  
  @js.native
  sealed trait cm extends StObject
  @scala.inline
  def cm: cm = "cm".asInstanceOf[cm]
  
  @js.native
  sealed trait `code-block` extends StObject
  @scala.inline
  def `code-block`: `code-block` = "code-block".asInstanceOf[`code-block`]
  
  @js.native
  sealed trait codeBlock extends StObject
  @scala.inline
  def codeBlock: codeBlock = "codeBlock".asInstanceOf[codeBlock]
  
  @js.native
  sealed trait col extends StObject
  @scala.inline
  def col: col = "col".asInstanceOf[col]
  
  @js.native
  sealed trait collapseAll extends StObject
  @scala.inline
  def collapseAll: collapseAll = "collapseAll".asInstanceOf[collapseAll]
  
  @js.native
  sealed trait collapsed extends StObject
  @scala.inline
  def collapsed: collapsed = "collapsed".asInstanceOf[collapsed]
  
  @js.native
  sealed trait color extends StObject
  @scala.inline
  def color: color = "color".asInstanceOf[color]
  
  @js.native
  sealed trait column extends StObject
  @scala.inline
  def column: column = "column".asInstanceOf[column]
  
  @js.native
  sealed trait columns extends StObject
  @scala.inline
  def columns: columns = "columns".asInstanceOf[columns]
  
  @js.native
  sealed trait compare extends StObject
  @scala.inline
  def compare: compare = "compare".asInstanceOf[compare]
  
  @js.native
  sealed trait connector extends StObject
  @scala.inline
  def connector: connector = "connector".asInstanceOf[connector]
  
  @js.native
  sealed trait connectorLineEnd extends StObject
  @scala.inline
  def connectorLineEnd: connectorLineEnd = "connectorLineEnd".asInstanceOf[connectorLineEnd]
  
  @js.native
  sealed trait connectorLineStart extends StObject
  @scala.inline
  def connectorLineStart: connectorLineStart = "connectorLineStart".asInstanceOf[connectorLineStart]
  
  @js.native
  sealed trait connectorLineType extends StObject
  @scala.inline
  def connectorLineType: connectorLineType = "connectorLineType".asInstanceOf[connectorLineType]
  
  @js.native
  sealed trait contained extends StObject
  @scala.inline
  def contained: contained = "contained".asInstanceOf[contained]
  
  @js.native
  sealed trait containers extends StObject
  @scala.inline
  def containers: containers = "containers".asInstanceOf[containers]
  
  @js.native
  sealed trait contains extends StObject
  @scala.inline
  def contains: contains = "contains".asInstanceOf[contains]
  
  @js.native
  sealed trait context extends StObject
  @scala.inline
  def context: context = "context".asInstanceOf[context]
  
  @js.native
  sealed trait continuous extends StObject
  @scala.inline
  def continuous: continuous = "continuous".asInstanceOf[continuous]
  
  @js.native
  sealed trait copy extends StObject
  @scala.inline
  def copy: copy = "copy".asInstanceOf[copy]
  
  @js.native
  sealed trait count extends StObject
  @scala.inline
  def count: count = "count".asInstanceOf[count]
  
  @js.native
  sealed trait create extends StObject
  @scala.inline
  def create: create = "create".asInstanceOf[create]
  
  @js.native
  sealed trait cross extends StObject
  @scala.inline
  def cross: cross = "cross".asInstanceOf[cross]
  
  @js.native
  sealed trait crossLabels extends StObject
  @scala.inline
  def crossLabels: crossLabels = "crossLabels".asInstanceOf[crossLabels]
  
  @js.native
  sealed trait css extends StObject
  @scala.inline
  def css: css = "css".asInstanceOf[css]
  
  @js.native
  sealed trait ctrl extends StObject
  @scala.inline
  def ctrl: ctrl = "ctrl".asInstanceOf[ctrl]
  
  @js.native
  sealed trait currency extends _format
  @scala.inline
  def currency: currency = "currency".asInstanceOf[currency]
  
  @js.native
  sealed trait custom extends StObject
  @scala.inline
  def custom: custom = "custom".asInstanceOf[custom]
  
  @js.native
  sealed trait cut extends StObject
  @scala.inline
  def cut: cut = "cut".asInstanceOf[cut]
  
  @js.native
  sealed trait danger extends StObject
  @scala.inline
  def danger: danger = "danger".asInstanceOf[danger]
  
  @js.native
  sealed trait darkDown extends StObject
  @scala.inline
  def darkDown: darkDown = "darkDown".asInstanceOf[darkDown]
  
  @js.native
  sealed trait darkGray extends StObject
  @scala.inline
  def darkGray: darkGray = "darkGray".asInstanceOf[darkGray]
  
  @js.native
  sealed trait darkGrid extends StObject
  @scala.inline
  def darkGrid: darkGrid = "darkGrid".asInstanceOf[darkGrid]
  
  @js.native
  sealed trait darkHorizontal extends StObject
  @scala.inline
  def darkHorizontal: darkHorizontal = "darkHorizontal".asInstanceOf[darkHorizontal]
  
  @js.native
  sealed trait darkTrellis extends StObject
  @scala.inline
  def darkTrellis: darkTrellis = "darkTrellis".asInstanceOf[darkTrellis]
  
  @js.native
  sealed trait darkUp extends StObject
  @scala.inline
  def darkUp: darkUp = "darkUp".asInstanceOf[darkUp]
  
  @js.native
  sealed trait darkVertical extends StObject
  @scala.inline
  def darkVertical: darkVertical = "darkVertical".asInstanceOf[darkVertical]
  
  @js.native
  sealed trait dash extends StObject
  @scala.inline
  def dash: dash = "dash".asInstanceOf[dash]
  
  @js.native
  sealed trait data extends StObject
  @scala.inline
  def data: data = "data".asInstanceOf[data]
  
  @js.native
  sealed trait database extends StObject
  @scala.inline
  def database: database = "database".asInstanceOf[database]
  
  @js.native
  sealed trait date extends StObject
  @scala.inline
  def date: date = "date".asInstanceOf[date]
  
  @js.native
  sealed trait datetime extends StObject
  @scala.inline
  def datetime: datetime = "datetime".asInstanceOf[datetime]
  
  @js.native
  sealed trait day
    extends _VizTimeInterval
       with _format
  @scala.inline
  def day: day = "day".asInstanceOf[day]
  
  @js.native
  sealed trait dayOfWeek extends _format
  @scala.inline
  def dayOfWeek: dayOfWeek = "dayOfWeek".asInstanceOf[dayOfWeek]
  
  @js.native
  sealed trait days extends StObject
  @scala.inline
  def days: days = "days".asInstanceOf[days]
  
  @js.native
  sealed trait dblClick extends StObject
  @scala.inline
  def dblClick: dblClick = "dblClick".asInstanceOf[dblClick]
  
  @js.native
  sealed trait dblclick_ extends StObject
  @scala.inline
  def dblclick_ : dblclick_ = "dblclick".asInstanceOf[dblclick_]
  
  @js.native
  sealed trait decade extends StObject
  @scala.inline
  def decade: decade = "decade".asInstanceOf[decade]
  
  @js.native
  sealed trait decimal_ extends _format
  @scala.inline
  def decimal_ : decimal_ = "decimal".asInstanceOf[decimal_]
  
  @js.native
  sealed trait decision extends StObject
  @scala.inline
  def decision: decision = "decision".asInstanceOf[decision]
  
  @js.native
  sealed trait decreaseIndent extends StObject
  @scala.inline
  def decreaseIndent: decreaseIndent = "decreaseIndent".asInstanceOf[decreaseIndent]
  
  @js.native
  sealed trait delay extends StObject
  @scala.inline
  def delay: delay = "delay".asInstanceOf[delay]
  
  @js.native
  sealed trait delete extends StObject
  @scala.inline
  def delete: delete = "delete".asInstanceOf[delete]
  
  @js.native
  sealed trait deleteColumn extends StObject
  @scala.inline
  def deleteColumn: deleteColumn = "deleteColumn".asInstanceOf[deleteColumn]
  
  @js.native
  sealed trait deleteConnector extends StObject
  @scala.inline
  def deleteConnector: deleteConnector = "deleteConnector".asInstanceOf[deleteConnector]
  
  @js.native
  sealed trait deleteDependency extends StObject
  @scala.inline
  def deleteDependency: deleteDependency = "deleteDependency".asInstanceOf[deleteDependency]
  
  @js.native
  sealed trait deleteRow extends StObject
  @scala.inline
  def deleteRow: deleteRow = "deleteRow".asInstanceOf[deleteRow]
  
  @js.native
  sealed trait deleteShape extends StObject
  @scala.inline
  def deleteShape: deleteShape = "deleteShape".asInstanceOf[deleteShape]
  
  @js.native
  sealed trait deleteShapeImage extends StObject
  @scala.inline
  def deleteShapeImage: deleteShapeImage = "deleteShapeImage".asInstanceOf[deleteShapeImage]
  
  @js.native
  sealed trait deleteTable extends StObject
  @scala.inline
  def deleteTable: deleteTable = "deleteTable".asInstanceOf[deleteTable]
  
  @js.native
  sealed trait deleteTask extends StObject
  @scala.inline
  def deleteTask: deleteTask = "deleteTask".asInstanceOf[deleteTask]
  
  @js.native
  sealed trait desc extends StObject
  @scala.inline
  def desc: desc = "desc".asInstanceOf[desc]
  
  @js.native
  sealed trait desktop extends StObject
  @scala.inline
  def desktop: desktop = "desktop".asInstanceOf[desktop]
  
  @js.native
  sealed trait detailExpand extends StObject
  @scala.inline
  def detailExpand: detailExpand = "detailExpand".asInstanceOf[detailExpand]
  
  @js.native
  sealed trait details extends StObject
  @scala.inline
  def details: details = "details".asInstanceOf[details]
  
  @js.native
  sealed trait dialog extends StObject
  @scala.inline
  def dialog: dialog = "dialog".asInstanceOf[dialog]
  
  @js.native
  sealed trait diamond extends StObject
  @scala.inline
  def diamond: diamond = "diamond".asInstanceOf[diamond]
  
  @js.native
  sealed trait disabled extends StObject
  @scala.inline
  def disabled: disabled = "disabled".asInstanceOf[disabled]
  
  @js.native
  sealed trait discrete extends StObject
  @scala.inline
  def discrete: discrete = "discrete".asInstanceOf[discrete]
  
  @js.native
  sealed trait display extends StObject
  @scala.inline
  def display: display = "display".asInstanceOf[display]
  
  @js.native
  sealed trait displayText extends StObject
  @scala.inline
  def displayText: displayText = "displayText".asInstanceOf[displayText]
  
  @js.native
  sealed trait distributed extends StObject
  @scala.inline
  def distributed: distributed = "distributed".asInstanceOf[distributed]
  
  @js.native
  sealed trait document extends StObject
  @scala.inline
  def document: document = "document".asInstanceOf[document]
  
  @js.native
  sealed trait donut extends StObject
  @scala.inline
  def donut: donut = "donut".asInstanceOf[donut]
  
  @js.native
  sealed trait dot extends StObject
  @scala.inline
  def dot: dot = "dot".asInstanceOf[dot]
  
  @js.native
  sealed trait double extends StObject
  @scala.inline
  def double: double = "double".asInstanceOf[double]
  
  @js.native
  sealed trait doubleAccounting extends StObject
  @scala.inline
  def doubleAccounting: doubleAccounting = "doubleAccounting".asInstanceOf[doubleAccounting]
  
  @js.native
  sealed trait doughnut extends StObject
  @scala.inline
  def doughnut: doughnut = "doughnut".asInstanceOf[doughnut]
  
  @js.native
  sealed trait down extends StObject
  @scala.inline
  def down: down = "down".asInstanceOf[down]
  
  @js.native
  sealed trait download extends StObject
  @scala.inline
  def download: download = "download".asInstanceOf[download]
  
  @js.native
  sealed trait drag extends StObject
  @scala.inline
  def drag: drag = "drag".asInstanceOf[drag]
  
  @js.native
  sealed trait dragAndDrop extends StObject
  @scala.inline
  def dragAndDrop: dragAndDrop = "dragAndDrop".asInstanceOf[dragAndDrop]
  
  @js.native
  sealed trait driving extends StObject
  @scala.inline
  def driving: driving = "driving".asInstanceOf[driving]
  
  @js.native
  sealed trait dropDown extends StObject
  @scala.inline
  def dropDown: dropDown = "dropDown".asInstanceOf[dropDown]
  
  @js.native
  sealed trait dxAutocomplete extends StObject
  @scala.inline
  def dxAutocomplete: dxAutocomplete = "dxAutocomplete".asInstanceOf[dxAutocomplete]
  
  @js.native
  sealed trait dxButton extends StObject
  @scala.inline
  def dxButton: dxButton = "dxButton".asInstanceOf[dxButton]
  
  @js.native
  sealed trait dxButtonGroup extends StObject
  @scala.inline
  def dxButtonGroup: dxButtonGroup = "dxButtonGroup".asInstanceOf[dxButtonGroup]
  
  @js.native
  sealed trait dxCalendar extends StObject
  @scala.inline
  def dxCalendar: dxCalendar = "dxCalendar".asInstanceOf[dxCalendar]
  
  @js.native
  sealed trait dxCheckBox extends StObject
  @scala.inline
  def dxCheckBox: dxCheckBox = "dxCheckBox".asInstanceOf[dxCheckBox]
  
  @js.native
  sealed trait dxColorBox extends StObject
  @scala.inline
  def dxColorBox: dxColorBox = "dxColorBox".asInstanceOf[dxColorBox]
  
  @js.native
  sealed trait dxDateBox extends StObject
  @scala.inline
  def dxDateBox: dxDateBox = "dxDateBox".asInstanceOf[dxDateBox]
  
  @js.native
  sealed trait dxDropDownBox extends StObject
  @scala.inline
  def dxDropDownBox: dxDropDownBox = "dxDropDownBox".asInstanceOf[dxDropDownBox]
  
  @js.native
  sealed trait dxDropDownButton extends StObject
  @scala.inline
  def dxDropDownButton: dxDropDownButton = "dxDropDownButton".asInstanceOf[dxDropDownButton]
  
  @js.native
  sealed trait dxHtmlEditor extends StObject
  @scala.inline
  def dxHtmlEditor: dxHtmlEditor = "dxHtmlEditor".asInstanceOf[dxHtmlEditor]
  
  @js.native
  sealed trait dxLookup extends StObject
  @scala.inline
  def dxLookup: dxLookup = "dxLookup".asInstanceOf[dxLookup]
  
  @js.native
  sealed trait dxMenu extends StObject
  @scala.inline
  def dxMenu: dxMenu = "dxMenu".asInstanceOf[dxMenu]
  
  @js.native
  sealed trait dxNumberBox extends StObject
  @scala.inline
  def dxNumberBox: dxNumberBox = "dxNumberBox".asInstanceOf[dxNumberBox]
  
  @js.native
  sealed trait dxRadioGroup extends StObject
  @scala.inline
  def dxRadioGroup: dxRadioGroup = "dxRadioGroup".asInstanceOf[dxRadioGroup]
  
  @js.native
  sealed trait dxRangeSlider extends StObject
  @scala.inline
  def dxRangeSlider: dxRangeSlider = "dxRangeSlider".asInstanceOf[dxRangeSlider]
  
  @js.native
  sealed trait dxSelectBox extends StObject
  @scala.inline
  def dxSelectBox: dxSelectBox = "dxSelectBox".asInstanceOf[dxSelectBox]
  
  @js.native
  sealed trait dxSlider extends StObject
  @scala.inline
  def dxSlider: dxSlider = "dxSlider".asInstanceOf[dxSlider]
  
  @js.native
  sealed trait dxSwitch extends StObject
  @scala.inline
  def dxSwitch: dxSwitch = "dxSwitch".asInstanceOf[dxSwitch]
  
  @js.native
  sealed trait dxTabs extends StObject
  @scala.inline
  def dxTabs: dxTabs = "dxTabs".asInstanceOf[dxTabs]
  
  @js.native
  sealed trait dxTagBox extends StObject
  @scala.inline
  def dxTagBox: dxTagBox = "dxTagBox".asInstanceOf[dxTagBox]
  
  @js.native
  sealed trait dxTextArea extends StObject
  @scala.inline
  def dxTextArea: dxTextArea = "dxTextArea".asInstanceOf[dxTextArea]
  
  @js.native
  sealed trait dxTextBox extends StObject
  @scala.inline
  def dxTextBox: dxTextBox = "dxTextBox".asInstanceOf[dxTextBox]
  
  @js.native
  sealed trait dynamicHeight extends StObject
  @scala.inline
  def dynamicHeight: dynamicHeight = "dynamicHeight".asInstanceOf[dynamicHeight]
  
  @js.native
  sealed trait dynamicSlope extends StObject
  @scala.inline
  def dynamicSlope: dynamicSlope = "dynamicSlope".asInstanceOf[dynamicSlope]
  
  @js.native
  sealed trait easeOutCubic extends StObject
  @scala.inline
  def easeOutCubic: easeOutCubic = "easeOutCubic".asInstanceOf[easeOutCubic]
  
  @js.native
  sealed trait edge extends StObject
  @scala.inline
  def edge: edge = "edge".asInstanceOf[edge]
  
  @js.native
  sealed trait edit extends StObject
  @scala.inline
  def edit: edit = "edit".asInstanceOf[edit]
  
  @js.native
  sealed trait editShapeImage extends StObject
  @scala.inline
  def editShapeImage: editShapeImage = "editShapeImage".asInstanceOf[editShapeImage]
  
  @js.native
  sealed trait ellipse extends StObject
  @scala.inline
  def ellipse: ellipse = "ellipse".asInstanceOf[ellipse]
  
  @js.native
  sealed trait ellipsis extends StObject
  @scala.inline
  def ellipsis: ellipsis = "ellipsis".asInstanceOf[ellipsis]
  
  @js.native
  sealed trait email extends StObject
  @scala.inline
  def email: email = "email".asInstanceOf[email]
  
  @js.native
  sealed trait empty extends StObject
  @scala.inline
  def empty: empty = "empty".asInstanceOf[empty]
  
  @js.native
  sealed trait end extends StObject
  @scala.inline
  def end: end = "end".asInstanceOf[end]
  
  @js.native
  sealed trait endswith extends StObject
  @scala.inline
  def endswith: endswith = "endswith".asInstanceOf[endswith]
  
  @js.native
  sealed trait equals extends StObject
  @scala.inline
  def equals: equals = "equals".asInstanceOf[equals]
  
  @js.native
  sealed trait equirectangular extends StObject
  @scala.inline
  def equirectangular: equirectangular = "equirectangular".asInstanceOf[equirectangular]
  
  @js.native
  sealed trait error extends StObject
  @scala.inline
  def error: error = "error".asInstanceOf[error]
  
  @js.native
  sealed trait exclude extends StObject
  @scala.inline
  def exclude: exclude = "exclude".asInstanceOf[exclude]
  
  @js.native
  sealed trait excludePoints extends StObject
  @scala.inline
  def excludePoints: excludePoints = "excludePoints".asInstanceOf[excludePoints]
  
  @js.native
  sealed trait expand extends StObject
  @scala.inline
  def expand: expand = "expand".asInstanceOf[expand]
  
  @js.native
  sealed trait expandAll extends StObject
  @scala.inline
  def expandAll: expandAll = "expandAll".asInstanceOf[expandAll]
  
  @js.native
  sealed trait exponential extends _format
  @scala.inline
  def exponential: exponential = "exponential".asInstanceOf[exponential]
  
  @js.native
  sealed trait exportJpg extends StObject
  @scala.inline
  def exportJpg: exportJpg = "exportJpg".asInstanceOf[exportJpg]
  
  @js.native
  sealed trait exportPng extends StObject
  @scala.inline
  def exportPng: exportPng = "exportPng".asInstanceOf[exportPng]
  
  @js.native
  sealed trait exportSvg extends StObject
  @scala.inline
  def exportSvg: exportSvg = "exportSvg".asInstanceOf[exportSvg]
  
  @js.native
  sealed trait extrapolate extends StObject
  @scala.inline
  def extrapolate: extrapolate = "extrapolate".asInstanceOf[extrapolate]
  
  @js.native
  sealed trait fade extends StObject
  @scala.inline
  def fade: fade = "fade".asInstanceOf[fade]
  
  @js.native
  sealed trait fadeIn extends StObject
  @scala.inline
  def fadeIn: fadeIn = "fadeIn".asInstanceOf[fadeIn]
  
  @js.native
  sealed trait fadeOut extends StObject
  @scala.inline
  def fadeOut: fadeOut = "fadeOut".asInstanceOf[fadeOut]
  
  @js.native
  sealed trait fill extends StObject
  @scala.inline
  def fill: fill = "fill".asInstanceOf[fill]
  
  @js.native
  sealed trait fillColor extends StObject
  @scala.inline
  def fillColor: fillColor = "fillColor".asInstanceOf[fillColor]
  
  @js.native
  sealed trait filled extends StObject
  @scala.inline
  def filled: filled = "filled".asInstanceOf[filled]
  
  @js.native
  sealed trait filledTriangle extends StObject
  @scala.inline
  def filledTriangle: filledTriangle = "filledTriangle".asInstanceOf[filledTriangle]
  
  @js.native
  sealed trait filter extends StObject
  @scala.inline
  def filter: filter = "filter".asInstanceOf[filter]
  
  @js.native
  sealed trait first extends StObject
  @scala.inline
  def first: first = "first".asInstanceOf[first]
  
  @js.native
  sealed trait fit extends StObject
  @scala.inline
  def fit: fit = "fit".asInstanceOf[fit]
  
  @js.native
  sealed trait `fit flip` extends StObject
  @scala.inline
  def `fit flip`: `fit flip` = ("fit flip").asInstanceOf[`fit flip`]
  
  @js.native
  sealed trait `fit flipfit` extends StObject
  @scala.inline
  def `fit flipfit`: `fit flipfit` = ("fit flipfit").asInstanceOf[`fit flipfit`]
  
  @js.native
  sealed trait `fit none` extends StObject
  @scala.inline
  def `fit none`: `fit none` = ("fit none").asInstanceOf[`fit none`]
  
  @js.native
  sealed trait fitContent extends StObject
  @scala.inline
  def fitContent: fitContent = "fitContent".asInstanceOf[fitContent]
  
  @js.native
  sealed trait fitWidth extends StObject
  @scala.inline
  def fitWidth: fitWidth = "fitWidth".asInstanceOf[fitWidth]
  
  @js.native
  sealed trait fixed extends StObject
  @scala.inline
  def fixed: fixed = "fixed".asInstanceOf[fixed]
  
  @js.native
  sealed trait fixedPoint extends _format
  @scala.inline
  def fixedPoint: fixedPoint = "fixedPoint".asInstanceOf[fixedPoint]
  
  @js.native
  sealed trait flip extends StObject
  @scala.inline
  def flip: flip = "flip".asInstanceOf[flip]
  
  @js.native
  sealed trait `flip fit` extends StObject
  @scala.inline
  def `flip fit`: `flip fit` = ("flip fit").asInstanceOf[`flip fit`]
  
  @js.native
  sealed trait `flip none` extends StObject
  @scala.inline
  def `flip none`: `flip none` = ("flip none").asInstanceOf[`flip none`]
  
  @js.native
  sealed trait flipfit extends StObject
  @scala.inline
  def flipfit: flipfit = "flipfit".asInstanceOf[flipfit]
  
  @js.native
  sealed trait `flipfit fit` extends StObject
  @scala.inline
  def `flipfit fit`: `flipfit fit` = ("flipfit fit").asInstanceOf[`flipfit fit`]
  
  @js.native
  sealed trait `flipfit none` extends StObject
  @scala.inline
  def `flipfit none`: `flipfit none` = ("flipfit none").asInstanceOf[`flipfit none`]
  
  @js.native
  sealed trait flowchart extends StObject
  @scala.inline
  def flowchart: flowchart = "flowchart".asInstanceOf[flowchart]
  
  @js.native
  sealed trait font extends StObject
  @scala.inline
  def font: font = "font".asInstanceOf[font]
  
  @js.native
  sealed trait fontColor extends StObject
  @scala.inline
  def fontColor: fontColor = "fontColor".asInstanceOf[fontColor]
  
  @js.native
  sealed trait fontName extends StObject
  @scala.inline
  def fontName: fontName = "fontName".asInstanceOf[fontName]
  
  @js.native
  sealed trait fontSize extends StObject
  @scala.inline
  def fontSize: fontSize = "fontSize".asInstanceOf[fontSize]
  
  @js.native
  sealed trait form extends StObject
  @scala.inline
  def form: form = "form".asInstanceOf[form]
  
  @js.native
  sealed trait full extends StObject
  @scala.inline
  def full: full = "full".asInstanceOf[full]
  
  @js.native
  sealed trait fullBranch extends StObject
  @scala.inline
  def fullBranch: fullBranch = "fullBranch".asInstanceOf[fullBranch]
  
  @js.native
  sealed trait fullScreen extends StObject
  @scala.inline
  def fullScreen: fullScreen = "fullScreen".asInstanceOf[fullScreen]
  
  @js.native
  sealed trait fullstackedarea extends StObject
  @scala.inline
  def fullstackedarea: fullstackedarea = "fullstackedarea".asInstanceOf[fullstackedarea]
  
  @js.native
  sealed trait fullstackedbar extends StObject
  @scala.inline
  def fullstackedbar: fullstackedbar = "fullstackedbar".asInstanceOf[fullstackedbar]
  
  @js.native
  sealed trait fullstackedline extends StObject
  @scala.inline
  def fullstackedline: fullstackedline = "fullstackedline".asInstanceOf[fullstackedline]
  
  @js.native
  sealed trait fullstackedspline extends StObject
  @scala.inline
  def fullstackedspline: fullstackedspline = "fullstackedspline".asInstanceOf[fullstackedspline]
  
  @js.native
  sealed trait fullstackedsplinearea extends StObject
  @scala.inline
  def fullstackedsplinearea: fullstackedsplinearea = "fullstackedsplinearea".asInstanceOf[fullstackedsplinearea]
  
  @js.native
  sealed trait general extends StObject
  @scala.inline
  def general: general = "general".asInstanceOf[general]
  
  @js.native
  sealed trait generic extends StObject
  @scala.inline
  def generic: generic = "generic".asInstanceOf[generic]
  
  @js.native
  sealed trait genericDotcarmine extends StObject
  @scala.inline
  def genericDotcarmine: genericDotcarmine = "generic.carmine".asInstanceOf[genericDotcarmine]
  
  @js.native
  sealed trait genericDotcontrast extends StObject
  @scala.inline
  def genericDotcontrast: genericDotcontrast = "generic.contrast".asInstanceOf[genericDotcontrast]
  
  @js.native
  sealed trait genericDotdark extends StObject
  @scala.inline
  def genericDotdark: genericDotdark = "generic.dark".asInstanceOf[genericDotdark]
  
  @js.native
  sealed trait genericDotdarkmoon extends StObject
  @scala.inline
  def genericDotdarkmoon: genericDotdarkmoon = "generic.darkmoon".asInstanceOf[genericDotdarkmoon]
  
  @js.native
  sealed trait genericDotdarkviolet extends StObject
  @scala.inline
  def genericDotdarkviolet: genericDotdarkviolet = "generic.darkviolet".asInstanceOf[genericDotdarkviolet]
  
  @js.native
  sealed trait genericDotgreenmist extends StObject
  @scala.inline
  def genericDotgreenmist: genericDotgreenmist = "generic.greenmist".asInstanceOf[genericDotgreenmist]
  
  @js.native
  sealed trait genericDotlight extends StObject
  @scala.inline
  def genericDotlight: genericDotlight = "generic.light".asInstanceOf[genericDotlight]
  
  @js.native
  sealed trait genericDotsoftblue extends StObject
  @scala.inline
  def genericDotsoftblue: genericDotsoftblue = "generic.softblue".asInstanceOf[genericDotsoftblue]
  
  @js.native
  sealed trait google extends StObject
  @scala.inline
  def google: google = "google".asInstanceOf[google]
  
  @js.native
  sealed trait googleStatic extends StObject
  @scala.inline
  def googleStatic: googleStatic = "googleStatic".asInstanceOf[googleStatic]
  
  @js.native
  sealed trait gradient extends StObject
  @scala.inline
  def gradient: gradient = "gradient".asInstanceOf[gradient]
  
  @js.native
  sealed trait gradientSet extends StObject
  @scala.inline
  def gradientSet: gradientSet = "gradientSet".asInstanceOf[gradientSet]
  
  @js.native
  sealed trait gray0625 extends StObject
  @scala.inline
  def gray0625: gray0625 = "gray0625".asInstanceOf[gray0625]
  
  @js.native
  sealed trait gray125 extends StObject
  @scala.inline
  def gray125: gray125 = "gray125".asInstanceOf[gray125]
  
  @js.native
  sealed trait gridSize extends StObject
  @scala.inline
  def gridSize: gridSize = "gridSize".asInstanceOf[gridSize]
  
  @js.native
  sealed trait group extends StObject
  @scala.inline
  def group: group = "group".asInstanceOf[group]
  
  @js.native
  sealed trait groupExpand extends StObject
  @scala.inline
  def groupExpand: groupExpand = "groupExpand".asInstanceOf[groupExpand]
  
  @js.native
  sealed trait hardDisk extends StObject
  @scala.inline
  def hardDisk: hardDisk = "hardDisk".asInstanceOf[hardDisk]
  
  @js.native
  sealed trait header extends StObject
  @scala.inline
  def header: header = "header".asInstanceOf[header]
  
  @js.native
  sealed trait heart extends StObject
  @scala.inline
  def heart: heart = "heart".asInstanceOf[heart]
  
  @js.native
  sealed trait hide extends StObject
  @scala.inline
  def hide: hide = "hide".asInstanceOf[hide]
  
  @js.native
  sealed trait high extends StObject
  @scala.inline
  def high: high = "high".asInstanceOf[high]
  
  @js.native
  sealed trait horizontal extends StObject
  @scala.inline
  def horizontal: horizontal = "horizontal".asInstanceOf[horizontal]
  
  @js.native
  sealed trait horizontalContainer extends StObject
  @scala.inline
  def horizontalContainer: horizontalContainer = "horizontalContainer".asInstanceOf[horizontalContainer]
  
  @js.native
  sealed trait hour
    extends _VizTimeInterval
       with _format
  @scala.inline
  def hour: hour = "hour".asInstanceOf[hour]
  
  @js.native
  sealed trait hours extends StObject
  @scala.inline
  def hours: hours = "hours".asInstanceOf[hours]
  
  @js.native
  sealed trait hovered extends StObject
  @scala.inline
  def hovered: hovered = "hovered".asInstanceOf[hovered]
  
  @js.native
  sealed trait html extends StObject
  @scala.inline
  def html: html = "html".asInstanceOf[html]
  
  @js.native
  sealed trait hybrid extends StObject
  @scala.inline
  def hybrid: hybrid = "hybrid".asInstanceOf[hybrid]
  
  @js.native
  sealed trait icons extends StObject
  @scala.inline
  def icons: icons = "icons".asInstanceOf[icons]
  
  @js.native
  sealed trait image extends StObject
  @scala.inline
  def image: image = "image".asInstanceOf[image]
  
  @js.native
  sealed trait in extends StObject
  @scala.inline
  def in: in = "in".asInstanceOf[in]
  
  @js.native
  sealed trait inMenu extends StObject
  @scala.inline
  def inMenu: inMenu = "inMenu".asInstanceOf[inMenu]
  
  @js.native
  sealed trait include extends StObject
  @scala.inline
  def include: include = "include".asInstanceOf[include]
  
  @js.native
  sealed trait includePoints extends StObject
  @scala.inline
  def includePoints: includePoints = "includePoints".asInstanceOf[includePoints]
  
  @js.native
  sealed trait increaseIndent extends StObject
  @scala.inline
  def increaseIndent: increaseIndent = "increaseIndent".asInstanceOf[increaseIndent]
  
  @js.native
  sealed trait indent extends StObject
  @scala.inline
  def indent: indent = "indent".asInstanceOf[indent]
  
  @js.native
  sealed trait indicate extends StObject
  @scala.inline
  def indicate: indicate = "indicate".asInstanceOf[indicate]
  
  @js.native
  sealed trait indicatingSet extends StObject
  @scala.inline
  def indicatingSet: indicatingSet = "indicatingSet".asInstanceOf[indicatingSet]
  
  @js.native
  sealed trait infinite extends StObject
  @scala.inline
  def infinite: infinite = "infinite".asInstanceOf[infinite]
  
  @js.native
  sealed trait info extends StObject
  @scala.inline
  def info: info = "info".asInstanceOf[info]
  
  @js.native
  sealed trait insertColumnLeft extends StObject
  @scala.inline
  def insertColumnLeft: insertColumnLeft = "insertColumnLeft".asInstanceOf[insertColumnLeft]
  
  @js.native
  sealed trait insertColumnRight extends StObject
  @scala.inline
  def insertColumnRight: insertColumnRight = "insertColumnRight".asInstanceOf[insertColumnRight]
  
  @js.native
  sealed trait insertRowAbove extends StObject
  @scala.inline
  def insertRowAbove: insertRowAbove = "insertRowAbove".asInstanceOf[insertRowAbove]
  
  @js.native
  sealed trait insertRowBelow extends StObject
  @scala.inline
  def insertRowBelow: insertRowBelow = "insertRowBelow".asInstanceOf[insertRowBelow]
  
  @js.native
  sealed trait insertShapeImage extends StObject
  @scala.inline
  def insertShapeImage: insertShapeImage = "insertShapeImage".asInstanceOf[insertShapeImage]
  
  @js.native
  sealed trait insertTable extends StObject
  @scala.inline
  def insertTable: insertTable = "insertTable".asInstanceOf[insertTable]
  
  @js.native
  sealed trait inside extends StObject
  @scala.inline
  def inside: inside = "inside".asInstanceOf[inside]
  
  @js.native
  sealed trait instantly extends StObject
  @scala.inline
  def instantly: instantly = "instantly".asInstanceOf[instantly]
  
  @js.native
  sealed trait internalStorage extends StObject
  @scala.inline
  def internalStorage: internalStorage = "internalStorage".asInstanceOf[internalStorage]
  
  @js.native
  sealed trait invalid extends StObject
  @scala.inline
  def invalid: invalid = "invalid".asInstanceOf[invalid]
  
  @js.native
  sealed trait inverted extends StObject
  @scala.inline
  def inverted: inverted = "inverted".asInstanceOf[inverted]
  
  @js.native
  sealed trait ios extends StObject
  @scala.inline
  def ios: ios = "ios".asInstanceOf[ios]
  
  @js.native
  sealed trait isblank extends StObject
  @scala.inline
  def isblank: isblank = "isblank".asInstanceOf[isblank]
  
  @js.native
  sealed trait isnotblank extends StObject
  @scala.inline
  def isnotblank: isnotblank = "isnotblank".asInstanceOf[isnotblank]
  
  @js.native
  sealed trait italic extends StObject
  @scala.inline
  def italic: italic = "italic".asInstanceOf[italic]
  
  @js.native
  sealed trait itemView extends StObject
  @scala.inline
  def itemView: itemView = "itemView".asInstanceOf[itemView]
  
  @js.native
  sealed trait jpg extends StObject
  @scala.inline
  def jpg: jpg = "jpg".asInstanceOf[jpg]
  
  @js.native
  sealed trait justify extends StObject
  @scala.inline
  def justify: justify = "justify".asInstanceOf[justify]
  
  @js.native
  sealed trait keep extends StObject
  @scala.inline
  def keep: keep = "keep".asInstanceOf[keep]
  
  @js.native
  sealed trait lambert extends StObject
  @scala.inline
  def lambert: lambert = "lambert".asInstanceOf[lambert]
  
  @js.native
  sealed trait landscape extends StObject
  @scala.inline
  def landscape: landscape = "landscape".asInstanceOf[landscape]
  
  @js.native
  sealed trait largeNumber extends _format
  @scala.inline
  def largeNumber: largeNumber = "largeNumber".asInstanceOf[largeNumber]
  
  @js.native
  sealed trait last extends StObject
  @scala.inline
  def last: last = "last".asInstanceOf[last]
  
  @js.native
  sealed trait layered extends StObject
  @scala.inline
  def layered: layered = "layered".asInstanceOf[layered]
  
  @js.native
  sealed trait layoutLayeredBottomToTop extends StObject
  @scala.inline
  def layoutLayeredBottomToTop: layoutLayeredBottomToTop = "layoutLayeredBottomToTop".asInstanceOf[layoutLayeredBottomToTop]
  
  @js.native
  sealed trait layoutLayeredLeftToRight extends StObject
  @scala.inline
  def layoutLayeredLeftToRight: layoutLayeredLeftToRight = "layoutLayeredLeftToRight".asInstanceOf[layoutLayeredLeftToRight]
  
  @js.native
  sealed trait layoutLayeredRightToLeft extends StObject
  @scala.inline
  def layoutLayeredRightToLeft: layoutLayeredRightToLeft = "layoutLayeredRightToLeft".asInstanceOf[layoutLayeredRightToLeft]
  
  @js.native
  sealed trait layoutLayeredTopToBottom extends StObject
  @scala.inline
  def layoutLayeredTopToBottom: layoutLayeredTopToBottom = "layoutLayeredTopToBottom".asInstanceOf[layoutLayeredTopToBottom]
  
  @js.native
  sealed trait layoutTreeBottomToTop extends StObject
  @scala.inline
  def layoutTreeBottomToTop: layoutTreeBottomToTop = "layoutTreeBottomToTop".asInstanceOf[layoutTreeBottomToTop]
  
  @js.native
  sealed trait layoutTreeLeftToRight extends StObject
  @scala.inline
  def layoutTreeLeftToRight: layoutTreeLeftToRight = "layoutTreeLeftToRight".asInstanceOf[layoutTreeLeftToRight]
  
  @js.native
  sealed trait layoutTreeRightToLeft extends StObject
  @scala.inline
  def layoutTreeRightToLeft: layoutTreeRightToLeft = "layoutTreeRightToLeft".asInstanceOf[layoutTreeRightToLeft]
  
  @js.native
  sealed trait layoutTreeTopToBottom extends StObject
  @scala.inline
  def layoutTreeTopToBottom: layoutTreeTopToBottom = "layoutTreeTopToBottom".asInstanceOf[layoutTreeTopToBottom]
  
  @js.native
  sealed trait left extends StObject
  @scala.inline
  def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait `left bottom` extends StObject
  @scala.inline
  def `left bottom`: `left bottom` = ("left bottom").asInstanceOf[`left bottom`]
  
  @js.native
  sealed trait `left top` extends StObject
  @scala.inline
  def `left top`: `left top` = ("left top").asInstanceOf[`left top`]
  
  @js.native
  sealed trait leftBottom extends StObject
  @scala.inline
  def leftBottom: leftBottom = "leftBottom".asInstanceOf[leftBottom]
  
  @js.native
  sealed trait leftBottomRightTop extends StObject
  @scala.inline
  def leftBottomRightTop: leftBottomRightTop = "leftBottomRightTop".asInstanceOf[leftBottomRightTop]
  
  @js.native
  sealed trait leftCenter extends StObject
  @scala.inline
  def leftCenter: leftCenter = "leftCenter".asInstanceOf[leftCenter]
  
  @js.native
  sealed trait leftOrTop extends StObject
  @scala.inline
  def leftOrTop: leftOrTop = "leftOrTop".asInstanceOf[leftOrTop]
  
  @js.native
  sealed trait leftTop extends StObject
  @scala.inline
  def leftTop: leftTop = "leftTop".asInstanceOf[leftTop]
  
  @js.native
  sealed trait leftTopRightBottom extends StObject
  @scala.inline
  def leftTopRightBottom: leftTopRightBottom = "leftTopRightBottom".asInstanceOf[leftTopRightBottom]
  
  @js.native
  sealed trait lightDown extends StObject
  @scala.inline
  def lightDown: lightDown = "lightDown".asInstanceOf[lightDown]
  
  @js.native
  sealed trait lightGray extends StObject
  @scala.inline
  def lightGray: lightGray = "lightGray".asInstanceOf[lightGray]
  
  @js.native
  sealed trait lightGrid extends StObject
  @scala.inline
  def lightGrid: lightGrid = "lightGrid".asInstanceOf[lightGrid]
  
  @js.native
  sealed trait lightHorizontal extends StObject
  @scala.inline
  def lightHorizontal: lightHorizontal = "lightHorizontal".asInstanceOf[lightHorizontal]
  
  @js.native
  sealed trait lightTrellis extends StObject
  @scala.inline
  def lightTrellis: lightTrellis = "lightTrellis".asInstanceOf[lightTrellis]
  
  @js.native
  sealed trait lightUp extends StObject
  @scala.inline
  def lightUp: lightUp = "lightUp".asInstanceOf[lightUp]
  
  @js.native
  sealed trait lightVertical extends StObject
  @scala.inline
  def lightVertical: lightVertical = "lightVertical".asInstanceOf[lightVertical]
  
  @js.native
  sealed trait line extends StObject
  @scala.inline
  def line: line = "line".asInstanceOf[line]
  
  @js.native
  sealed trait lineColor extends StObject
  @scala.inline
  def lineColor: lineColor = "lineColor".asInstanceOf[lineColor]
  
  @js.native
  sealed trait linear extends StObject
  @scala.inline
  def linear: linear = "linear".asInstanceOf[linear]
  
  @js.native
  sealed trait link extends StObject
  @scala.inline
  def link: link = "link".asInstanceOf[link]
  
  @js.native
  sealed trait list extends StObject
  @scala.inline
  def list: list = "list".asInstanceOf[list]
  
  @js.native
  sealed trait local extends StObject
  @scala.inline
  def local: local = "local".asInstanceOf[local]
  
  @js.native
  sealed trait localStorage extends StObject
  @scala.inline
  def localStorage: localStorage = "localStorage".asInstanceOf[localStorage]
  
  @js.native
  sealed trait lock extends StObject
  @scala.inline
  def lock: lock = "lock".asInstanceOf[lock]
  
  @js.native
  sealed trait logarithmic extends StObject
  @scala.inline
  def logarithmic: logarithmic = "logarithmic".asInstanceOf[logarithmic]
  
  @js.native
  sealed trait longDash extends StObject
  @scala.inline
  def longDash: longDash = "longDash".asInstanceOf[longDash]
  
  @js.native
  sealed trait longDate extends _format
  @scala.inline
  def longDate: longDate = "longDate".asInstanceOf[longDate]
  
  @js.native
  sealed trait longDateLongTime extends _format
  @scala.inline
  def longDateLongTime: longDateLongTime = "longDateLongTime".asInstanceOf[longDateLongTime]
  
  @js.native
  sealed trait longTime extends _format
  @scala.inline
  def longTime: longTime = "longTime".asInstanceOf[longTime]
  
  @js.native
  sealed trait low extends StObject
  @scala.inline
  def low: low = "low".asInstanceOf[low]
  
  @js.native
  sealed trait manualInput extends StObject
  @scala.inline
  def manualInput: manualInput = "manualInput".asInstanceOf[manualInput]
  
  @js.native
  sealed trait manualOperation extends StObject
  @scala.inline
  def manualOperation: manualOperation = "manualOperation".asInstanceOf[manualOperation]
  
  @js.native
  sealed trait markdown extends StObject
  @scala.inline
  def markdown: markdown = "markdown".asInstanceOf[markdown]
  
  @js.native
  sealed trait marker extends StObject
  @scala.inline
  def marker: marker = "marker".asInstanceOf[marker]
  
  @js.native
  sealed trait matchOnly extends StObject
  @scala.inline
  def matchOnly: matchOnly = "matchOnly".asInstanceOf[matchOnly]
  
  @js.native
  sealed trait materialDotblueDotlight extends StObject
  @scala.inline
  def materialDotblueDotlight: materialDotblueDotlight = "material.blue.light".asInstanceOf[materialDotblueDotlight]
  
  @js.native
  sealed trait materialDotlimeDotlight extends StObject
  @scala.inline
  def materialDotlimeDotlight: materialDotlimeDotlight = "material.lime.light".asInstanceOf[materialDotlimeDotlight]
  
  @js.native
  sealed trait materialDotorangeDotlight extends StObject
  @scala.inline
  def materialDotorangeDotlight: materialDotorangeDotlight = "material.orange.light".asInstanceOf[materialDotorangeDotlight]
  
  @js.native
  sealed trait materialDotpurpleDotlight extends StObject
  @scala.inline
  def materialDotpurpleDotlight: materialDotpurpleDotlight = "material.purple.light".asInstanceOf[materialDotpurpleDotlight]
  
  @js.native
  sealed trait materialDottealDotlight extends StObject
  @scala.inline
  def materialDottealDotlight: materialDottealDotlight = "material.teal.light".asInstanceOf[materialDottealDotlight]
  
  @js.native
  sealed trait max extends StObject
  @scala.inline
  def max: max = "max".asInstanceOf[max]
  
  @js.native
  sealed trait mediumGray extends StObject
  @scala.inline
  def mediumGray: mediumGray = "mediumGray".asInstanceOf[mediumGray]
  
  @js.native
  sealed trait mercator extends StObject
  @scala.inline
  def mercator: mercator = "mercator".asInstanceOf[mercator]
  
  @js.native
  sealed trait merge extends StObject
  @scala.inline
  def merge: merge = "merge".asInstanceOf[merge]
  
  @js.native
  sealed trait meta extends StObject
  @scala.inline
  def meta: meta = "meta".asInstanceOf[meta]
  
  @js.native
  sealed trait miller extends StObject
  @scala.inline
  def miller: miller = "miller".asInstanceOf[miller]
  
  @js.native
  sealed trait millions extends _format
  @scala.inline
  def millions: millions = "millions".asInstanceOf[millions]
  
  @js.native
  sealed trait millisecond
    extends _VizTimeInterval
       with _format
  @scala.inline
  def millisecond: millisecond = "millisecond".asInstanceOf[millisecond]
  
  @js.native
  sealed trait min extends StObject
  @scala.inline
  def min: min = "min".asInstanceOf[min]
  
  @js.native
  sealed trait minute
    extends _VizTimeInterval
       with _format
  @scala.inline
  def minute: minute = "minute".asInstanceOf[minute]
  
  @js.native
  sealed trait minutes extends StObject
  @scala.inline
  def minutes: minutes = "minutes".asInstanceOf[minutes]
  
  @js.native
  sealed trait month
    extends _VizTimeInterval
       with _format
  @scala.inline
  def month: month = "month".asInstanceOf[month]
  
  @js.native
  sealed trait monthAndDay extends _format
  @scala.inline
  def monthAndDay: monthAndDay = "monthAndDay".asInstanceOf[monthAndDay]
  
  @js.native
  sealed trait monthAndYear extends _format
  @scala.inline
  def monthAndYear: monthAndYear = "monthAndYear".asInstanceOf[monthAndYear]
  
  @js.native
  sealed trait months extends StObject
  @scala.inline
  def months: months = "months".asInstanceOf[months]
  
  @js.native
  sealed trait move extends StObject
  @scala.inline
  def move: move = "move".asInstanceOf[move]
  
  @js.native
  sealed trait moveFocus extends StObject
  @scala.inline
  def moveFocus: moveFocus = "moveFocus".asInstanceOf[moveFocus]
  
  @js.native
  sealed trait moveShape extends StObject
  @scala.inline
  def moveShape: moveShape = "moveShape".asInstanceOf[moveShape]
  
  @js.native
  sealed trait multiple extends StObject
  @scala.inline
  def multiple: multiple = "multiple".asInstanceOf[multiple]
  
  @js.native
  sealed trait multipleDocuments extends StObject
  @scala.inline
  def multipleDocuments: multipleDocuments = "multipleDocuments".asInstanceOf[multipleDocuments]
  
  @js.native
  sealed trait native extends StObject
  @scala.inline
  def native: native = "native".asInstanceOf[native]
  
  @js.native
  sealed trait navPane extends StObject
  @scala.inline
  def navPane: navPane = "navPane".asInstanceOf[navPane]
  
  @js.native
  sealed trait nearestPoint extends StObject
  @scala.inline
  def nearestPoint: nearestPoint = "nearestPoint".asInstanceOf[nearestPoint]
  
  @js.native
  sealed trait never extends StObject
  @scala.inline
  def never: never = "never".asInstanceOf[never]
  
  @js.native
  sealed trait nextButton extends StObject
  @scala.inline
  def nextButton: nextButton = "nextButton".asInstanceOf[nextButton]
  
  @js.native
  sealed trait nextColumn extends StObject
  @scala.inline
  def nextColumn: nextColumn = "nextColumn".asInstanceOf[nextColumn]
  
  @js.native
  sealed trait none extends StObject
  @scala.inline
  def none: none = "none".asInstanceOf[none]
  
  @js.native
  sealed trait `none fit` extends StObject
  @scala.inline
  def `none fit`: `none fit` = ("none fit").asInstanceOf[`none fit`]
  
  @js.native
  sealed trait `none flip` extends StObject
  @scala.inline
  def `none flip`: `none flip` = ("none flip").asInstanceOf[`none flip`]
  
  @js.native
  sealed trait `none flipfit` extends StObject
  @scala.inline
  def `none flipfit`: `none flipfit` = ("none flipfit").asInstanceOf[`none flipfit`]
  
  @js.native
  sealed trait noneof extends StObject
  @scala.inline
  def noneof: noneof = "noneof".asInstanceOf[noneof]
  
  @js.native
  sealed trait normal extends StObject
  @scala.inline
  def normal: normal = "normal".asInstanceOf[normal]
  
  @js.native
  sealed trait notAnd extends StObject
  @scala.inline
  def notAnd: notAnd = "notAnd".asInstanceOf[notAnd]
  
  @js.native
  sealed trait notOr extends StObject
  @scala.inline
  def notOr: notOr = "notOr".asInstanceOf[notOr]
  
  @js.native
  sealed trait notcontains extends StObject
  @scala.inline
  def notcontains: notcontains = "notcontains".asInstanceOf[notcontains]
  
  @js.native
  sealed trait number extends StObject
  @scala.inline
  def number: number = "number".asInstanceOf[number]
  
  @js.native
  sealed trait numeric extends StObject
  @scala.inline
  def numeric: numeric = "numeric".asInstanceOf[numeric]
  
  @js.native
  sealed trait `object` extends StObject
  @scala.inline
  def `object`: `object` = "object".asInstanceOf[`object`]
  
  @js.native
  sealed trait occurrence extends StObject
  @scala.inline
  def occurrence: occurrence = "occurrence".asInstanceOf[occurrence]
  
  @js.native
  sealed trait octagon extends StObject
  @scala.inline
  def octagon: octagon = "octagon".asInstanceOf[octagon]
  
  @js.native
  sealed trait odata extends StObject
  @scala.inline
  def odata: odata = "odata".asInstanceOf[odata]
  
  @js.native
  sealed trait off extends StObject
  @scala.inline
  def off: off = "off".asInstanceOf[off]
  
  @js.native
  sealed trait ohlc extends StObject
  @scala.inline
  def ohlc: ohlc = "ohlc".asInstanceOf[ohlc]
  
  @js.native
  sealed trait onClick extends StObject
  @scala.inline
  def onClick: onClick = "onClick".asInstanceOf[onClick]
  
  @js.native
  sealed trait onDemand extends StObject
  @scala.inline
  def onDemand: onDemand = "onDemand".asInstanceOf[onDemand]
  
  @js.native
  sealed trait onFocus extends StObject
  @scala.inline
  def onFocus: onFocus = "onFocus".asInstanceOf[onFocus]
  
  @js.native
  sealed trait onHover extends StObject
  @scala.inline
  def onHover: onHover = "onHover".asInstanceOf[onHover]
  
  @js.native
  sealed trait onLongTap extends StObject
  @scala.inline
  def onLongTap: onLongTap = "onLongTap".asInstanceOf[onLongTap]
  
  @js.native
  sealed trait onMoving extends StObject
  @scala.inline
  def onMoving: onMoving = "onMoving".asInstanceOf[onMoving]
  
  @js.native
  sealed trait onMovingComplete extends StObject
  @scala.inline
  def onMovingComplete: onMovingComplete = "onMovingComplete".asInstanceOf[onMovingComplete]
  
  @js.native
  sealed trait onScroll extends StObject
  @scala.inline
  def onScroll: onScroll = "onScroll".asInstanceOf[onScroll]
  
  @js.native
  sealed trait onlyPoint extends StObject
  @scala.inline
  def onlyPoint: onlyPoint = "onlyPoint".asInstanceOf[onlyPoint]
  
  @js.native
  sealed trait open extends StObject
  @scala.inline
  def open: open = "open".asInstanceOf[open]
  
  @js.native
  sealed trait or extends StObject
  @scala.inline
  def or: or = "or".asInstanceOf[or]
  
  @js.native
  sealed trait orderedList extends StObject
  @scala.inline
  def orderedList: orderedList = "orderedList".asInstanceOf[orderedList]
  
  @js.native
  sealed trait orgChart extends StObject
  @scala.inline
  def orgChart: orgChart = "orgChart".asInstanceOf[orgChart]
  
  @js.native
  sealed trait orthogonal extends StObject
  @scala.inline
  def orthogonal: orthogonal = "orthogonal".asInstanceOf[orthogonal]
  
  @js.native
  sealed trait outlined extends StObject
  @scala.inline
  def outlined: outlined = "outlined".asInstanceOf[outlined]
  
  @js.native
  sealed trait outlinedTriangle extends StObject
  @scala.inline
  def outlinedTriangle: outlinedTriangle = "outlinedTriangle".asInstanceOf[outlinedTriangle]
  
  @js.native
  sealed trait outside extends StObject
  @scala.inline
  def outside: outside = "outside".asInstanceOf[outside]
  
  @js.native
  sealed trait overlap extends StObject
  @scala.inline
  def overlap: overlap = "overlap".asInstanceOf[overlap]
  
  @js.native
  sealed trait page extends StObject
  @scala.inline
  def page: page = "page".asInstanceOf[page]
  
  @js.native
  sealed trait pageColor extends StObject
  @scala.inline
  def pageColor: pageColor = "pageColor".asInstanceOf[pageColor]
  
  @js.native
  sealed trait pageOrientation extends StObject
  @scala.inline
  def pageOrientation: pageOrientation = "pageOrientation".asInstanceOf[pageOrientation]
  
  @js.native
  sealed trait pageSize extends StObject
  @scala.inline
  def pageSize: pageSize = "pageSize".asInstanceOf[pageSize]
  
  @js.native
  sealed trait pan extends StObject
  @scala.inline
  def pan: pan = "pan".asInstanceOf[pan]
  
  @js.native
  sealed trait paperTape extends StObject
  @scala.inline
  def paperTape: paperTape = "paperTape".asInstanceOf[paperTape]
  
  @js.native
  sealed trait password extends StObject
  @scala.inline
  def password: password = "password".asInstanceOf[password]
  
  @js.native
  sealed trait paste extends StObject
  @scala.inline
  def paste: paste = "paste".asInstanceOf[paste]
  
  @js.native
  sealed trait pattern extends StObject
  @scala.inline
  def pattern: pattern = "pattern".asInstanceOf[pattern]
  
  @js.native
  sealed trait pending extends StObject
  @scala.inline
  def pending: pending = "pending".asInstanceOf[pending]
  
  @js.native
  sealed trait pentagon extends StObject
  @scala.inline
  def pentagon: pentagon = "pentagon".asInstanceOf[pentagon]
  
  @js.native
  sealed trait percent extends _format
  @scala.inline
  def percent: percent = "percent".asInstanceOf[percent]
  
  @js.native
  sealed trait percentOfColumnGrandTotal extends StObject
  @scala.inline
  def percentOfColumnGrandTotal: percentOfColumnGrandTotal = "percentOfColumnGrandTotal".asInstanceOf[percentOfColumnGrandTotal]
  
  @js.native
  sealed trait percentOfColumnTotal extends StObject
  @scala.inline
  def percentOfColumnTotal: percentOfColumnTotal = "percentOfColumnTotal".asInstanceOf[percentOfColumnTotal]
  
  @js.native
  sealed trait percentOfGrandTotal extends StObject
  @scala.inline
  def percentOfGrandTotal: percentOfGrandTotal = "percentOfGrandTotal".asInstanceOf[percentOfGrandTotal]
  
  @js.native
  sealed trait percentOfRowGrandTotal extends StObject
  @scala.inline
  def percentOfRowGrandTotal: percentOfRowGrandTotal = "percentOfRowGrandTotal".asInstanceOf[percentOfRowGrandTotal]
  
  @js.native
  sealed trait percentOfRowTotal extends StObject
  @scala.inline
  def percentOfRowTotal: percentOfRowTotal = "percentOfRowTotal".asInstanceOf[percentOfRowTotal]
  
  @js.native
  sealed trait percentVariation extends StObject
  @scala.inline
  def percentVariation: percentVariation = "percentVariation".asInstanceOf[percentVariation]
  
  @js.native
  sealed trait phone extends StObject
  @scala.inline
  def phone: phone = "phone".asInstanceOf[phone]
  
  @js.native
  sealed trait pie extends StObject
  @scala.inline
  def pie: pie = "pie".asInstanceOf[pie]
  
  @js.native
  sealed trait plain extends StObject
  @scala.inline
  def plain: plain = "plain".asInstanceOf[plain]
  
  @js.native
  sealed trait png_ extends StObject
  @scala.inline
  def png_ : png_ = "png".asInstanceOf[png_]
  
  @js.native
  sealed trait polygon extends StObject
  @scala.inline
  def polygon: polygon = "polygon".asInstanceOf[polygon]
  
  @js.native
  sealed trait pop extends StObject
  @scala.inline
  def pop: pop = "pop".asInstanceOf[pop]
  
  @js.native
  sealed trait popup extends StObject
  @scala.inline
  def popup: popup = "popup".asInstanceOf[popup]
  
  @js.native
  sealed trait portrait extends StObject
  @scala.inline
  def portrait: portrait = "portrait".asInstanceOf[portrait]
  
  @js.native
  sealed trait predefinedProcess extends StObject
  @scala.inline
  def predefinedProcess: predefinedProcess = "predefinedProcess".asInstanceOf[predefinedProcess]
  
  @js.native
  sealed trait preparation extends StObject
  @scala.inline
  def preparation: preparation = "preparation".asInstanceOf[preparation]
  
  @js.native
  sealed trait process extends StObject
  @scala.inline
  def process: process = "process".asInstanceOf[process]
  
  @js.native
  sealed trait processed extends StObject
  @scala.inline
  def processed: processed = "processed".asInstanceOf[processed]
  
  @js.native
  sealed trait push extends StObject
  @scala.inline
  def push: push = "push".asInstanceOf[push]
  
  @js.native
  sealed trait px extends StObject
  @scala.inline
  def px: px = "px".asInstanceOf[px]
  
  @js.native
  sealed trait quarter
    extends _VizTimeInterval
       with _format
  @scala.inline
  def quarter: quarter = "quarter".asInstanceOf[quarter]
  
  @js.native
  sealed trait quarterAndYear extends _format
  @scala.inline
  def quarterAndYear: quarterAndYear = "quarterAndYear".asInstanceOf[quarterAndYear]
  
  @js.native
  sealed trait quarters extends StObject
  @scala.inline
  def quarters: quarters = "quarters".asInstanceOf[quarters]
  
  @js.native
  sealed trait range extends StObject
  @scala.inline
  def range: range = "range".asInstanceOf[range]
  
  @js.native
  sealed trait rangeBar extends StObject
  @scala.inline
  def rangeBar: rangeBar = "rangeBar".asInstanceOf[rangeBar]
  
  @js.native
  sealed trait rangearea extends StObject
  @scala.inline
  def rangearea: rangearea = "rangearea".asInstanceOf[rangearea]
  
  @js.native
  sealed trait rangebar_ extends StObject
  @scala.inline
  def rangebar_ : rangebar_ = "rangebar".asInstanceOf[rangebar_]
  
  @js.native
  sealed trait raw extends StObject
  @scala.inline
  def raw: raw = "raw".asInstanceOf[raw]
  
  @js.native
  sealed trait rectangle extends StObject
  @scala.inline
  def rectangle: rectangle = "rectangle".asInstanceOf[rectangle]
  
  @js.native
  sealed trait rectangleNeedle extends StObject
  @scala.inline
  def rectangleNeedle: rectangleNeedle = "rectangleNeedle".asInstanceOf[rectangleNeedle]
  
  @js.native
  sealed trait redo extends StObject
  @scala.inline
  def redo: redo = "redo".asInstanceOf[redo]
  
  @js.native
  sealed trait refresh extends StObject
  @scala.inline
  def refresh: refresh = "refresh".asInstanceOf[refresh]
  
  @js.native
  sealed trait rename extends StObject
  @scala.inline
  def rename: rename = "rename".asInstanceOf[rename]
  
  @js.native
  sealed trait repaint extends StObject
  @scala.inline
  def repaint: repaint = "repaint".asInstanceOf[repaint]
  
  @js.native
  sealed trait required extends StObject
  @scala.inline
  def required: required = "required".asInstanceOf[required]
  
  @js.native
  sealed trait reset extends StObject
  @scala.inline
  def reset: reset = "reset".asInstanceOf[reset]
  
  @js.native
  sealed trait reshape extends StObject
  @scala.inline
  def reshape: reshape = "reshape".asInstanceOf[reshape]
  
  @js.native
  sealed trait resizeShape extends StObject
  @scala.inline
  def resizeShape: resizeShape = "resizeShape".asInstanceOf[resizeShape]
  
  @js.native
  sealed trait rhombus extends StObject
  @scala.inline
  def rhombus: rhombus = "rhombus".asInstanceOf[rhombus]
  
  @js.native
  sealed trait right extends StObject
  @scala.inline
  def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait `right bottom` extends StObject
  @scala.inline
  def `right bottom`: `right bottom` = ("right bottom").asInstanceOf[`right bottom`]
  
  @js.native
  sealed trait `right top` extends StObject
  @scala.inline
  def `right top`: `right top` = ("right top").asInstanceOf[`right top`]
  
  @js.native
  sealed trait rightBottom extends StObject
  @scala.inline
  def rightBottom: rightBottom = "rightBottom".asInstanceOf[rightBottom]
  
  @js.native
  sealed trait rightBottomLeftTop extends StObject
  @scala.inline
  def rightBottomLeftTop: rightBottomLeftTop = "rightBottomLeftTop".asInstanceOf[rightBottomLeftTop]
  
  @js.native
  sealed trait rightCenter extends StObject
  @scala.inline
  def rightCenter: rightCenter = "rightCenter".asInstanceOf[rightCenter]
  
  @js.native
  sealed trait rightOrBottom extends StObject
  @scala.inline
  def rightOrBottom: rightOrBottom = "rightOrBottom".asInstanceOf[rightOrBottom]
  
  @js.native
  sealed trait rightTop extends StObject
  @scala.inline
  def rightTop: rightTop = "rightTop".asInstanceOf[rightTop]
  
  @js.native
  sealed trait rightTopLeftBottom extends StObject
  @scala.inline
  def rightTopLeftBottom: rightTopLeftBottom = "rightTopLeftBottom".asInstanceOf[rightTopLeftBottom]
  
  @js.native
  sealed trait roadmap extends StObject
  @scala.inline
  def roadmap: roadmap = "roadmap".asInstanceOf[roadmap]
  
  @js.native
  sealed trait rollers extends StObject
  @scala.inline
  def rollers: rollers = "rollers".asInstanceOf[rollers]
  
  @js.native
  sealed trait rotate extends StObject
  @scala.inline
  def rotate: rotate = "rotate".asInstanceOf[rotate]
  
  @js.native
  sealed trait row extends StObject
  @scala.inline
  def row: row = "row".asInstanceOf[row]
  
  @js.native
  sealed trait rowClick extends StObject
  @scala.inline
  def rowClick: rowClick = "rowClick".asInstanceOf[rowClick]
  
  @js.native
  sealed trait rows extends StObject
  @scala.inline
  def rows: rows = "rows".asInstanceOf[rows]
  
  @js.native
  sealed trait satellite extends StObject
  @scala.inline
  def satellite: satellite = "satellite".asInstanceOf[satellite]
  
  @js.native
  sealed trait save extends StObject
  @scala.inline
  def save: save = "save".asInstanceOf[save]
  
  @js.native
  sealed trait scatter extends StObject
  @scala.inline
  def scatter: scatter = "scatter".asInstanceOf[scatter]
  
  @js.native
  sealed trait script extends StObject
  @scala.inline
  def script: script = "script".asInstanceOf[script]
  
  @js.native
  sealed trait scrollBottom extends StObject
  @scala.inline
  def scrollBottom: scrollBottom = "scrollBottom".asInstanceOf[scrollBottom]
  
  @js.native
  sealed trait search extends StObject
  @scala.inline
  def search: search = "search".asInstanceOf[search]
  
  @js.native
  sealed trait second
    extends _VizTimeInterval
       with _format
  @scala.inline
  def second: second = "second".asInstanceOf[second]
  
  @js.native
  sealed trait select extends StObject
  @scala.inline
  def select: select = "select".asInstanceOf[select]
  
  @js.native
  sealed trait selectAll extends StObject
  @scala.inline
  def selectAll: selectAll = "selectAll".asInstanceOf[selectAll]
  
  @js.native
  sealed trait selected extends StObject
  @scala.inline
  def selected: selected = "selected".asInstanceOf[selected]
  
  @js.native
  sealed trait selection extends StObject
  @scala.inline
  def selection: selection = "selection".asInstanceOf[selection]
  
  @js.native
  sealed trait semidiscrete extends StObject
  @scala.inline
  def semidiscrete: semidiscrete = "semidiscrete".asInstanceOf[semidiscrete]
  
  @js.native
  sealed trait sendToBack extends StObject
  @scala.inline
  def sendToBack: sendToBack = "sendToBack".asInstanceOf[sendToBack]
  
  @js.native
  sealed trait separator extends StObject
  @scala.inline
  def separator: separator = "separator".asInstanceOf[separator]
  
  @js.native
  sealed trait series extends StObject
  @scala.inline
  def series: series = "series".asInstanceOf[series]
  
  @js.native
  sealed trait sessionStorage extends StObject
  @scala.inline
  def sessionStorage: sessionStorage = "sessionStorage".asInstanceOf[sessionStorage]
  
  @js.native
  sealed trait shape extends StObject
  @scala.inline
  def shape: shape = "shape".asInstanceOf[shape]
  
  @js.native
  sealed trait shift extends StObject
  @scala.inline
  def shift: shift = "shift".asInstanceOf[shift]
  
  @js.native
  sealed trait shortDate extends _format
  @scala.inline
  def shortDate: shortDate = "shortDate".asInstanceOf[shortDate]
  
  @js.native
  sealed trait shortDateShortTime extends _format
  @scala.inline
  def shortDateShortTime: shortDateShortTime = "shortDateShortTime".asInstanceOf[shortDateShortTime]
  
  @js.native
  sealed trait shortTime extends _format
  @scala.inline
  def shortTime: shortTime = "shortTime".asInstanceOf[shortTime]
  
  @js.native
  sealed trait showGrid extends StObject
  @scala.inline
  def showGrid: showGrid = "showGrid".asInstanceOf[showGrid]
  
  @js.native
  sealed trait showNavPane extends StObject
  @scala.inline
  def showNavPane: showNavPane = "showNavPane".asInstanceOf[showNavPane]
  
  @js.native
  sealed trait shrink extends StObject
  @scala.inline
  def shrink: shrink = "shrink".asInstanceOf[shrink]
  
  @js.native
  sealed trait simple extends StObject
  @scala.inline
  def simple: simple = "simple".asInstanceOf[simple]
  
  @js.native
  sealed trait simpleSet extends StObject
  @scala.inline
  def simpleSet: simpleSet = "simpleSet".asInstanceOf[simpleSet]
  
  @js.native
  sealed trait singleAccounting extends StObject
  @scala.inline
  def singleAccounting: singleAccounting = "singleAccounting".asInstanceOf[singleAccounting]
  
  @js.native
  sealed trait single_ extends StObject
  @scala.inline
  def single_ : single_ = "single".asInstanceOf[single_]
  
  @js.native
  sealed trait size extends StObject
  @scala.inline
  def size: size = "size".asInstanceOf[size]
  
  @js.native
  sealed trait sliceanddice extends StObject
  @scala.inline
  def sliceanddice: sliceanddice = "sliceanddice".asInstanceOf[sliceanddice]
  
  @js.native
  sealed trait slide extends StObject
  @scala.inline
  def slide: slide = "slide".asInstanceOf[slide]
  
  @js.native
  sealed trait slideButton extends StObject
  @scala.inline
  def slideButton: slideButton = "slideButton".asInstanceOf[slideButton]
  
  @js.native
  sealed trait slideIn extends StObject
  @scala.inline
  def slideIn: slideIn = "slideIn".asInstanceOf[slideIn]
  
  @js.native
  sealed trait slideItem extends StObject
  @scala.inline
  def slideItem: slideItem = "slideItem".asInstanceOf[slideItem]
  
  @js.native
  sealed trait slideOut extends StObject
  @scala.inline
  def slideOut: slideOut = "slideOut".asInstanceOf[slideOut]
  
  @js.native
  sealed trait smallValueThreshold extends StObject
  @scala.inline
  def smallValueThreshold: smallValueThreshold = "smallValueThreshold".asInstanceOf[smallValueThreshold]
  
  @js.native
  sealed trait snapToGrid extends StObject
  @scala.inline
  def snapToGrid: snapToGrid = "snapToGrid".asInstanceOf[snapToGrid]
  
  @js.native
  sealed trait solid extends StObject
  @scala.inline
  def solid: solid = "solid".asInstanceOf[solid]
  
  @js.native
  sealed trait source extends StObject
  @scala.inline
  def source: source = "source".asInstanceOf[source]
  
  @js.native
  sealed trait `space-around` extends StObject
  @scala.inline
  def `space-around`: `space-around` = "space-around".asInstanceOf[`space-around`]
  
  @js.native
  sealed trait `space-between` extends StObject
  @scala.inline
  def `space-between`: `space-between` = "space-between".asInstanceOf[`space-between`]
  
  @js.native
  sealed trait spins extends StObject
  @scala.inline
  def spins: spins = "spins".asInstanceOf[spins]
  
  @js.native
  sealed trait spline extends StObject
  @scala.inline
  def spline: spline = "spline".asInstanceOf[spline]
  
  @js.native
  sealed trait splinearea extends StObject
  @scala.inline
  def splinearea: splinearea = "splinearea".asInstanceOf[splinearea]
  
  @js.native
  sealed trait square extends StObject
  @scala.inline
  def square: square = "square".asInstanceOf[square]
  
  @js.native
  sealed trait squarified extends StObject
  @scala.inline
  def squarified: squarified = "squarified".asInstanceOf[squarified]
  
  @js.native
  sealed trait stack extends StObject
  @scala.inline
  def stack: stack = "stack".asInstanceOf[stack]
  
  @js.native
  sealed trait stackedarea extends StObject
  @scala.inline
  def stackedarea: stackedarea = "stackedarea".asInstanceOf[stackedarea]
  
  @js.native
  sealed trait stackedbar extends StObject
  @scala.inline
  def stackedbar: stackedbar = "stackedbar".asInstanceOf[stackedbar]
  
  @js.native
  sealed trait stackedline extends StObject
  @scala.inline
  def stackedline: stackedline = "stackedline".asInstanceOf[stackedline]
  
  @js.native
  sealed trait stackedspline extends StObject
  @scala.inline
  def stackedspline: stackedspline = "stackedspline".asInstanceOf[stackedspline]
  
  @js.native
  sealed trait stackedsplinearea extends StObject
  @scala.inline
  def stackedsplinearea: stackedsplinearea = "stackedsplinearea".asInstanceOf[stackedsplinearea]
  
  @js.native
  sealed trait stagger extends StObject
  @scala.inline
  def stagger: stagger = "stagger".asInstanceOf[stagger]
  
  @js.native
  sealed trait standard extends StObject
  @scala.inline
  def standard: standard = "standard".asInstanceOf[standard]
  
  @js.native
  sealed trait star extends StObject
  @scala.inline
  def star: star = "star".asInstanceOf[star]
  
  @js.native
  sealed trait start extends StObject
  @scala.inline
  def start: start = "start".asInstanceOf[start]
  
  @js.native
  sealed trait startEdit extends StObject
  @scala.inline
  def startEdit: startEdit = "startEdit".asInstanceOf[startEdit]
  
  @js.native
  sealed trait startswith extends StObject
  @scala.inline
  def startswith: startswith = "startswith".asInstanceOf[startswith]
  
  @js.native
  sealed trait static extends StObject
  @scala.inline
  def static: static = "static".asInstanceOf[static]
  
  @js.native
  sealed trait stdDeviation extends StObject
  @scala.inline
  def stdDeviation: stdDeviation = "stdDeviation".asInstanceOf[stdDeviation]
  
  @js.native
  sealed trait stdError extends StObject
  @scala.inline
  def stdError: stdError = "stdError".asInstanceOf[stdError]
  
  @js.native
  sealed trait steparea extends StObject
  @scala.inline
  def steparea: steparea = "steparea".asInstanceOf[steparea]
  
  @js.native
  sealed trait stepline extends StObject
  @scala.inline
  def stepline: stepline = "stepline".asInstanceOf[stepline]
  
  @js.native
  sealed trait stock extends StObject
  @scala.inline
  def stock: stock = "stock".asInstanceOf[stock]
  
  @js.native
  sealed trait storedData extends StObject
  @scala.inline
  def storedData: storedData = "storedData".asInstanceOf[storedData]
  
  @js.native
  sealed trait straight extends StObject
  @scala.inline
  def straight: straight = "straight".asInstanceOf[straight]
  
  @js.native
  sealed trait stretch extends StObject
  @scala.inline
  def stretch: stretch = "stretch".asInstanceOf[stretch]
  
  @js.native
  sealed trait strike extends StObject
  @scala.inline
  def strike: strike = "strike".asInstanceOf[strike]
  
  @js.native
  sealed trait stringLength extends StObject
  @scala.inline
  def stringLength: stringLength = "stringLength".asInstanceOf[stringLength]
  
  @js.native
  sealed trait string_ extends StObject
  @scala.inline
  def string_ : string_ = "string".asInstanceOf[string_]
  
  @js.native
  sealed trait strip extends StObject
  @scala.inline
  def strip: strip = "strip".asInstanceOf[strip]
  
  @js.native
  sealed trait subscript extends StObject
  @scala.inline
  def subscript: subscript = "subscript".asInstanceOf[subscript]
  
  @js.native
  sealed trait success extends StObject
  @scala.inline
  def success: success = "success".asInstanceOf[success]
  
  @js.native
  sealed trait sum extends StObject
  @scala.inline
  def sum: sum = "sum".asInstanceOf[sum]
  
  @js.native
  sealed trait summingJunction extends StObject
  @scala.inline
  def summingJunction: summingJunction = "summingJunction".asInstanceOf[summingJunction]
  
  @js.native
  sealed trait superscript extends StObject
  @scala.inline
  def superscript: superscript = "superscript".asInstanceOf[superscript]
  
  @js.native
  sealed trait svg_ extends StObject
  @scala.inline
  def svg_ : svg_ = "svg".asInstanceOf[svg_]
  
  @js.native
  sealed trait swipe extends StObject
  @scala.inline
  def swipe: swipe = "swipe".asInstanceOf[swipe]
  
  @js.native
  sealed trait switchView extends StObject
  @scala.inline
  def switchView: switchView = "switchView".asInstanceOf[switchView]
  
  @js.native
  sealed trait tabbed extends StObject
  @scala.inline
  def tabbed: tabbed = "tabbed".asInstanceOf[tabbed]
  
  @js.native
  sealed trait tablet extends StObject
  @scala.inline
  def tablet: tablet = "tablet".asInstanceOf[tablet]
  
  @js.native
  sealed trait target extends StObject
  @scala.inline
  def target: target = "target".asInstanceOf[target]
  
  @js.native
  sealed trait taskDetails extends StObject
  @scala.inline
  def taskDetails: taskDetails = "taskDetails".asInstanceOf[taskDetails]
  
  @js.native
  sealed trait tel extends StObject
  @scala.inline
  def tel: tel = "tel".asInstanceOf[tel]
  
  @js.native
  sealed trait terminator extends StObject
  @scala.inline
  def terminator: terminator = "terminator".asInstanceOf[terminator]
  
  @js.native
  sealed trait text extends StObject
  @scala.inline
  def text: text = "text".asInstanceOf[text]
  
  @js.native
  sealed trait textAlignCenter extends StObject
  @scala.inline
  def textAlignCenter: textAlignCenter = "textAlignCenter".asInstanceOf[textAlignCenter]
  
  @js.native
  sealed trait textAlignLeft extends StObject
  @scala.inline
  def textAlignLeft: textAlignLeft = "textAlignLeft".asInstanceOf[textAlignLeft]
  
  @js.native
  sealed trait textAlignRight extends StObject
  @scala.inline
  def textAlignRight: textAlignRight = "textAlignRight".asInstanceOf[textAlignRight]
  
  @js.native
  sealed trait textCloud extends StObject
  @scala.inline
  def textCloud: textCloud = "textCloud".asInstanceOf[textCloud]
  
  @js.native
  sealed trait texts extends StObject
  @scala.inline
  def texts: texts = "texts".asInstanceOf[texts]
  
  @js.native
  sealed trait thousands extends _format
  @scala.inline
  def thousands: thousands = "thousands".asInstanceOf[thousands]
  
  @js.native
  sealed trait thumbnails extends StObject
  @scala.inline
  def thumbnails: thumbnails = "thumbnails".asInstanceOf[thumbnails]
  
  @js.native
  sealed trait time extends StObject
  @scala.inline
  def time: time = "time".asInstanceOf[time]
  
  @js.native
  sealed trait timelineDay extends StObject
  @scala.inline
  def timelineDay: timelineDay = "timelineDay".asInstanceOf[timelineDay]
  
  @js.native
  sealed trait timelineMonth extends StObject
  @scala.inline
  def timelineMonth: timelineMonth = "timelineMonth".asInstanceOf[timelineMonth]
  
  @js.native
  sealed trait timelineWeek extends StObject
  @scala.inline
  def timelineWeek: timelineWeek = "timelineWeek".asInstanceOf[timelineWeek]
  
  @js.native
  sealed trait timelineWorkWeek extends StObject
  @scala.inline
  def timelineWorkWeek: timelineWorkWeek = "timelineWorkWeek".asInstanceOf[timelineWorkWeek]
  
  @js.native
  sealed trait toggle extends StObject
  @scala.inline
  def toggle: toggle = "toggle".asInstanceOf[toggle]
  
  @js.native
  sealed trait top extends StObject
  @scala.inline
  def top: top = "top".asInstanceOf[top]
  
  @js.native
  sealed trait topN extends StObject
  @scala.inline
  def topN: topN = "topN".asInstanceOf[topN]
  
  @js.native
  sealed trait tree extends StObject
  @scala.inline
  def tree: tree = "tree".asInstanceOf[tree]
  
  @js.native
  sealed trait triangle extends StObject
  @scala.inline
  def triangle: triangle = "triangle".asInstanceOf[triangle]
  
  @js.native
  sealed trait triangleDown extends StObject
  @scala.inline
  def triangleDown: triangleDown = "triangleDown".asInstanceOf[triangleDown]
  
  @js.native
  sealed trait triangleMarker extends StObject
  @scala.inline
  def triangleMarker: triangleMarker = "triangleMarker".asInstanceOf[triangleMarker]
  
  @js.native
  sealed trait triangleNeedle extends StObject
  @scala.inline
  def triangleNeedle: triangleNeedle = "triangleNeedle".asInstanceOf[triangleNeedle]
  
  @js.native
  sealed trait triangleUp extends StObject
  @scala.inline
  def triangleUp: triangleUp = "triangleUp".asInstanceOf[triangleUp]
  
  @js.native
  sealed trait trillions extends _format
  @scala.inline
  def trillions: trillions = "trillions".asInstanceOf[trillions]
  
  @js.native
  sealed trait twoColorNeedle extends StObject
  @scala.inline
  def twoColorNeedle: twoColorNeedle = "twoColorNeedle".asInstanceOf[twoColorNeedle]
  
  @js.native
  sealed trait undelete extends StObject
  @scala.inline
  def undelete: undelete = "undelete".asInstanceOf[undelete]
  
  @js.native
  sealed trait underline extends StObject
  @scala.inline
  def underline: underline = "underline".asInstanceOf[underline]
  
  @js.native
  sealed trait underlined extends StObject
  @scala.inline
  def underlined: underlined = "underlined".asInstanceOf[underlined]
  
  @js.native
  sealed trait undo extends StObject
  @scala.inline
  def undo: undo = "undo".asInstanceOf[undo]
  
  @js.native
  sealed trait units extends StObject
  @scala.inline
  def units: units = "units".asInstanceOf[units]
  
  @js.native
  sealed trait unlimited extends StObject
  @scala.inline
  def unlimited: unlimited = "unlimited".asInstanceOf[unlimited]
  
  @js.native
  sealed trait unlock extends StObject
  @scala.inline
  def unlock: unlock = "unlock".asInstanceOf[unlock]
  
  @js.native
  sealed trait up extends StObject
  @scala.inline
  def up: up = "up".asInstanceOf[up]
  
  @js.native
  sealed trait upload extends StObject
  @scala.inline
  def upload: upload = "upload".asInstanceOf[upload]
  
  @js.native
  sealed trait url extends StObject
  @scala.inline
  def url: url = "url".asInstanceOf[url]
  
  @js.native
  sealed trait useButtons extends StObject
  @scala.inline
  def useButtons: useButtons = "useButtons".asInstanceOf[useButtons]
  
  @js.native
  sealed trait useForm extends StObject
  @scala.inline
  def useForm: useForm = "useForm".asInstanceOf[useForm]
  
  @js.native
  sealed trait valid extends StObject
  @scala.inline
  def valid: valid = "valid".asInstanceOf[valid]
  
  @js.native
  sealed trait value extends StObject
  @scala.inline
  def value: value = "value".asInstanceOf[value]
  
  @js.native
  sealed trait variable extends StObject
  @scala.inline
  def variable: variable = "variable".asInstanceOf[variable]
  
  @js.native
  sealed trait variance extends StObject
  @scala.inline
  def variance: variance = "variance".asInstanceOf[variance]
  
  @js.native
  sealed trait vertical extends StObject
  @scala.inline
  def vertical: vertical = "vertical".asInstanceOf[vertical]
  
  @js.native
  sealed trait verticalContainer extends StObject
  @scala.inline
  def verticalContainer: verticalContainer = "verticalContainer".asInstanceOf[verticalContainer]
  
  @js.native
  sealed trait virtual extends StObject
  @scala.inline
  def virtual: virtual = "virtual".asInstanceOf[virtual]
  
  @js.native
  sealed trait visible extends StObject
  @scala.inline
  def visible: visible = "visible".asInstanceOf[visible]
  
  @js.native
  sealed trait walking extends StObject
  @scala.inline
  def walking: walking = "walking".asInstanceOf[walking]
  
  @js.native
  sealed trait warning extends StObject
  @scala.inline
  def warning: warning = "warning".asInstanceOf[warning]
  
  @js.native
  sealed trait waved extends StObject
  @scala.inline
  def waved: waved = "waved".asInstanceOf[waved]
  
  @js.native
  sealed trait week extends _VizTimeInterval
  @scala.inline
  def week: week = "week".asInstanceOf[week]
  
  @js.native
  sealed trait weeks extends StObject
  @scala.inline
  def weeks: weeks = "weeks".asInstanceOf[weeks]
  
  @js.native
  sealed trait widget extends StObject
  @scala.inline
  def widget: widget = "widget".asInstanceOf[widget]
  
  @js.native
  sealed trait winloss extends StObject
  @scala.inline
  def winloss: winloss = "winloss".asInstanceOf[winloss]
  
  @js.native
  sealed trait withAncestors extends StObject
  @scala.inline
  def withAncestors: withAncestors = "withAncestors".asInstanceOf[withAncestors]
  
  @js.native
  sealed trait workWeek extends StObject
  @scala.inline
  def workWeek: workWeek = "workWeek".asInstanceOf[workWeek]
  
  @js.native
  sealed trait xmla extends StObject
  @scala.inline
  def xmla: xmla = "xmla".asInstanceOf[xmla]
  
  @js.native
  sealed trait year
    extends _VizTimeInterval
       with _format
  @scala.inline
  def year: year = "year".asInstanceOf[year]
  
  @js.native
  sealed trait years extends StObject
  @scala.inline
  def years: years = "years".asInstanceOf[years]
  
  @js.native
  sealed trait zoom extends StObject
  @scala.inline
  def zoom: zoom = "zoom".asInstanceOf[zoom]
  
  @js.native
  sealed trait zoomIn extends StObject
  @scala.inline
  def zoomIn: zoomIn = "zoomIn".asInstanceOf[zoomIn]
  
  @js.native
  sealed trait zoomLevel extends StObject
  @scala.inline
  def zoomLevel: zoomLevel = "zoomLevel".asInstanceOf[zoomLevel]
  
  @js.native
  sealed trait zoomOut extends StObject
  @scala.inline
  def zoomOut: zoomOut = "zoomOut".asInstanceOf[zoomOut]
}
