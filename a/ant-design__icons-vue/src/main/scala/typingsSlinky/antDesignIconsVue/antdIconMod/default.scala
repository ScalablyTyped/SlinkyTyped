package typingsSlinky.antDesignIconsVue.antdIconMod

import typingsSlinky.antDesignIconsVue.twoTonePrimaryColorMod.TwoToneColor
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("@ant-design/icons-vue/es/components/AntdIcon", JSImport.Default)
@js.native
object default extends js.Object {
  
  def apply(
    props: IconComponentProps,
    context: /* import warning: transforms.QualifyReferences#resolveTypeRef many Couldn't qualify SetupContext */ js.Any
  ): js.Any = js.native
  
  var displayName: String = js.native
  
  def getTwoToneColor(): TwoToneColor = js.native
  @JSName("getTwoToneColor")
  var getTwoToneColor_Original: js.Function0[TwoToneColor] = js.native
  
  var inheritAttrs: Boolean = js.native
  
  def setTwoToneColor(twoToneColor: TwoToneColor): Unit = js.native
  @JSName("setTwoToneColor")
  var setTwoToneColor_Original: js.Function1[/* twoToneColor */ TwoToneColor, Unit] = js.native
}
