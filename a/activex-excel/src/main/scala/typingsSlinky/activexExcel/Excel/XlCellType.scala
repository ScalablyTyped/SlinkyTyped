package typingsSlinky.activexExcel.Excel

import typingsSlinky.activexExcel.activexExcelNumbers.`-4123`
import typingsSlinky.activexExcel.activexExcelNumbers.`-4144`
import typingsSlinky.activexExcel.activexExcelNumbers.`-4172`
import typingsSlinky.activexExcel.activexExcelNumbers.`-4173`
import typingsSlinky.activexExcel.activexExcelNumbers.`-4174`
import typingsSlinky.activexExcel.activexExcelNumbers.`-4175`
import typingsSlinky.activexExcel.activexExcelNumbers.`11`
import typingsSlinky.activexExcel.activexExcelNumbers.`12`
import typingsSlinky.activexExcel.activexExcelNumbers.`2`
import typingsSlinky.activexExcel.activexExcelNumbers.`4`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typingsSlinky.activexExcel.activexExcelNumbers.`-4172`
  - typingsSlinky.activexExcel.activexExcelNumbers.`-4174`
  - typingsSlinky.activexExcel.activexExcelNumbers.`4`
  - typingsSlinky.activexExcel.activexExcelNumbers.`-4144`
  - typingsSlinky.activexExcel.activexExcelNumbers.`2`
  - typingsSlinky.activexExcel.activexExcelNumbers.`-4123`
  - typingsSlinky.activexExcel.activexExcelNumbers.`11`
  - typingsSlinky.activexExcel.activexExcelNumbers.`-4173`
  - typingsSlinky.activexExcel.activexExcelNumbers.`-4175`
  - typingsSlinky.activexExcel.activexExcelNumbers.`12`
*/
trait XlCellType extends StObject
object XlCellType {
  
  @scala.inline
  def xlCellTypeAllFormatConditions: `-4172` = -4172.asInstanceOf[`-4172`]
  
  @scala.inline
  def xlCellTypeAllValidation: `-4174` = -4174.asInstanceOf[`-4174`]
  
  @scala.inline
  def xlCellTypeBlanks: `4` = 4.asInstanceOf[`4`]
  
  @scala.inline
  def xlCellTypeComments: `-4144` = -4144.asInstanceOf[`-4144`]
  
  @scala.inline
  def xlCellTypeConstants: `2` = 2.asInstanceOf[`2`]
  
  @scala.inline
  def xlCellTypeFormulas: `-4123` = -4123.asInstanceOf[`-4123`]
  
  @scala.inline
  def xlCellTypeLastCell: `11` = 11.asInstanceOf[`11`]
  
  @scala.inline
  def xlCellTypeSameFormatConditions: `-4173` = -4173.asInstanceOf[`-4173`]
  
  @scala.inline
  def xlCellTypeSameValidation: `-4175` = -4175.asInstanceOf[`-4175`]
  
  @scala.inline
  def xlCellTypeVisible: `12` = 12.asInstanceOf[`12`]
}
