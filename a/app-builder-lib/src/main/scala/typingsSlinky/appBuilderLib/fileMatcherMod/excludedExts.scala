package typingsSlinky.appBuilderLib.fileMatcherMod

import org.scalablytyped.runtime.TopLevel
import typingsSlinky.appBuilderLib.appBuilderLibStrings.imlCommahprofCommaorigCommapycCommapyoCommarbcCommaswpCommacsprojCommaslnCommasuoCommaxprojCommaccCommadDotts
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("app-builder-lib/out/fileMatcher", "excludedExts")
@js.native
object excludedExts extends TopLevel[
      imlCommahprofCommaorigCommapycCommapyoCommarbcCommaswpCommacsprojCommaslnCommasuoCommaxprojCommaccCommadDotts
    ]

