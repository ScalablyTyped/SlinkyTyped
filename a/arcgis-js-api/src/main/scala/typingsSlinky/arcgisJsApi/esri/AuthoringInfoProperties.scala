...
object AuthoringInfoProperties {
  ...
  def apply(
    ...
    `type`: `class-breaks-size` | `class-breaks-color` | `classed-color` | `classed-size` | typingsSlinky.arcgisJsApi.arcgisJsApiStrings.predominance | typingsSlinky.arcgisJsApi.arcgisJsApiStrings.relationship | `univariate-color-size` = null,
