package typingsSlinky.arcgisDashJsDashApi.esriWidgetsSwipeMod

import typingsSlinky.arcgisDashJsDashApi.__esri.Swipe
import typingsSlinky.arcgisDashJsDashApi.__esri.SwipeProperties
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("esri/widgets/Swipe", JSImport.Namespace)
@js.native
/**
  * The Swipe widget provides a tool to show a portion of a layer or layers on top of a map. Layers can be swiped vertically or horizontally to easily compare two layers or see what is underneath a layer.
  *
  * [Read more...](https://developers.arcgis.com/javascript/latest/api-reference/esri-widgets-Swipe.html)
  */
class Class () extends Swipe {
  def this(properties: SwipeProperties) = this()
}

