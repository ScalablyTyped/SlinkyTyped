...
object XHalfFloatReadOnlyBitmap {
  ...
  implicit class XHalfFloatReadOnlyBitmapOps[Self <: XHalfFloatReadOnlyBitmap] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withGetData(value: IntegerRectangle2D => SafeArray[Double]): Self = {
        val ret = this.duplicate
    ...
    @scala.inline
    def withGetPixel(value: IntegerPoint2D => SafeArray[Double]): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("getPixel")(js.Any.fromFunction1(value))
        ret
    }
  }
