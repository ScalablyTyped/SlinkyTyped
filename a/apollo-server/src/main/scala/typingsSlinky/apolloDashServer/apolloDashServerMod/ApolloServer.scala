package typingsSlinky.apolloDashServer.apolloDashServerMod

import typingsSlinky.apolloDashServer.Anon_Cors
import typingsSlinky.apolloDashServerDashExpress.distApolloServerMod.ApolloServerExpressConfig
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("apollo-server", "ApolloServer")
@js.native
class ApolloServer protected ()
  extends typingsSlinky.apolloDashServerDashExpress.apolloDashServerDashExpressMod.ApolloServer {
  def this(config: ApolloServerExpressConfig with Anon_Cors) = this()
  var cors: js.UndefOr[js.Any] = js.native
  var createServerInfo: js.Any = js.native
  var httpServer: js.UndefOr[js.Any] = js.native
  var onHealthCheck: js.UndefOr[js.Any] = js.native
  def applyMiddleware(): Unit = js.native
  def listen(opts: js.Any*): js.Promise[ServerInfo] = js.native
}

