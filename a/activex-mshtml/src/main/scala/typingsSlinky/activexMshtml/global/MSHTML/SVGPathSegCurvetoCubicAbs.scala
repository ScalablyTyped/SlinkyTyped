package typingsSlinky.activexMshtml.global.MSHTML

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("MSHTML.SVGPathSegCurvetoCubicAbs")
@js.native
class SVGPathSegCurvetoCubicAbs protected ()
  extends typingsSlinky.activexMshtml.MSHTML.SVGPathSegCurvetoCubicAbs {
  /* CompleteClass */
  @JSName("MSHTML.SVGPathSegCurvetoCubicAbs_typekey")
  override var MSHTMLDotSVGPathSegCurvetoCubicAbs_typekey: typingsSlinky.activexMshtml.MSHTML.SVGPathSegCurvetoCubicAbs = js.native
  /* CompleteClass */
  override var pathSegType: Double = js.native
  /* CompleteClass */
  override val pathSegTypeAsLetter: String = js.native
  /* CompleteClass */
  override var x: Double = js.native
  /* CompleteClass */
  override var x1: Double = js.native
  /* CompleteClass */
  override var x2: Double = js.native
  /* CompleteClass */
  override var y: Double = js.native
  /* CompleteClass */
  override var y1: Double = js.native
  /* CompleteClass */
  override var y2: Double = js.native
}

