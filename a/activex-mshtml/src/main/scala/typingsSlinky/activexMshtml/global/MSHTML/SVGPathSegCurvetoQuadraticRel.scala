package typingsSlinky.activexMshtml.global.MSHTML

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("MSHTML.SVGPathSegCurvetoQuadraticRel")
@js.native
class SVGPathSegCurvetoQuadraticRel protected ()
  extends typingsSlinky.activexMshtml.MSHTML.SVGPathSegCurvetoQuadraticRel {
  /* CompleteClass */
  @JSName("MSHTML.SVGPathSegCurvetoQuadraticRel_typekey")
  override var MSHTMLDotSVGPathSegCurvetoQuadraticRel_typekey: typingsSlinky.activexMshtml.MSHTML.SVGPathSegCurvetoQuadraticRel = js.native
  /* CompleteClass */
  override var pathSegType: Double = js.native
  /* CompleteClass */
  override val pathSegTypeAsLetter: String = js.native
  /* CompleteClass */
  override var x: Double = js.native
  /* CompleteClass */
  override var x1: Double = js.native
  /* CompleteClass */
  override var y: Double = js.native
  /* CompleteClass */
  override var y1: Double = js.native
}

