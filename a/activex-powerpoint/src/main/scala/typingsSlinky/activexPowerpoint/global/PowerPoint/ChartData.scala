package typingsSlinky.activexPowerpoint.global.PowerPoint

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("PowerPoint.ChartData")
@js.native
class ChartData protected ()
  extends typingsSlinky.activexPowerpoint.PowerPoint.ChartData {
  /* CompleteClass */
  override val IsLinked: Boolean = js.native
  /* CompleteClass */
  @JSName("PowerPoint.ChartData_typekey")
  override var PowerPointDotChartData_typekey: typingsSlinky.activexPowerpoint.PowerPoint.ChartData = js.native
  /* CompleteClass */
  override val Workbook: js.Any = js.native
  /* CompleteClass */
  override def Activate(): Unit = js.native
  /* CompleteClass */
  override def BreakLink(): Unit = js.native
}

