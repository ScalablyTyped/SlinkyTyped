package typingsSlinky.awsSdkClientS3Node

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object awsSdkClientS3NodeStrings {
  
  @js.native
  sealed trait AES256 extends StObject
  @scala.inline
  def AES256: AES256 = "AES256".asInstanceOf[AES256]
  
  @js.native
  sealed trait ALWAYS extends StObject
  @scala.inline
  def ALWAYS: ALWAYS = "ALWAYS".asInstanceOf[ALWAYS]
  
  @js.native
  sealed trait ASNEEDED extends StObject
  @scala.inline
  def ASNEEDED: ASNEEDED = "ASNEEDED".asInstanceOf[ASNEEDED]
  
  @js.native
  sealed trait All extends StObject
  @scala.inline
  def All: All = "All".asInstanceOf[All]
  
  @js.native
  sealed trait AmazonCustomerByEmail extends StObject
  @scala.inline
  def AmazonCustomerByEmail: AmazonCustomerByEmail = "AmazonCustomerByEmail".asInstanceOf[AmazonCustomerByEmail]
  
  @js.native
  sealed trait BZIP2 extends StObject
  @scala.inline
  def BZIP2: BZIP2 = "BZIP2".asInstanceOf[BZIP2]
  
  @js.native
  sealed trait BucketAlreadyExists extends StObject
  @scala.inline
  def BucketAlreadyExists: BucketAlreadyExists = "BucketAlreadyExists".asInstanceOf[BucketAlreadyExists]
  
  @js.native
  sealed trait BucketAlreadyOwnedByYou extends StObject
  @scala.inline
  def BucketAlreadyOwnedByYou: BucketAlreadyOwnedByYou = "BucketAlreadyOwnedByYou".asInstanceOf[BucketAlreadyOwnedByYou]
  
  @js.native
  sealed trait BucketOwner extends StObject
  @scala.inline
  def BucketOwner: BucketOwner = "BucketOwner".asInstanceOf[BucketOwner]
  
  @js.native
  sealed trait Bulk extends StObject
  @scala.inline
  def Bulk: Bulk = "Bulk".asInstanceOf[Bulk]
  
  @js.native
  sealed trait COMPLETE extends StObject
  @scala.inline
  def COMPLETE: COMPLETE = "COMPLETE".asInstanceOf[COMPLETE]
  
  @js.native
  sealed trait COPY extends StObject
  @scala.inline
  def COPY: COPY = "COPY".asInstanceOf[COPY]
  
  @js.native
  sealed trait CSV extends StObject
  @scala.inline
  def CSV: CSV = "CSV".asInstanceOf[CSV]
  
  @js.native
  sealed trait CanonicalUser extends StObject
  @scala.inline
  def CanonicalUser: CanonicalUser = "CanonicalUser".asInstanceOf[CanonicalUser]
  
  @js.native
  sealed trait Current extends StObject
  @scala.inline
  def Current: Current = "Current".asInstanceOf[Current]
  
  @js.native
  sealed trait DOCUMENT extends StObject
  @scala.inline
  def DOCUMENT: DOCUMENT = "DOCUMENT".asInstanceOf[DOCUMENT]
  
  @js.native
  sealed trait Daily extends StObject
  @scala.inline
  def Daily: Daily = "Daily".asInstanceOf[Daily]
  
  @js.native
  sealed trait Destination extends StObject
  @scala.inline
  def Destination: Destination = "Destination".asInstanceOf[Destination]
  
  @js.native
  sealed trait Disabled extends StObject
  @scala.inline
  def Disabled: Disabled = "Disabled".asInstanceOf[Disabled]
  
  @js.native
  sealed trait ETag extends StObject
  @scala.inline
  def ETag: ETag = "ETag".asInstanceOf[ETag]
  
  @js.native
  sealed trait EU extends StObject
  @scala.inline
  def EU: EU = "EU".asInstanceOf[EU]
  
  @js.native
  sealed trait Enabled extends StObject
  @scala.inline
  def Enabled: Enabled = "Enabled".asInstanceOf[Enabled]
  
  @js.native
  sealed trait EncryptionStatus extends StObject
  @scala.inline
  def EncryptionStatus: EncryptionStatus = "EncryptionStatus".asInstanceOf[EncryptionStatus]
  
  @js.native
  sealed trait Expedited extends StObject
  @scala.inline
  def Expedited: Expedited = "Expedited".asInstanceOf[Expedited]
  
  @js.native
  sealed trait FAILED extends StObject
  @scala.inline
  def FAILED: FAILED = "FAILED".asInstanceOf[FAILED]
  
  @js.native
  sealed trait FULL_CONTROL extends StObject
  @scala.inline
  def FULL_CONTROL: FULL_CONTROL = "FULL_CONTROL".asInstanceOf[FULL_CONTROL]
  
  @js.native
  sealed trait GLACIER extends StObject
  @scala.inline
  def GLACIER: GLACIER = "GLACIER".asInstanceOf[GLACIER]
  
  @js.native
  sealed trait GZIP extends StObject
  @scala.inline
  def GZIP: GZIP = "GZIP".asInstanceOf[GZIP]
  
  @js.native
  sealed trait Group extends StObject
  @scala.inline
  def Group: Group = "Group".asInstanceOf[Group]
  
  @js.native
  sealed trait IGNORE extends StObject
  @scala.inline
  def IGNORE: IGNORE = "IGNORE".asInstanceOf[IGNORE]
  
  @js.native
  sealed trait IsMultipartUploaded extends StObject
  @scala.inline
  def IsMultipartUploaded: IsMultipartUploaded = "IsMultipartUploaded".asInstanceOf[IsMultipartUploaded]
  
  @js.native
  sealed trait LINES extends StObject
  @scala.inline
  def LINES: LINES = "LINES".asInstanceOf[LINES]
  
  @js.native
  sealed trait LastModifiedDate extends StObject
  @scala.inline
  def LastModifiedDate: LastModifiedDate = "LastModifiedDate".asInstanceOf[LastModifiedDate]
  
  @js.native
  sealed trait NONE extends StObject
  @scala.inline
  def NONE: NONE = "NONE".asInstanceOf[NONE]
  
  @js.native
  sealed trait NoSuchBucket extends StObject
  @scala.inline
  def NoSuchBucket: NoSuchBucket = "NoSuchBucket".asInstanceOf[NoSuchBucket]
  
  @js.native
  sealed trait NoSuchKey extends StObject
  @scala.inline
  def NoSuchKey: NoSuchKey = "NoSuchKey".asInstanceOf[NoSuchKey]
  
  @js.native
  sealed trait NoSuchUpload extends StObject
  @scala.inline
  def NoSuchUpload: NoSuchUpload = "NoSuchUpload".asInstanceOf[NoSuchUpload]
  
  @js.native
  sealed trait ONEZONE_IA extends StObject
  @scala.inline
  def ONEZONE_IA: ONEZONE_IA = "ONEZONE_IA".asInstanceOf[ONEZONE_IA]
  
  @js.native
  sealed trait ORC extends StObject
  @scala.inline
  def ORC: ORC = "ORC".asInstanceOf[ORC]
  
  @js.native
  sealed trait ObjectAlreadyInActiveTierError extends StObject
  @scala.inline
  def ObjectAlreadyInActiveTierError: ObjectAlreadyInActiveTierError = "ObjectAlreadyInActiveTierError".asInstanceOf[ObjectAlreadyInActiveTierError]
  
  @js.native
  sealed trait ObjectNotInActiveTierError extends StObject
  @scala.inline
  def ObjectNotInActiveTierError: ObjectNotInActiveTierError = "ObjectNotInActiveTierError".asInstanceOf[ObjectNotInActiveTierError]
  
  @js.native
  sealed trait PENDING extends StObject
  @scala.inline
  def PENDING: PENDING = "PENDING".asInstanceOf[PENDING]
  
  @js.native
  sealed trait READ extends StObject
  @scala.inline
  def READ: READ = "READ".asInstanceOf[READ]
  
  @js.native
  sealed trait READ_ACP extends StObject
  @scala.inline
  def READ_ACP: READ_ACP = "READ_ACP".asInstanceOf[READ_ACP]
  
  @js.native
  sealed trait REDUCED_REDUNDANCY extends StObject
  @scala.inline
  def REDUCED_REDUNDANCY: REDUCED_REDUNDANCY = "REDUCED_REDUNDANCY".asInstanceOf[REDUCED_REDUNDANCY]
  
  @js.native
  sealed trait REPLACE extends StObject
  @scala.inline
  def REPLACE: REPLACE = "REPLACE".asInstanceOf[REPLACE]
  
  @js.native
  sealed trait REPLICA extends StObject
  @scala.inline
  def REPLICA: REPLICA = "REPLICA".asInstanceOf[REPLICA]
  
  @js.native
  sealed trait ReplicationStatus extends StObject
  @scala.inline
  def ReplicationStatus: ReplicationStatus = "ReplicationStatus".asInstanceOf[ReplicationStatus]
  
  @js.native
  sealed trait Requester extends StObject
  @scala.inline
  def Requester: Requester = "Requester".asInstanceOf[Requester]
  
  @js.native
  sealed trait SELECT extends StObject
  @scala.inline
  def SELECT: SELECT = "SELECT".asInstanceOf[SELECT]
  
  @js.native
  sealed trait SQL extends StObject
  @scala.inline
  def SQL: SQL = "SQL".asInstanceOf[SQL]
  
  @js.native
  sealed trait STANDARD extends StObject
  @scala.inline
  def STANDARD: STANDARD = "STANDARD".asInstanceOf[STANDARD]
  
  @js.native
  sealed trait STANDARD_IA extends StObject
  @scala.inline
  def STANDARD_IA: STANDARD_IA = "STANDARD_IA".asInstanceOf[STANDARD_IA]
  
  @js.native
  sealed trait Size extends StObject
  @scala.inline
  def Size: Size = "Size".asInstanceOf[Size]
  
  @js.native
  sealed trait Standard_ extends StObject
  @scala.inline
  def Standard_ : Standard_ = "Standard".asInstanceOf[Standard_]
  
  @js.native
  sealed trait StorageClass extends StObject
  @scala.inline
  def StorageClass: StorageClass = "StorageClass".asInstanceOf[StorageClass]
  
  @js.native
  sealed trait Suspended extends StObject
  @scala.inline
  def Suspended: Suspended = "Suspended".asInstanceOf[Suspended]
  
  @js.native
  sealed trait USE extends StObject
  @scala.inline
  def USE: USE = "USE".asInstanceOf[USE]
  
  @js.native
  sealed trait V_1 extends StObject
  @scala.inline
  def V_1: V_1 = "V_1".asInstanceOf[V_1]
  
  @js.native
  sealed trait WRITE extends StObject
  @scala.inline
  def WRITE: WRITE = "WRITE".asInstanceOf[WRITE]
  
  @js.native
  sealed trait WRITE_ACP extends StObject
  @scala.inline
  def WRITE_ACP: WRITE_ACP = "WRITE_ACP".asInstanceOf[WRITE_ACP]
  
  @js.native
  sealed trait Weekly extends StObject
  @scala.inline
  def Weekly: Weekly = "Weekly".asInstanceOf[Weekly]
  
  @js.native
  sealed trait `ap-northeast-1` extends StObject
  @scala.inline
  def `ap-northeast-1`: `ap-northeast-1` = "ap-northeast-1".asInstanceOf[`ap-northeast-1`]
  
  @js.native
  sealed trait `ap-south-1` extends StObject
  @scala.inline
  def `ap-south-1`: `ap-south-1` = "ap-south-1".asInstanceOf[`ap-south-1`]
  
  @js.native
  sealed trait `ap-southeast-1` extends StObject
  @scala.inline
  def `ap-southeast-1`: `ap-southeast-1` = "ap-southeast-1".asInstanceOf[`ap-southeast-1`]
  
  @js.native
  sealed trait `ap-southeast-2` extends StObject
  @scala.inline
  def `ap-southeast-2`: `ap-southeast-2` = "ap-southeast-2".asInstanceOf[`ap-southeast-2`]
  
  @js.native
  sealed trait `authenticated-read` extends StObject
  @scala.inline
  def `authenticated-read`: `authenticated-read` = "authenticated-read".asInstanceOf[`authenticated-read`]
  
  @js.native
  sealed trait `aws-exec-read` extends StObject
  @scala.inline
  def `aws-exec-read`: `aws-exec-read` = "aws-exec-read".asInstanceOf[`aws-exec-read`]
  
  @js.native
  sealed trait awsColonkms extends StObject
  @scala.inline
  def awsColonkms: awsColonkms = "aws:kms".asInstanceOf[awsColonkms]
  
  @js.native
  sealed trait `bucket-owner-full-control` extends StObject
  @scala.inline
  def `bucket-owner-full-control`: `bucket-owner-full-control` = "bucket-owner-full-control".asInstanceOf[`bucket-owner-full-control`]
  
  @js.native
  sealed trait `bucket-owner-read` extends StObject
  @scala.inline
  def `bucket-owner-read`: `bucket-owner-read` = "bucket-owner-read".asInstanceOf[`bucket-owner-read`]
  
  @js.native
  sealed trait `cn-north-1` extends StObject
  @scala.inline
  def `cn-north-1`: `cn-north-1` = "cn-north-1".asInstanceOf[`cn-north-1`]
  
  @js.native
  sealed trait `eu-central-1` extends StObject
  @scala.inline
  def `eu-central-1`: `eu-central-1` = "eu-central-1".asInstanceOf[`eu-central-1`]
  
  @js.native
  sealed trait `eu-west-1` extends StObject
  @scala.inline
  def `eu-west-1`: `eu-west-1` = "eu-west-1".asInstanceOf[`eu-west-1`]
  
  @js.native
  sealed trait http extends StObject
  @scala.inline
  def http: http = "http".asInstanceOf[http]
  
  @js.native
  sealed trait https extends StObject
  @scala.inline
  def https: https = "https".asInstanceOf[https]
  
  @js.native
  sealed trait prefix extends StObject
  @scala.inline
  def prefix: prefix = "prefix".asInstanceOf[prefix]
  
  @js.native
  sealed trait `private` extends StObject
  @scala.inline
  def `private`: `private` = "private".asInstanceOf[`private`]
  
  @js.native
  sealed trait `public-read` extends StObject
  @scala.inline
  def `public-read`: `public-read` = "public-read".asInstanceOf[`public-read`]
  
  @js.native
  sealed trait `public-read-write` extends StObject
  @scala.inline
  def `public-read-write`: `public-read-write` = "public-read-write".asInstanceOf[`public-read-write`]
  
  @js.native
  sealed trait requester_ extends StObject
  @scala.inline
  def requester_ : requester_ = "requester".asInstanceOf[requester_]
  
  @js.native
  sealed trait s3ColonObjectCreatedColonAsterisk extends StObject
  @scala.inline
  def s3ColonObjectCreatedColonAsterisk: s3ColonObjectCreatedColonAsterisk = "s3:ObjectCreated:*".asInstanceOf[s3ColonObjectCreatedColonAsterisk]
  
  @js.native
  sealed trait s3ColonObjectCreatedColonCompleteMultipartUpload extends StObject
  @scala.inline
  def s3ColonObjectCreatedColonCompleteMultipartUpload: s3ColonObjectCreatedColonCompleteMultipartUpload = "s3:ObjectCreated:CompleteMultipartUpload".asInstanceOf[s3ColonObjectCreatedColonCompleteMultipartUpload]
  
  @js.native
  sealed trait s3ColonObjectCreatedColonCopy extends StObject
  @scala.inline
  def s3ColonObjectCreatedColonCopy: s3ColonObjectCreatedColonCopy = "s3:ObjectCreated:Copy".asInstanceOf[s3ColonObjectCreatedColonCopy]
  
  @js.native
  sealed trait s3ColonObjectCreatedColonPost extends StObject
  @scala.inline
  def s3ColonObjectCreatedColonPost: s3ColonObjectCreatedColonPost = "s3:ObjectCreated:Post".asInstanceOf[s3ColonObjectCreatedColonPost]
  
  @js.native
  sealed trait s3ColonObjectCreatedColonPut extends StObject
  @scala.inline
  def s3ColonObjectCreatedColonPut: s3ColonObjectCreatedColonPut = "s3:ObjectCreated:Put".asInstanceOf[s3ColonObjectCreatedColonPut]
  
  @js.native
  sealed trait s3ColonObjectRemovedColonAsterisk extends StObject
  @scala.inline
  def s3ColonObjectRemovedColonAsterisk: s3ColonObjectRemovedColonAsterisk = "s3:ObjectRemoved:*".asInstanceOf[s3ColonObjectRemovedColonAsterisk]
  
  @js.native
  sealed trait s3ColonObjectRemovedColonDelete extends StObject
  @scala.inline
  def s3ColonObjectRemovedColonDelete: s3ColonObjectRemovedColonDelete = "s3:ObjectRemoved:Delete".asInstanceOf[s3ColonObjectRemovedColonDelete]
  
  @js.native
  sealed trait s3ColonObjectRemovedColonDeleteMarkerCreated extends StObject
  @scala.inline
  def s3ColonObjectRemovedColonDeleteMarkerCreated: s3ColonObjectRemovedColonDeleteMarkerCreated = "s3:ObjectRemoved:DeleteMarkerCreated".asInstanceOf[s3ColonObjectRemovedColonDeleteMarkerCreated]
  
  @js.native
  sealed trait s3ColonReducedRedundancyLostObject extends StObject
  @scala.inline
  def s3ColonReducedRedundancyLostObject: s3ColonReducedRedundancyLostObject = "s3:ReducedRedundancyLostObject".asInstanceOf[s3ColonReducedRedundancyLostObject]
  
  @js.native
  sealed trait `sa-east-1` extends StObject
  @scala.inline
  def `sa-east-1`: `sa-east-1` = "sa-east-1".asInstanceOf[`sa-east-1`]
  
  @js.native
  sealed trait suffix extends StObject
  @scala.inline
  def suffix: suffix = "suffix".asInstanceOf[suffix]
  
  @js.native
  sealed trait url extends StObject
  @scala.inline
  def url: url = "url".asInstanceOf[url]
  
  @js.native
  sealed trait `us-west-1` extends StObject
  @scala.inline
  def `us-west-1`: `us-west-1` = "us-west-1".asInstanceOf[`us-west-1`]
  
  @js.native
  sealed trait `us-west-2` extends StObject
  @scala.inline
  def `us-west-2`: `us-west-2` = "us-west-2".asInstanceOf[`us-west-2`]
}
