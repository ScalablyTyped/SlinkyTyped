package typingsSlinky.antdMobileRn

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object paginationLocaleEnUSMod {
  
  object default {
    
    @JSImport("antd-mobile-rn/lib/pagination/locale/en_US", JSImport.Default)
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("antd-mobile-rn/lib/pagination/locale/en_US", "default.nextText")
    @js.native
    def nextText: String = js.native
    @scala.inline
    def nextText_=(x: String): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("nextText")(x.asInstanceOf[js.Any])
    
    @JSImport("antd-mobile-rn/lib/pagination/locale/en_US", "default.prevText")
    @js.native
    def prevText: String = js.native
    @scala.inline
    def prevText_=(x: String): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("prevText")(x.asInstanceOf[js.Any])
  }
}
