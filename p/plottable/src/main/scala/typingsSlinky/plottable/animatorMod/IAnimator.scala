  ...
  implicit class IAnimatorOps[Self <: IAnimator] (val x: Self) extends AnyVal {
    ...
    def withAnimate(
      value: (SimpleSelection[_], AttributeToAppliedProjector) => SimpleSelection[_] | (Transition_[_, _, _, _])
        ...
        ret.asInstanceOf[js.Dynamic].updateDynamic("animate")(js.Any.fromFunction2(value))
        ret
    }
    @scala.inline
    def withTotalTime(value: Double => Double): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("totalTime")(js.Any.fromFunction1(value))
        ret
    }
  }
