...
import typingsSlinky.pThrottle.anon.Abort
...
object mod {
  ...
  trait AbortErrorClass extends Error {
    ...
    @JSName("name")
    val name_AbortErrorClass: typingsSlinky.pThrottle.pThrottleStrings.AbortError = js.native
  }
  object AbortErrorClass {
    
    @scala.inline
    def apply(message: String, name: typingsSlinky.pThrottle.pThrottleStrings.AbortError): AbortErrorClass = {
    ...
    implicit class AbortErrorClassMutableBuilder[Self <: AbortErrorClass] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setName(value: typingsSlinky.pThrottle.pThrottleStrings.AbortError): Self = StObject.set(x, "name", value.asInstanceOf[js.Any])
    }
  }
