package typingsSlinky.leafletMousePosition.mod

import typingsSlinky.leafletMousePosition.mod.Control_.MousePosition
import typingsSlinky.leafletMousePosition.mod.Control_.MousePositionControlOptions
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("leaflet", "control")
@js.native
object control extends js.Object {
  
  def mousePosition(): MousePosition = js.native
  def mousePosition(options: MousePositionControlOptions): MousePosition = js.native
}
