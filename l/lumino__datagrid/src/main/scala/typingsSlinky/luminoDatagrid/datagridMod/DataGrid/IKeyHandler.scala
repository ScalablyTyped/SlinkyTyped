...
trait IKeyHandler extends IDisposable {
  ...
  /**
    ...
    */
  def onKeyDown(grid: typingsSlinky.luminoDatagrid.datagridMod.DataGrid, event: KeyboardEvent): Unit = js.native
}
object IKeyHandler {
  ...
  def apply(
    dispose: () => Unit,
    isDisposed: Boolean,
    onKeyDown: (typingsSlinky.luminoDatagrid.datagridMod.DataGrid, KeyboardEvent) => Unit
  ): IKeyHandler = {
  ...
  implicit class IKeyHandlerOps[Self <: IKeyHandler] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setOnKeyDown(value: (typingsSlinky.luminoDatagrid.datagridMod.DataGrid, KeyboardEvent) => Unit): Self = this.set("onKeyDown", js.Any.fromFunction2(value))
  }
}
