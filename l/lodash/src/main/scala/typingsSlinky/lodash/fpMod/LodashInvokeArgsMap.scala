package typingsSlinky.lodash.fpMod

import typingsSlinky.lodash.lodashMod.__
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait LodashInvokeArgsMap extends js.Object {
  def apply(methodNameOrMethod: __, args: js.Array[_]): LodashInvokeArgsMap1x2 = js.native
  def apply(methodNameOrMethod: __, args: js.Array[_], collection: js.Object): LodashInvokeArgsMap1x6 = js.native
  def apply(methodNameOrMethod: __, args: __): LodashInvokeArgsMap1x4 = js.native
  def apply(methodNameOrMethod: __, args: __, collection: js.Object): LodashInvokeArgsMap1x4 = js.native
  def apply(methodName: String): LodashInvokeArgsMap1x1 = js.native
  def apply(methodName: String, args: js.Array[_]): LodashInvokeArgsMap1x3 = js.native
  def apply(methodName: String, args: js.Array[_], collection: js.Object): js.Array[_] = js.native
  def apply(methodName: String, args: __): LodashInvokeArgsMap1x5 = js.native
  def apply(methodName: String, args: __, collection: js.Object): LodashInvokeArgsMap1x5 = js.native
  def apply[TResult](method: js.Function1[/* repeated */ js.Any, TResult]): LodashInvokeArgsMap2x1[TResult] = js.native
  def apply[TResult](method: js.Function1[/* repeated */ js.Any, TResult], args: js.Array[_]): LodashInvokeArgsMap2x3[TResult] = js.native
  def apply[TResult](method: js.Function1[/* repeated */ js.Any, TResult], args: js.Array[_], collection: js.Object): js.Array[TResult] = js.native
  def apply[TResult](method: js.Function1[/* repeated */ js.Any, TResult], args: __): LodashInvokeArgsMap2x5[TResult] = js.native
  def apply[TResult](method: js.Function1[/* repeated */ js.Any, TResult], args: __, collection: js.Object): LodashInvokeArgsMap2x5[TResult] = js.native
}

