package typingsSlinky.javascriptTimeAgo

import org.scalablytyped.runtime.Shortcut
import typingsSlinky.javascriptTimeAgo.gradationMod.Gradation
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object canonicalMod extends Shortcut {
  
  @JSImport("javascript-time-ago/gradation/canonical", JSImport.Default)
  @js.native
  val default: js.Array[Gradation] = js.native
  
  type _To = js.Array[Gradation]
  
  /* This means you don't have to write `default`, but can instead just say `canonicalMod.foo` */
  override def _to: js.Array[Gradation] = default
}
