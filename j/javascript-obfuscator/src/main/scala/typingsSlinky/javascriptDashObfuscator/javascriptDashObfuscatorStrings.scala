package typingsSlinky.javascriptDashObfuscator

import typingsSlinky.javascriptDashObfuscator.espreeMod.SourceType
import typingsSlinky.javascriptDashObfuscator.srcTypesNodeDashTransformersTVisitorDirectionMod.TVisitorDirection
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object javascriptDashObfuscatorStrings {
  @js.native
  sealed trait compact extends js.Object
  
  @js.native
  sealed trait config extends js.Object
  
  @js.native
  sealed trait controlFlowFlattening extends js.Object
  
  @js.native
  sealed trait controlFlowFlatteningThreshold extends js.Object
  
  @js.native
  sealed trait deadCodeInjection extends js.Object
  
  @js.native
  sealed trait deadCodeInjectionThreshold extends js.Object
  
  @js.native
  sealed trait debugProtection extends js.Object
  
  @js.native
  sealed trait debugProtectionInterval extends js.Object
  
  @js.native
  sealed trait disableConsoleOutput extends js.Object
  
  @js.native
  sealed trait domainLock extends js.Object
  
  @js.native
  sealed trait enter extends TVisitorDirection
  
  @js.native
  sealed trait exclude extends js.Object
  
  @js.native
  sealed trait identifierNamesGenerator extends js.Object
  
  @js.native
  sealed trait identifiersPrefix extends js.Object
  
  @js.native
  sealed trait inputFileName extends js.Object
  
  @js.native
  sealed trait leave extends TVisitorDirection
  
  @js.native
  sealed trait log extends js.Object
  
  @js.native
  sealed trait module extends SourceType
  
  @js.native
  sealed trait output extends js.Object
  
  @js.native
  sealed trait renameGlobals extends js.Object
  
  @js.native
  sealed trait reservedNames extends js.Object
  
  @js.native
  sealed trait reservedStrings extends js.Object
  
  @js.native
  sealed trait rotateStringArray extends js.Object
  
  @js.native
  sealed trait script extends SourceType
  
  @js.native
  sealed trait seed extends js.Object
  
  @js.native
  sealed trait selfDefending extends js.Object
  
  @js.native
  sealed trait sourceMap extends js.Object
  
  @js.native
  sealed trait sourceMapBaseUrl extends js.Object
  
  @js.native
  sealed trait sourceMapFileName extends js.Object
  
  @js.native
  sealed trait sourceMapMode extends js.Object
  
  @js.native
  sealed trait stringArray extends js.Object
  
  @js.native
  sealed trait stringArrayEncoding extends js.Object
  
  @js.native
  sealed trait stringArrayThreshold extends js.Object
  
  @js.native
  sealed trait target extends js.Object
  
  @js.native
  sealed trait transformObjectKeys extends js.Object
  
  @js.native
  sealed trait unicodeEscapeSequence extends js.Object
  
  @js.native
  sealed trait `use strict` extends js.Object
  
  @js.native
  sealed trait version extends js.Object
  
  @scala.inline
  def compact: compact = "compact".asInstanceOf[compact]
  @scala.inline
  def config: config = "config".asInstanceOf[config]
  @scala.inline
  def controlFlowFlattening: controlFlowFlattening = "controlFlowFlattening".asInstanceOf[controlFlowFlattening]
  @scala.inline
  def controlFlowFlatteningThreshold: controlFlowFlatteningThreshold = "controlFlowFlatteningThreshold".asInstanceOf[controlFlowFlatteningThreshold]
  @scala.inline
  def deadCodeInjection: deadCodeInjection = "deadCodeInjection".asInstanceOf[deadCodeInjection]
  @scala.inline
  def deadCodeInjectionThreshold: deadCodeInjectionThreshold = "deadCodeInjectionThreshold".asInstanceOf[deadCodeInjectionThreshold]
  @scala.inline
  def debugProtection: debugProtection = "debugProtection".asInstanceOf[debugProtection]
  @scala.inline
  def debugProtectionInterval: debugProtectionInterval = "debugProtectionInterval".asInstanceOf[debugProtectionInterval]
  @scala.inline
  def disableConsoleOutput: disableConsoleOutput = "disableConsoleOutput".asInstanceOf[disableConsoleOutput]
  @scala.inline
  def domainLock: domainLock = "domainLock".asInstanceOf[domainLock]
  @scala.inline
  def enter: enter = "enter".asInstanceOf[enter]
  @scala.inline
  def exclude: exclude = "exclude".asInstanceOf[exclude]
  @scala.inline
  def identifierNamesGenerator: identifierNamesGenerator = "identifierNamesGenerator".asInstanceOf[identifierNamesGenerator]
  @scala.inline
  def identifiersPrefix: identifiersPrefix = "identifiersPrefix".asInstanceOf[identifiersPrefix]
  @scala.inline
  def inputFileName: inputFileName = "inputFileName".asInstanceOf[inputFileName]
  @scala.inline
  def leave: leave = "leave".asInstanceOf[leave]
  @scala.inline
  def log: log = "log".asInstanceOf[log]
  @scala.inline
  def module: module = "module".asInstanceOf[module]
  @scala.inline
  def output: output = "output".asInstanceOf[output]
  @scala.inline
  def renameGlobals: renameGlobals = "renameGlobals".asInstanceOf[renameGlobals]
  @scala.inline
  def reservedNames: reservedNames = "reservedNames".asInstanceOf[reservedNames]
  @scala.inline
  def reservedStrings: reservedStrings = "reservedStrings".asInstanceOf[reservedStrings]
  @scala.inline
  def rotateStringArray: rotateStringArray = "rotateStringArray".asInstanceOf[rotateStringArray]
  @scala.inline
  def script: script = "script".asInstanceOf[script]
  @scala.inline
  def seed: seed = "seed".asInstanceOf[seed]
  @scala.inline
  def selfDefending: selfDefending = "selfDefending".asInstanceOf[selfDefending]
  @scala.inline
  def sourceMap: sourceMap = "sourceMap".asInstanceOf[sourceMap]
  @scala.inline
  def sourceMapBaseUrl: sourceMapBaseUrl = "sourceMapBaseUrl".asInstanceOf[sourceMapBaseUrl]
  @scala.inline
  def sourceMapFileName: sourceMapFileName = "sourceMapFileName".asInstanceOf[sourceMapFileName]
  @scala.inline
  def sourceMapMode: sourceMapMode = "sourceMapMode".asInstanceOf[sourceMapMode]
  @scala.inline
  def stringArray: stringArray = "stringArray".asInstanceOf[stringArray]
  @scala.inline
  def stringArrayEncoding: stringArrayEncoding = "stringArrayEncoding".asInstanceOf[stringArrayEncoding]
  @scala.inline
  def stringArrayThreshold: stringArrayThreshold = "stringArrayThreshold".asInstanceOf[stringArrayThreshold]
  @scala.inline
  def target: target = "target".asInstanceOf[target]
  @scala.inline
  def transformObjectKeys: transformObjectKeys = "transformObjectKeys".asInstanceOf[transformObjectKeys]
  @scala.inline
  def unicodeEscapeSequence: unicodeEscapeSequence = "unicodeEscapeSequence".asInstanceOf[unicodeEscapeSequence]
  @scala.inline
  def `use strict`: `use strict` = "use strict".asInstanceOf[`use strict`]
  @scala.inline
  def version: version = "version".asInstanceOf[version]
}

