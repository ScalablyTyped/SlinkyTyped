package typingsSlinky.atJupyterlabConsoleDashExtension

import typingsSlinky.atJupyterlabApplication.libFrontendMod.JupyterFrontEndPlugin
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@jupyterlab/console-extension/lib/foreign", JSImport.Namespace)
@js.native
object libForeignMod extends js.Object {
  val default: JupyterFrontEndPlugin[Unit] = js.native
  val foreign: JupyterFrontEndPlugin[Unit] = js.native
}

