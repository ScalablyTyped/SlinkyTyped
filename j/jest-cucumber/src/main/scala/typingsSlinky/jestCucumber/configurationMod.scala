package typingsSlinky.jestCucumber

import typingsSlinky.jestCucumber.anon.Errors
import typingsSlinky.jestCucumber.modelsMod.Options
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object configurationMod {
  
  @JSImport("jest-cucumber/dist/src/configuration", "getJestCucumberConfiguration")
  @js.native
  def getJestCucumberConfiguration(): Errors = js.native
  @JSImport("jest-cucumber/dist/src/configuration", "getJestCucumberConfiguration")
  @js.native
  def getJestCucumberConfiguration(options: Options): Errors = js.native
  
  @JSImport("jest-cucumber/dist/src/configuration", "setJestCucumberConfiguration")
  @js.native
  def setJestCucumberConfiguration(options: Options): Unit = js.native
}
