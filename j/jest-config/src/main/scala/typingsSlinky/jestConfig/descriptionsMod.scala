package typingsSlinky.jestConfig

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("jest-config/build/Descriptions", JSImport.Namespace)
@js.native
object descriptionsMod extends js.Object {
  
  @js.native
  object default extends js.Object {
    
    var automock: js.UndefOr[String] = js.native
    
    var bail: js.UndefOr[String] = js.native
    
    var cache: js.UndefOr[String] = js.native
    
    var cacheDirectory: js.UndefOr[String] = js.native
    
    var changedFilesWithAncestor: js.UndefOr[String] = js.native
    
    var changedSince: js.UndefOr[String] = js.native
    
    var clearMocks: js.UndefOr[String] = js.native
    
    var collectCoverage: js.UndefOr[String] = js.native
    
    var collectCoverageFrom: js.UndefOr[String] = js.native
    
    var collectCoverageOnlyFrom: js.UndefOr[String] = js.native
    
    var coverageDirectory: js.UndefOr[String] = js.native
    
    var coveragePathIgnorePatterns: js.UndefOr[String] = js.native
    
    var coverageProvider: js.UndefOr[String] = js.native
    
    var coverageReporters: js.UndefOr[String] = js.native
    
    var coverageThreshold: js.UndefOr[String] = js.native
    
    var dependencyExtractor: js.UndefOr[String] = js.native
    
    var detectLeaks: js.UndefOr[String] = js.native
    
    var detectOpenHandles: js.UndefOr[String] = js.native
    
    var displayName: js.UndefOr[String] = js.native
    
    var errorOnDeprecated: js.UndefOr[String] = js.native
    
    var expand: js.UndefOr[String] = js.native
    
    var extraGlobals: js.UndefOr[String] = js.native
    
    var filter: js.UndefOr[String] = js.native
    
    var findRelatedTests: js.UndefOr[String] = js.native
    
    var forceCoverageMatch: js.UndefOr[String] = js.native
    
    var forceExit: js.UndefOr[String] = js.native
    
    var globalSetup: js.UndefOr[String] = js.native
    
    var globalTeardown: js.UndefOr[String] = js.native
    
    var globals: js.UndefOr[String] = js.native
    
    var haste: js.UndefOr[String] = js.native
    
    var injectGlobals: js.UndefOr[String] = js.native
    
    var json: js.UndefOr[String] = js.native
    
    var lastCommit: js.UndefOr[String] = js.native
    
    var listTests: js.UndefOr[String] = js.native
    
    var logHeapUsage: js.UndefOr[String] = js.native
    
    var mapCoverage: js.UndefOr[String] = js.native
    
    var maxConcurrency: js.UndefOr[String] = js.native
    
    var maxWorkers: js.UndefOr[String] = js.native
    
    var moduleDirectories: js.UndefOr[String] = js.native
    
    var moduleFileExtensions: js.UndefOr[String] = js.native
    
    var moduleLoader: js.UndefOr[String] = js.native
    
    var moduleNameMapper: js.UndefOr[String] = js.native
    
    var modulePathIgnorePatterns: js.UndefOr[String] = js.native
    
    var modulePaths: js.UndefOr[String] = js.native
    
    var name: js.UndefOr[String] = js.native
    
    var noStackTrace: js.UndefOr[String] = js.native
    
    var notifyMode: js.UndefOr[String] = js.native
    
    @JSName("notify")
    var notify_Fdefault: js.UndefOr[String] = js.native
    
    var onlyChanged: js.UndefOr[String] = js.native
    
    var onlyFailures: js.UndefOr[String] = js.native
    
    var outputFile: js.UndefOr[String] = js.native
    
    var passWithNoTests: js.UndefOr[String] = js.native
    
    var preprocessorIgnorePatterns: js.UndefOr[String] = js.native
    
    var preset: js.UndefOr[String] = js.native
    
    var prettierPath: js.UndefOr[String] = js.native
    
    var projects: js.UndefOr[String] = js.native
    
    var replname: js.UndefOr[String] = js.native
    
    var reporters: js.UndefOr[String] = js.native
    
    var resetMocks: js.UndefOr[String] = js.native
    
    var resetModules: js.UndefOr[String] = js.native
    
    var resolver: js.UndefOr[String] = js.native
    
    var restoreMocks: js.UndefOr[String] = js.native
    
    var rootDir: js.UndefOr[String] = js.native
    
    var roots: js.UndefOr[String] = js.native
    
    var runTestsByPath: js.UndefOr[String] = js.native
    
    var runner: js.UndefOr[String] = js.native
    
    var scriptPreprocessor: js.UndefOr[String] = js.native
    
    var setupFiles: js.UndefOr[String] = js.native
    
    var setupFilesAfterEnv: js.UndefOr[String] = js.native
    
    var setupTestFrameworkScriptFile: js.UndefOr[String] = js.native
    
    var silent: js.UndefOr[String] = js.native
    
    var skipFilter: js.UndefOr[String] = js.native
    
    var skipNodeResolution: js.UndefOr[String] = js.native
    
    var slowTestThreshold: js.UndefOr[String] = js.native
    
    var snapshotResolver: js.UndefOr[String] = js.native
    
    var snapshotSerializers: js.UndefOr[String] = js.native
    
    var testEnvironment: js.UndefOr[String] = js.native
    
    var testEnvironmentOptions: js.UndefOr[String] = js.native
    
    var testFailureExitCode: js.UndefOr[String] = js.native
    
    var testLocationInResults: js.UndefOr[String] = js.native
    
    var testMatch: js.UndefOr[String] = js.native
    
    var testNamePattern: js.UndefOr[String] = js.native
    
    var testPathDirs: js.UndefOr[String] = js.native
    
    var testPathIgnorePatterns: js.UndefOr[String] = js.native
    
    var testRegex: js.UndefOr[String] = js.native
    
    var testResultsProcessor: js.UndefOr[String] = js.native
    
    var testRunner: js.UndefOr[String] = js.native
    
    var testSequencer: js.UndefOr[String] = js.native
    
    var testTimeout: js.UndefOr[String] = js.native
    
    var testURL: js.UndefOr[String] = js.native
    
    var timers: js.UndefOr[String] = js.native
    
    var transform: js.UndefOr[String] = js.native
    
    var transformIgnorePatterns: js.UndefOr[String] = js.native
    
    var unmockedModulePathPatterns: js.UndefOr[String] = js.native
    
    var updateSnapshot: js.UndefOr[String] = js.native
    
    var useStderr: js.UndefOr[String] = js.native
    
    var verbose: js.UndefOr[String] = js.native
    
    var watch: js.UndefOr[String] = js.native
    
    var watchAll: js.UndefOr[String] = js.native
    
    var watchPathIgnorePatterns: js.UndefOr[String] = js.native
    
    var watchPlugins: js.UndefOr[String] = js.native
    
    var watchman: js.UndefOr[String] = js.native
  }
}
