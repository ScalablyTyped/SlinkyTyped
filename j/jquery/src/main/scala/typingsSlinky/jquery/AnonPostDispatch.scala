...
trait AnonPostDispatch[TTarget]
  ...
  def postDispatch(event: typingsSlinky.jquery.JQuery_.Event): Unit = js.native
}

object AnonPostDispatch {
  @scala.inline
  def apply[TTarget](postDispatch: typingsSlinky.jquery.JQuery_.Event => Unit): AnonPostDispatch[TTarget] = {
  ...
  implicit class AnonPostDispatchOps[Self[ttarget] <: AnonPostDispatch[ttarget], TTarget] (val x: Self[TTarget]) extends AnyVal {
    ...
    @scala.inline
    def withPostDispatch(value: typingsSlinky.jquery.JQuery_.Event => Unit): Self[TTarget] = {
