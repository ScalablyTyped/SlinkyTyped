...
trait AnonTrigger[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
    ...
    */
  def trigger(event: typingsSlinky.jquery.JQuery_.Event, data: TData): Unit | `false` = js.native
}

object AnonTrigger {
  @scala.inline
  def apply[TTarget, TData](trigger: (typingsSlinky.jquery.JQuery_.Event, TData) => Unit | `false`): AnonTrigger[TTarget, TData] = {
  ...
  implicit class AnonTriggerOps[Self[ttarget, tdata] <: AnonTrigger[ttarget, tdata], TTarget, TData] (val x: Self[TTarget, TData]) extends AnyVal {
    ...
    @scala.inline
    def withTrigger(value: (typingsSlinky.jquery.JQuery_.Event, TData) => Unit | `false`): Self[TTarget, TData] = {
