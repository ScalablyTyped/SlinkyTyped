...
object MouseLeaveEvent {
  ...
  @scala.inline
  implicit class MouseLeaveEventOps[Self <: MouseLeaveEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (MouseLeaveEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
