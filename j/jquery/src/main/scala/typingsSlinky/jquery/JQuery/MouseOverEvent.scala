...
object MouseOverEvent {
  ...
  @scala.inline
  implicit class MouseOverEventOps[Self <: MouseOverEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (MouseOverEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
