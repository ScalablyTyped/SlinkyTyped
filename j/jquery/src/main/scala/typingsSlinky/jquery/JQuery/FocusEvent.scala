...
object FocusEvent {
  ...
  @scala.inline
  implicit class FocusEventOps[Self <: FocusEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (FocusEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def setType(value: focus): Self = this.set("type", value.asInstanceOf[js.Any])
  }
