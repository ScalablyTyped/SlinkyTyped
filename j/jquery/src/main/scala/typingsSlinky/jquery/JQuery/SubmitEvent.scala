...
object SubmitEvent {
  ...
  @scala.inline
  implicit class SubmitEventOps[Self <: SubmitEvent[_, _, _, _], TDelegateTarget, TData, TCurrentTarget, TTarget] (val x: Self with (SubmitEvent[TDelegateTarget, TData, TCurrentTarget, TTarget])) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def setType(value: submit): Self = this.set("type", value.asInstanceOf[js.Any])
  }
