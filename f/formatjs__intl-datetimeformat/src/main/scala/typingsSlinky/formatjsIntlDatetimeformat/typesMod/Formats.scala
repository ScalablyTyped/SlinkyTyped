...
/* Inlined std.Pick<@formatjs/intl-datetimeformat.@formatjs/intl-datetimeformat/src/types.DateTimeFormatOptions, 'weekday' | 'era' | 'year' | 'month' | 'day' | 'hour' | 'minute' | 'second' | 'timeZoneName'> & {  hour12 :boolean | undefined,   pattern :string,   pattern12 :string,   skeleton :string} */
@js.native
