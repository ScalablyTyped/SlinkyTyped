package typingsSlinky.formatjsIntlNumberformat

import typingsSlinky.formatjsIntlNumberformat.mod.DecimalFormatNum
import typingsSlinky.formatjsIntlNumberformat.mod.LDMLPluralRule
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatNotation
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsCompactDisplay
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsCurrencyDisplay
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsCurrencySign
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsLocaleMatcher
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsSignDisplay
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsStyle
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatOptionsUnitDisplay
import typingsSlinky.formatjsIntlNumberformat.mod.NumberFormatRoundingType
import typingsSlinky.formatjsIntlNumberformat.mod._NumberFormatPartTypes
import typingsSlinky.formatjsIntlNumberformat.unitsConstantsMod.Unit
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object formatjsIntlNumberformatStrings {
  @js.native
  sealed trait `1000` extends DecimalFormatNum
  
  @js.native
  sealed trait `10000` extends DecimalFormatNum
  
  @js.native
  sealed trait `100000` extends DecimalFormatNum
  
  @js.native
  sealed trait `1000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `10000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `100000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `1000000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `10000000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `100000000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `1000000000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `10000000000000` extends DecimalFormatNum
  
  @js.native
  sealed trait `100000000000000` extends DecimalFormatNum
  
  @js.native
  sealed trait accounting
    extends NumberFormatOptionsCurrencySign
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCurrencySign
  
  @js.native
  sealed trait acre extends Unit
  
  @js.native
  sealed trait always
    extends NumberFormatOptionsSignDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsSignDisplay
  
  @js.native
  sealed trait auto
    extends NumberFormatOptionsSignDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsSignDisplay
  
  @js.native
  sealed trait `best fit`
    extends NumberFormatOptionsLocaleMatcher
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsLocaleMatcher
  
  @js.native
  sealed trait bit extends Unit
  
  @js.native
  sealed trait byte extends Unit
  
  @js.native
  sealed trait celsius extends Unit
  
  @js.native
  sealed trait centimeter extends Unit
  
  @js.native
  sealed trait code
    extends NumberFormatOptionsCurrencyDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCurrencyDisplay
  
  @js.native
  sealed trait compact
    extends NumberFormatNotation
       with _NumberFormatPartTypes
       with typingsSlinky.formatjsIntlNumberformat.typesMod._NumberFormatPartTypes
  
  @js.native
  sealed trait compactRounding extends NumberFormatRoundingType
  
  @js.native
  sealed trait currency
    extends NumberFormatOptionsStyle
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsStyle
  
  @js.native
  sealed trait day extends Unit
  
  @js.native
  sealed trait decimal
    extends NumberFormatOptionsStyle
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsStyle
  
  @js.native
  sealed trait degree extends Unit
  
  @js.native
  sealed trait engineering extends NumberFormatNotation
  
  @js.native
  sealed trait exceptZero
    extends NumberFormatOptionsSignDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsSignDisplay
  
  @js.native
  sealed trait exponentInteger
    extends _NumberFormatPartTypes
       with typingsSlinky.formatjsIntlNumberformat.typesMod._NumberFormatPartTypes
  
  @js.native
  sealed trait exponentMinusSign
    extends _NumberFormatPartTypes
       with typingsSlinky.formatjsIntlNumberformat.typesMod._NumberFormatPartTypes
  
  @js.native
  sealed trait exponentSeparator
    extends _NumberFormatPartTypes
       with typingsSlinky.formatjsIntlNumberformat.typesMod._NumberFormatPartTypes
  
  @js.native
  sealed trait fahrenheit extends Unit
  
  @js.native
  sealed trait few extends LDMLPluralRule
  
  @js.native
  sealed trait `fluid-ounce` extends Unit
  
  @js.native
  sealed trait foot extends Unit
  
  @js.native
  sealed trait fractionDigits extends NumberFormatRoundingType
  
  @js.native
  sealed trait gallon extends Unit
  
  @js.native
  sealed trait gigabit extends Unit
  
  @js.native
  sealed trait gigabyte extends Unit
  
  @js.native
  sealed trait gram extends Unit
  
  @js.native
  sealed trait hectare extends Unit
  
  @js.native
  sealed trait hour extends Unit
  
  @js.native
  sealed trait inch extends Unit
  
  @js.native
  sealed trait kilobit extends Unit
  
  @js.native
  sealed trait kilobyte extends Unit
  
  @js.native
  sealed trait kilogram extends Unit
  
  @js.native
  sealed trait kilometer extends Unit
  
  @js.native
  sealed trait liter extends Unit
  
  @js.native
  sealed trait literal
    extends _NumberFormatPartTypes
       with typingsSlinky.formatjsIntlNumberformat.typesMod._NumberFormatPartTypes
  
  @js.native
  sealed trait long
    extends NumberFormatOptionsCompactDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCompactDisplay
       with NumberFormatOptionsUnitDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsUnitDisplay
  
  @js.native
  sealed trait lookup
    extends NumberFormatOptionsLocaleMatcher
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsLocaleMatcher
  
  @js.native
  sealed trait many extends LDMLPluralRule
  
  @js.native
  sealed trait megabit extends Unit
  
  @js.native
  sealed trait megabyte extends Unit
  
  @js.native
  sealed trait meter extends Unit
  
  @js.native
  sealed trait mile extends Unit
  
  @js.native
  sealed trait `mile-scandinavian` extends Unit
  
  @js.native
  sealed trait milliliter extends Unit
  
  @js.native
  sealed trait millimeter extends Unit
  
  @js.native
  sealed trait millisecond extends Unit
  
  @js.native
  sealed trait minute extends Unit
  
  @js.native
  sealed trait month extends Unit
  
  @js.native
  sealed trait name
    extends NumberFormatOptionsCurrencyDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCurrencyDisplay
  
  @js.native
  sealed trait narrow
    extends NumberFormatOptionsUnitDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsUnitDisplay
  
  @js.native
  sealed trait narrowSymbol
    extends NumberFormatOptionsCurrencyDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCurrencyDisplay
  
  @js.native
  sealed trait never
    extends NumberFormatOptionsSignDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsSignDisplay
  
  @js.native
  sealed trait one extends LDMLPluralRule
  
  @js.native
  sealed trait other extends LDMLPluralRule
  
  @js.native
  sealed trait ounce extends Unit
  
  @js.native
  sealed trait percent
    extends NumberFormatOptionsStyle
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsStyle
       with Unit
  
  @js.native
  sealed trait petabyte extends Unit
  
  @js.native
  sealed trait pound extends Unit
  
  @js.native
  sealed trait scientific extends NumberFormatNotation
  
  @js.native
  sealed trait second extends Unit
  
  @js.native
  sealed trait short
    extends NumberFormatOptionsCompactDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCompactDisplay
       with NumberFormatOptionsUnitDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsUnitDisplay
  
  @js.native
  sealed trait significantDigits extends NumberFormatRoundingType
  
  @js.native
  sealed trait standard
    extends NumberFormatNotation
       with NumberFormatOptionsCurrencySign
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCurrencySign
  
  @js.native
  sealed trait stone extends Unit
  
  @js.native
  sealed trait symbol
    extends NumberFormatOptionsCurrencyDisplay
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsCurrencyDisplay
  
  @js.native
  sealed trait terabit extends Unit
  
  @js.native
  sealed trait terabyte extends Unit
  
  @js.native
  sealed trait two extends LDMLPluralRule
  
  @js.native
  sealed trait unit
    extends NumberFormatOptionsStyle
       with typingsSlinky.formatjsIntlNumberformat.typesMod.NumberFormatOptionsStyle
       with _NumberFormatPartTypes
       with typingsSlinky.formatjsIntlNumberformat.typesMod._NumberFormatPartTypes
  
  @js.native
  sealed trait week extends Unit
  
  @js.native
  sealed trait yard extends Unit
  
  @js.native
  sealed trait year extends Unit
  
  @js.native
  sealed trait zero extends LDMLPluralRule
  
  @scala.inline
  def `1000`: `1000` = "1000".asInstanceOf[`1000`]
  @scala.inline
  def `10000`: `10000` = "10000".asInstanceOf[`10000`]
  @scala.inline
  def `100000`: `100000` = "100000".asInstanceOf[`100000`]
  @scala.inline
  def `1000000`: `1000000` = "1000000".asInstanceOf[`1000000`]
  @scala.inline
  def `10000000`: `10000000` = "10000000".asInstanceOf[`10000000`]
  @scala.inline
  def `100000000`: `100000000` = "100000000".asInstanceOf[`100000000`]
  @scala.inline
  def `1000000000`: `1000000000` = "1000000000".asInstanceOf[`1000000000`]
  @scala.inline
  def `10000000000`: `10000000000` = "10000000000".asInstanceOf[`10000000000`]
  @scala.inline
  def `100000000000`: `100000000000` = "100000000000".asInstanceOf[`100000000000`]
  @scala.inline
  def `1000000000000`: `1000000000000` = "1000000000000".asInstanceOf[`1000000000000`]
  @scala.inline
  def `10000000000000`: `10000000000000` = "10000000000000".asInstanceOf[`10000000000000`]
  @scala.inline
  def `100000000000000`: `100000000000000` = "100000000000000".asInstanceOf[`100000000000000`]
  @scala.inline
  def accounting: accounting = "accounting".asInstanceOf[accounting]
  @scala.inline
  def acre: acre = "acre".asInstanceOf[acre]
  @scala.inline
  def always: always = "always".asInstanceOf[always]
  @scala.inline
  def auto: auto = "auto".asInstanceOf[auto]
  @scala.inline
  def `best fit`: `best fit` = ("best fit").asInstanceOf[`best fit`]
  @scala.inline
  def bit: bit = "bit".asInstanceOf[bit]
  @scala.inline
  def byte: byte = "byte".asInstanceOf[byte]
  @scala.inline
  def celsius: celsius = "celsius".asInstanceOf[celsius]
  @scala.inline
  def centimeter: centimeter = "centimeter".asInstanceOf[centimeter]
  @scala.inline
  def code: code = "code".asInstanceOf[code]
  @scala.inline
  def compact: compact = "compact".asInstanceOf[compact]
  @scala.inline
  def compactRounding: compactRounding = "compactRounding".asInstanceOf[compactRounding]
  @scala.inline
  def currency: currency = "currency".asInstanceOf[currency]
  @scala.inline
  def day: day = "day".asInstanceOf[day]
  @scala.inline
  def decimal: decimal = "decimal".asInstanceOf[decimal]
  @scala.inline
  def degree: degree = "degree".asInstanceOf[degree]
  @scala.inline
  def engineering: engineering = "engineering".asInstanceOf[engineering]
  @scala.inline
  def exceptZero: exceptZero = "exceptZero".asInstanceOf[exceptZero]
  @scala.inline
  def exponentInteger: exponentInteger = "exponentInteger".asInstanceOf[exponentInteger]
  @scala.inline
  def exponentMinusSign: exponentMinusSign = "exponentMinusSign".asInstanceOf[exponentMinusSign]
  @scala.inline
  def exponentSeparator: exponentSeparator = "exponentSeparator".asInstanceOf[exponentSeparator]
  @scala.inline
  def fahrenheit: fahrenheit = "fahrenheit".asInstanceOf[fahrenheit]
  @scala.inline
  def few: few = "few".asInstanceOf[few]
  @scala.inline
  def `fluid-ounce`: `fluid-ounce` = "fluid-ounce".asInstanceOf[`fluid-ounce`]
  @scala.inline
  def foot: foot = "foot".asInstanceOf[foot]
  @scala.inline
  def fractionDigits: fractionDigits = "fractionDigits".asInstanceOf[fractionDigits]
  @scala.inline
  def gallon: gallon = "gallon".asInstanceOf[gallon]
  @scala.inline
  def gigabit: gigabit = "gigabit".asInstanceOf[gigabit]
  @scala.inline
  def gigabyte: gigabyte = "gigabyte".asInstanceOf[gigabyte]
  @scala.inline
  def gram: gram = "gram".asInstanceOf[gram]
  @scala.inline
  def hectare: hectare = "hectare".asInstanceOf[hectare]
  @scala.inline
  def hour: hour = "hour".asInstanceOf[hour]
  @scala.inline
  def inch: inch = "inch".asInstanceOf[inch]
  @scala.inline
  def kilobit: kilobit = "kilobit".asInstanceOf[kilobit]
  @scala.inline
  def kilobyte: kilobyte = "kilobyte".asInstanceOf[kilobyte]
  @scala.inline
  def kilogram: kilogram = "kilogram".asInstanceOf[kilogram]
  @scala.inline
  def kilometer: kilometer = "kilometer".asInstanceOf[kilometer]
  @scala.inline
  def liter: liter = "liter".asInstanceOf[liter]
  @scala.inline
  def literal: literal = "literal".asInstanceOf[literal]
  @scala.inline
  def long: long = "long".asInstanceOf[long]
  @scala.inline
  def lookup: lookup = "lookup".asInstanceOf[lookup]
  @scala.inline
  def many: many = "many".asInstanceOf[many]
  @scala.inline
  def megabit: megabit = "megabit".asInstanceOf[megabit]
  @scala.inline
  def megabyte: megabyte = "megabyte".asInstanceOf[megabyte]
  @scala.inline
  def meter: meter = "meter".asInstanceOf[meter]
  @scala.inline
  def mile: mile = "mile".asInstanceOf[mile]
  @scala.inline
  def `mile-scandinavian`: `mile-scandinavian` = "mile-scandinavian".asInstanceOf[`mile-scandinavian`]
  @scala.inline
  def milliliter: milliliter = "milliliter".asInstanceOf[milliliter]
  @scala.inline
  def millimeter: millimeter = "millimeter".asInstanceOf[millimeter]
  @scala.inline
  def millisecond: millisecond = "millisecond".asInstanceOf[millisecond]
  @scala.inline
  def minute: minute = "minute".asInstanceOf[minute]
  @scala.inline
  def month: month = "month".asInstanceOf[month]
  @scala.inline
  def name: name = "name".asInstanceOf[name]
  @scala.inline
  def narrow: narrow = "narrow".asInstanceOf[narrow]
  @scala.inline
  def narrowSymbol: narrowSymbol = "narrowSymbol".asInstanceOf[narrowSymbol]
  @scala.inline
  def never: never = "never".asInstanceOf[never]
  @scala.inline
  def one: one = "one".asInstanceOf[one]
  @scala.inline
  def other: other = "other".asInstanceOf[other]
  @scala.inline
  def ounce: ounce = "ounce".asInstanceOf[ounce]
  @scala.inline
  def percent: percent = "percent".asInstanceOf[percent]
  @scala.inline
  def petabyte: petabyte = "petabyte".asInstanceOf[petabyte]
  @scala.inline
  def pound: pound = "pound".asInstanceOf[pound]
  @scala.inline
  def scientific: scientific = "scientific".asInstanceOf[scientific]
  @scala.inline
  def second: second = "second".asInstanceOf[second]
  @scala.inline
  def short: short = "short".asInstanceOf[short]
  @scala.inline
  def significantDigits: significantDigits = "significantDigits".asInstanceOf[significantDigits]
  @scala.inline
  def standard: standard = "standard".asInstanceOf[standard]
  @scala.inline
  def stone: stone = "stone".asInstanceOf[stone]
  @scala.inline
  def symbol: symbol = "symbol".asInstanceOf[symbol]
  @scala.inline
  def terabit: terabit = "terabit".asInstanceOf[terabit]
  @scala.inline
  def terabyte: terabyte = "terabyte".asInstanceOf[terabyte]
  @scala.inline
  def two: two = "two".asInstanceOf[two]
  @scala.inline
  def unit: unit = "unit".asInstanceOf[unit]
  @scala.inline
  def week: week = "week".asInstanceOf[week]
  @scala.inline
  def yard: yard = "yard".asInstanceOf[yard]
  @scala.inline
  def year: year = "year".asInstanceOf[year]
  @scala.inline
  def zero: zero = "zero".asInstanceOf[zero]
}

