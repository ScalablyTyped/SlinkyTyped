...
object ZoomSettingsScope {
  ...
  @scala.inline
  def `per-origin`: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.`per-origin` = this.cast("per-origin")
  @scala.inline
  def `per-tab`: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.`per-tab` = this.cast("per-tab")
}
