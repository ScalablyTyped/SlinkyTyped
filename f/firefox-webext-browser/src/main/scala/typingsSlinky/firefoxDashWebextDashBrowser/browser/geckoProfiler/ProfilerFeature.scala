package typingsSlinky.firefoxDashWebextDashBrowser.browser.geckoProfiler

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* geckoProfiler types */
/* Rewritten from type alias, can be one of: 
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.java_
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.js_
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.leaf
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.mainthreadio
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.privacy
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.responsiveness
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.screenshots
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.seqstyle
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.stackwalk
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.tasktracer
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.threads
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.trackopts
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.jstracer
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.jsallocations
  - typings.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.preferencereads
*/
trait ProfilerFeature extends js.Object

object ProfilerFeature {
  @scala.inline
  /* private */ def cast[T](in: js.Any): T = in.asInstanceOf[T]
  @JSName("java")
  @scala.inline
  def java_ : typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.java_ = this.cast("java")
  @JSName("js")
  @scala.inline
  def js_ : typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.js_ = this.cast("js")
  @scala.inline
  def jsallocations: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.jsallocations = this.cast("jsallocations")
  @scala.inline
  def jstracer: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.jstracer = this.cast("jstracer")
  @scala.inline
  def leaf: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.leaf = this.cast("leaf")
  @scala.inline
  def mainthreadio: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.mainthreadio = this.cast("mainthreadio")
  @scala.inline
  def preferencereads: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.preferencereads = this.cast("preferencereads")
  @scala.inline
  def privacy: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.privacy = this.cast("privacy")
  @scala.inline
  def responsiveness: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.responsiveness = this.cast("responsiveness")
  @scala.inline
  def screenshots: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.screenshots = this.cast("screenshots")
  @scala.inline
  def seqstyle: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.seqstyle = this.cast("seqstyle")
  @scala.inline
  def stackwalk: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.stackwalk = this.cast("stackwalk")
  @scala.inline
  def tasktracer: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.tasktracer = this.cast("tasktracer")
  @scala.inline
  def threads: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.threads = this.cast("threads")
  @scala.inline
  def trackopts: typingsSlinky.firefoxDashWebextDashBrowser.firefoxDashWebextDashBrowserStrings.trackopts = this.cast("trackopts")
}

