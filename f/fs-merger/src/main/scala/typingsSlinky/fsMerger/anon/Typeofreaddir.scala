...
trait Typeofreaddir extends js.Object {
  ...
  def apply(
    path: PathLike,
    options: js.UndefOr[
      BaseEncodingOptionswithFi | BufferEncoding | Null | typingsSlinky.node.anon.WithFileTypes
    ],
  ...
  def __promisify__(path: PathLike, options: typingsSlinky.node.anon.Encoding): js.Promise[js.Array[Buffer]] = js.native
  def __promisify__(path: PathLike, options: typingsSlinky.node.anon.WithFileTypes): js.Promise[js.Array[String]] = js.native
  /**
    * Asynchronous readdir(3) - read a directory.
    * @param path A path to a file. If a URL is provided, it must use the `file:` protocol.
    * @param options The encoding (or an object specifying the encoding), used as the encoding of the result. If not provided, `'utf8'` is used.
