...
object SocketType {
  ...
  @scala.inline
  def `unix:dgram`: typingsSlinky.fridaDashGum.fridaDashGumStrings.`unix:dgram` = this.cast("unix:dgram")
  @scala.inline
  def `unix:stream`: typingsSlinky.fridaDashGum.fridaDashGumStrings.`unix:stream` = this.cast("unix:stream")
}
