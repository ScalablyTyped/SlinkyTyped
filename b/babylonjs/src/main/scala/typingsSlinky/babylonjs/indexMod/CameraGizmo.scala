package typingsSlinky.babylonjs.indexMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("babylonjs/index", "CameraGizmo")
@js.native
/**
  * Creates a CameraGizmo
  * @param gizmoLayer The utility layer the gizmo will be added to
  */
class CameraGizmo ()
  extends typingsSlinky.babylonjs.gizmosIndexMod.CameraGizmo {
  def this(gizmoLayer: typingsSlinky.babylonjs.utilityLayerRendererMod.UtilityLayerRenderer) = this()
}
/* static members */
object CameraGizmo {
  
  @JSImport("babylonjs/index", "CameraGizmo")
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("babylonjs/index", "CameraGizmo._CreateCameraFrustum")
  @js.native
  def _CreateCameraFrustum: js.Any = js.native
  @scala.inline
  def _CreateCameraFrustum_=(x: js.Any): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("_CreateCameraFrustum")(x.asInstanceOf[js.Any])
  
  @JSImport("babylonjs/index", "CameraGizmo._CreateCameraMesh")
  @js.native
  def _CreateCameraMesh: js.Any = js.native
  @scala.inline
  def _CreateCameraMesh_=(x: js.Any): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("_CreateCameraMesh")(x.asInstanceOf[js.Any])
  
  @JSImport("babylonjs/index", "CameraGizmo._Scale")
  @js.native
  def _Scale: js.Any = js.native
  @scala.inline
  def _Scale_=(x: js.Any): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("_Scale")(x.asInstanceOf[js.Any])
}
