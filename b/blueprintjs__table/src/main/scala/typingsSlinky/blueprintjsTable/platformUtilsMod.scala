package typingsSlinky.blueprintjsTable

import org.scalajs.dom.raw.KeyboardEvent
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object platformUtilsMod {
  
  @JSImport("@blueprintjs/table/lib/esm/common/internal/platformUtils", "isMac")
  @js.native
  def isMac(): Boolean = js.native
  @JSImport("@blueprintjs/table/lib/esm/common/internal/platformUtils", "isMac")
  @js.native
  def isMac(platformOverride: String): Boolean = js.native
  
  @JSImport("@blueprintjs/table/lib/esm/common/internal/platformUtils", "isModKeyPressed")
  @js.native
  def isModKeyPressed(event: KeyboardEvent): Boolean = js.native
  @JSImport("@blueprintjs/table/lib/esm/common/internal/platformUtils", "isModKeyPressed")
  @js.native
  def isModKeyPressed(event: KeyboardEvent, platformOverride: String): Boolean = js.native
}
