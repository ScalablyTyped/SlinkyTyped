package typingsSlinky.blueprintjsDatetime

import org.scalajs.dom.raw.HTMLElement
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object utilsMod {
  
  @JSImport("@blueprintjs/datetime/lib/esm/common/utils", "measureTextWidth")
  @js.native
  def measureTextWidth(text: String): Double = js.native
  @JSImport("@blueprintjs/datetime/lib/esm/common/utils", "measureTextWidth")
  @js.native
  def measureTextWidth(text: String, className: js.UndefOr[scala.Nothing], containerElement: HTMLElement): Double = js.native
  @JSImport("@blueprintjs/datetime/lib/esm/common/utils", "measureTextWidth")
  @js.native
  def measureTextWidth(text: String, className: String): Double = js.native
  @JSImport("@blueprintjs/datetime/lib/esm/common/utils", "measureTextWidth")
  @js.native
  def measureTextWidth(text: String, className: String, containerElement: HTMLElement): Double = js.native
  
  @JSImport("@blueprintjs/datetime/lib/esm/common/utils", "padWithZeroes")
  @js.native
  def padWithZeroes(str: String, minLength: Double): String = js.native
}
