...
import typingsSlinky.openapiTypes.mod.OpenAPIV2.SecuritySchemeObject
...
object mod {
  ...
  object OpenAPISecurityHandlerArgs {
    ...
    implicit class OpenAPISecurityHandlerArgsMutableBuilder[Self <: OpenAPISecurityHandlerArgs] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setOperationSecurityVarargs(
        value: (SecurityRequirementObject | typingsSlinky.openapiTypes.mod.OpenAPIV3.SecurityRequirementObject)*
      ): Self = StObject.set(x, "operationSecurity", js.Array(value :_*))
