package typingsSlinky.atOracleOraclejet.ojdatetimepickerMod

import org.scalablytyped.runtime.StringDictionary
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* import warning: RemoveDifficultInheritance.summarizeChanges 
- Dropped {[ P in keyof @oracle/oraclejet.@oracle/oraclejet/ojdatetimepicker.ojInputDateSettableProperties ]:? @oracle/oraclejet.@oracle/oraclejet/ojdatetimepicker.ojInputDateSettableProperties[P]} */ trait ojInputDateSettablePropertiesLenient
  extends /* key */ StringDictionary[js.Any]

object ojInputDateSettablePropertiesLenient {
  @scala.inline
  def apply(StringDictionary: /* key */ StringDictionary[js.Any] = null): ojInputDateSettablePropertiesLenient = {
    val __obj = js.Dynamic.literal()
    if (StringDictionary != null) js.Dynamic.global.Object.assign(__obj, StringDictionary)
    __obj.asInstanceOf[ojInputDateSettablePropertiesLenient]
  }
}

