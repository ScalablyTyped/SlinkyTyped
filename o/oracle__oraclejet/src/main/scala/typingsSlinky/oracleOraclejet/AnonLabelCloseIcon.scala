package typingsSlinky.oracleOraclejet

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@js.native
trait AnonLabelCloseIcon extends js.Object {
  var labelCloseIcon: js.UndefOr[String] = js.native
}

object AnonLabelCloseIcon {
  @scala.inline
  def apply(): AnonLabelCloseIcon = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[AnonLabelCloseIcon]
  }
  @scala.inline
  implicit class AnonLabelCloseIconOps[Self <: AnonLabelCloseIcon] (val x: Self) extends AnyVal {
    @scala.inline
    def duplicate: Self = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x)).asInstanceOf[Self]
    @scala.inline
    def combineWith[Other <: js.Any](other: Other): Self with Other = (js.Dynamic.global.Object.assign(js.Dynamic.literal(), x, other.asInstanceOf[js.Any])).asInstanceOf[Self with Other]
    @scala.inline
    def withLabelCloseIcon(value: String): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("labelCloseIcon")(value.asInstanceOf[js.Any])
        ret
    }
    @scala.inline
    def withoutLabelCloseIcon: Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("labelCloseIcon")(js.undefined)
        ret
    }
  }
  
}

