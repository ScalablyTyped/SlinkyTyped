package typingsSlinky.oracleOraclejet

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object ojloggerMod {
  
  @JSImport("@oracle/oraclejet/ojlogger", JSImport.Namespace)
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("@oracle/oraclejet/ojlogger", "LEVEL_ERROR")
  @js.native
  def LEVEL_ERROR: Double = js.native
  @scala.inline
  def LEVEL_ERROR_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEVEL_ERROR")(x.asInstanceOf[js.Any])
  
  @JSImport("@oracle/oraclejet/ojlogger", "LEVEL_INFO")
  @js.native
  def LEVEL_INFO: Double = js.native
  @scala.inline
  def LEVEL_INFO_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEVEL_INFO")(x.asInstanceOf[js.Any])
  
  @JSImport("@oracle/oraclejet/ojlogger", "LEVEL_LOG")
  @js.native
  def LEVEL_LOG: Double = js.native
  @scala.inline
  def LEVEL_LOG_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEVEL_LOG")(x.asInstanceOf[js.Any])
  
  @JSImport("@oracle/oraclejet/ojlogger", "LEVEL_NONE")
  @js.native
  def LEVEL_NONE: Double = js.native
  @scala.inline
  def LEVEL_NONE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEVEL_NONE")(x.asInstanceOf[js.Any])
  
  @JSImport("@oracle/oraclejet/ojlogger", "LEVEL_WARN")
  @js.native
  def LEVEL_WARN: Double = js.native
  @scala.inline
  def LEVEL_WARN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEVEL_WARN")(x.asInstanceOf[js.Any])
  
  @JSImport("@oracle/oraclejet/ojlogger", "error")
  @js.native
  def error(args: (js.Object | String | Double)*): Unit = js.native
  
  @JSImport("@oracle/oraclejet/ojlogger", "info")
  @js.native
  def info(args: (js.Object | String | Double)*): Unit = js.native
  
  @JSImport("@oracle/oraclejet/ojlogger", "log")
  @js.native
  def log(args: (js.Object | String | Double)*): Unit = js.native
  
  @JSImport("@oracle/oraclejet/ojlogger", "option")
  @js.native
  def option(): js.Any = js.native
  @JSImport("@oracle/oraclejet/ojlogger", "option")
  @js.native
  def option(key: js.UndefOr[scala.Nothing], value: js.Any): js.Any = js.native
  @JSImport("@oracle/oraclejet/ojlogger", "option")
  @js.native
  def option(key: String): js.Any = js.native
  @JSImport("@oracle/oraclejet/ojlogger", "option")
  @js.native
  def option(key: String, value: js.Any): js.Any = js.native
  @JSImport("@oracle/oraclejet/ojlogger", "option")
  @js.native
  def option(key: js.Object): js.Any = js.native
  @JSImport("@oracle/oraclejet/ojlogger", "option")
  @js.native
  def option(key: js.Object, value: js.Any): js.Any = js.native
  
  @JSImport("@oracle/oraclejet/ojlogger", "warn")
  @js.native
  def warn(args: (js.Object | String | Double)*): Unit = js.native
}
