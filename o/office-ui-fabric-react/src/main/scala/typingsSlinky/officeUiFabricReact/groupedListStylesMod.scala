package typingsSlinky.officeUiFabricReact

import typingsSlinky.officeUiFabricReact.groupedListTypesMod.IGroupedListStyleProps
import typingsSlinky.officeUiFabricReact.groupedListTypesMod.IGroupedListStyles
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object groupedListStylesMod {
  
  @JSImport("office-ui-fabric-react/lib/components/GroupedList/GroupedList.styles", "getStyles")
  @js.native
  def getStyles(props: IGroupedListStyleProps): IGroupedListStyles = js.native
}
