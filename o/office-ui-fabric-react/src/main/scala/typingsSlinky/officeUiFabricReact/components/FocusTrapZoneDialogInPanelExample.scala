package typingsSlinky.officeUiFabricReact.components

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object FocusTrapZoneDialogInPanelExample {
  @JSImport("office-ui-fabric-react/lib/components/FocusTrapZone/examples/FocusTrapZone.DialogInPanel.Example", "FocusTrapZoneDialogInPanelExample")
  @js.native
  object component extends js.Object
  
  def withProps(p: js.Object): SharedBuilder_Object_159303587[
    typingsSlinky.officeUiFabricReact.focusTrapZoneDialogInPanelExampleMod.FocusTrapZoneDialogInPanelExample
  ] = new SharedBuilder_Object_159303587[
    typingsSlinky.officeUiFabricReact.focusTrapZoneDialogInPanelExampleMod.FocusTrapZoneDialogInPanelExample
  ](js.Array(this.component, p.asInstanceOf[js.Any]))
  implicit def make(companion: FocusTrapZoneDialogInPanelExample.type): SharedBuilder_Object_159303587[
    typingsSlinky.officeUiFabricReact.focusTrapZoneDialogInPanelExampleMod.FocusTrapZoneDialogInPanelExample
  ] = new SharedBuilder_Object_159303587[
    typingsSlinky.officeUiFabricReact.focusTrapZoneDialogInPanelExampleMod.FocusTrapZoneDialogInPanelExample
  ](js.Array(this.component, js.Dictionary.empty))()
}

