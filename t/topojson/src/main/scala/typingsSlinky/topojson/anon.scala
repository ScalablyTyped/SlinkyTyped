...
object anon {
  ...
  trait Geometries extends StObject {
    ...
    var `type`: typingsSlinky.topojson.topojsonStrings.GeometryCollection = js.native
  }
  object Geometries {
    ...
    def apply(
      ...
      `type`: typingsSlinky.topojson.topojsonStrings.GeometryCollection
    ): Geometries = {
    ...
    implicit class GeometriesMutableBuilder[Self <: Geometries] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setType(value: typingsSlinky.topojson.topojsonStrings.GeometryCollection): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
    }
  }
}
