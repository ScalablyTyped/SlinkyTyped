package typingsSlinky.typescriptDashServices.typescriptDashServicesMod.Services

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("typescript-services", "Services.FindReferenceHelpers")
@js.native
class FindReferenceHelpers ()
  extends typingsSlinky.typescriptDashServices.TypeScript.Services.FindReferenceHelpers

/* static members */
@JSImport("typescript-services", "Services.FindReferenceHelpers")
@js.native
object FindReferenceHelpers extends js.Object {
  /* private */ def checkSymbolsForDeclarationEquality(firstSymbol: js.Any, secondSymbol: js.Any): js.Any = js.native
  def compareSymbolsForLexicalIdentity(
    firstSymbol: typingsSlinky.typescriptDashServices.TypeScript.PullSymbol,
    secondSymbol: typingsSlinky.typescriptDashServices.TypeScript.PullSymbol
  ): Boolean = js.native
  /* private */ def declarationsAreSameOrParents(firstDecl: js.Any, secondDecl: js.Any): js.Any = js.native
}

