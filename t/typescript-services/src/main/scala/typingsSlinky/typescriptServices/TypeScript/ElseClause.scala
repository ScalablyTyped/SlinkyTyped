...
object ElseClause {
  ...
  def apply(
    ...
    start: () => Double,
    statement: AST,
    structuralEquals: (ElseClause, Boolean) => Boolean,
    syntaxID: () => Double,
  ...
  implicit class ElseClauseOps[Self <: ElseClause] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setStructuralEquals(value: (ElseClause, Boolean) => Boolean): Self = this.set("structuralEquals", js.Any.fromFunction2(value))
  }
