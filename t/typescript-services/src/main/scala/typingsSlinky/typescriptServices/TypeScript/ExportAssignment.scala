...
@js.native
trait ExportAssignment extends AST {
...
object ExportAssignment {
  ...
  def apply(
    ...
    start: () => Double,
    structuralEquals: (ExportAssignment, Boolean) => Boolean,
    syntaxID: () => Double,
  ...
  @scala.inline
  implicit class ExportAssignmentOps[Self <: ExportAssignment] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setStructuralEquals(value: (ExportAssignment, Boolean) => Boolean): Self = this.set("structuralEquals", js.Any.fromFunction2(value))
  }
}
