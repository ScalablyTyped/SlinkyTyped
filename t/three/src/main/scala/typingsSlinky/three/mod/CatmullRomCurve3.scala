package typingsSlinky.three.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("three", "CatmullRomCurve3")
@js.native
/**
	 * @param [points=[]]
	 * @param [closed=false]
	 * @param [curveType='centripetal']
	 * @param [tension=0.5]
	 */
class CatmullRomCurve3 ()
  extends typingsSlinky.three.curvesMod.CatmullRomCurve3 {
  def this(points: js.Array[typingsSlinky.three.vector3Mod.Vector3]) = this()
  def this(points: js.UndefOr[scala.Nothing], closed: Boolean) = this()
  def this(points: js.Array[typingsSlinky.three.vector3Mod.Vector3], closed: Boolean) = this()
  def this(points: js.UndefOr[scala.Nothing], closed: js.UndefOr[scala.Nothing], curveType: String) = this()
  def this(points: js.UndefOr[scala.Nothing], closed: Boolean, curveType: String) = this()
  def this(
    points: js.Array[typingsSlinky.three.vector3Mod.Vector3],
    closed: js.UndefOr[scala.Nothing],
    curveType: String
  ) = this()
  def this(points: js.Array[typingsSlinky.three.vector3Mod.Vector3], closed: Boolean, curveType: String) = this()
  def this(
    points: js.UndefOr[scala.Nothing],
    closed: js.UndefOr[scala.Nothing],
    curveType: js.UndefOr[scala.Nothing],
    tension: Double
  ) = this()
  def this(
    points: js.UndefOr[scala.Nothing],
    closed: js.UndefOr[scala.Nothing],
    curveType: String,
    tension: Double
  ) = this()
  def this(
    points: js.UndefOr[scala.Nothing],
    closed: Boolean,
    curveType: js.UndefOr[scala.Nothing],
    tension: Double
  ) = this()
  def this(points: js.UndefOr[scala.Nothing], closed: Boolean, curveType: String, tension: Double) = this()
  def this(
    points: js.Array[typingsSlinky.three.vector3Mod.Vector3],
    closed: js.UndefOr[scala.Nothing],
    curveType: js.UndefOr[scala.Nothing],
    tension: Double
  ) = this()
  def this(
    points: js.Array[typingsSlinky.three.vector3Mod.Vector3],
    closed: js.UndefOr[scala.Nothing],
    curveType: String,
    tension: Double
  ) = this()
  def this(
    points: js.Array[typingsSlinky.three.vector3Mod.Vector3],
    closed: Boolean,
    curveType: js.UndefOr[scala.Nothing],
    tension: Double
  ) = this()
  def this(
    points: js.Array[typingsSlinky.three.vector3Mod.Vector3],
    closed: Boolean,
    curveType: String,
    tension: Double
  ) = this()
}
