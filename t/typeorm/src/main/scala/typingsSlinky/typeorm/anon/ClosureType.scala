...
object ClosureType {
  ...
  def apply(
    ...
    connection: typingsSlinky.typeorm.connectionMod.Connection,
  ...
  implicit class ClosureTypeOps[Self <: ClosureType] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setConnection(value: typingsSlinky.typeorm.connectionMod.Connection): Self = this.set("connection", value.asInstanceOf[js.Any])
