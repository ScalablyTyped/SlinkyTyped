package typingsSlinky.tensorflowTfjsLayers.exportsLayersMod

import typingsSlinky.tensorflowTfjsLayers.convolutionalMod.UpSampling2DLayerArgs
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("@tensorflow/tfjs-layers/dist/exports_layers", "upSampling2d")
@js.native
object upSampling2d extends js.Object {
  
  def apply(args: UpSampling2DLayerArgs): typingsSlinky.tensorflowTfjsLayers.topologyMod.Layer = js.native
}
