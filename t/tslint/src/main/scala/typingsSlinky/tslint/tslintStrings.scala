package typingsSlinky.tslint

import typingsSlinky.tslint.completedDocsRuleMod._DocType
import typingsSlinky.tslint.completedDocsRuleMod._Location
import typingsSlinky.tslint.completedDocsRuleMod._Privacy
import typingsSlinky.tslint.completedDocsRuleMod._Visibility
import typingsSlinky.tslint.formatterMod.ConsumerType
import typingsSlinky.tslint.libUtilsMod.Encoding
import typingsSlinky.tslint.ruleMod.RuleSeverity
import typingsSlinky.tslint.ruleMod.RuleType
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object tslintStrings {
  @js.native
  sealed trait NoInfer extends js.Object
  
  @js.native
  sealed trait `Shorthand method assignments have been disallowedDot` extends js.Object
  
  @js.native
  sealed trait `Shorthand property and method assignments have been disallowedDot` extends js.Object
  
  @js.native
  sealed trait `Shorthand property assignments have been disallowedDot` extends js.Object
  
  @js.native
  sealed trait Tildenil extends js.Object
  
  @js.native
  sealed trait after extends js.Object
  
  @js.native
  sealed trait all extends js.Object
  
  @js.native
  sealed trait before extends js.Object
  
  @js.native
  sealed trait classes extends _DocType
  
  @js.native
  sealed trait content extends js.Object
  
  @js.native
  sealed trait default extends js.Object
  
  @js.native
  sealed trait `enum-members` extends _DocType
  
  @js.native
  sealed trait enums extends _DocType
  
  @js.native
  sealed trait error extends RuleSeverity
  
  @js.native
  sealed trait existence extends js.Object
  
  @js.native
  sealed trait exported extends _Visibility
  
  @js.native
  sealed trait formatting extends RuleType
  
  @js.native
  sealed trait functionality extends RuleType
  
  @js.native
  sealed trait functions extends _DocType
  
  @js.native
  sealed trait human extends ConsumerType
  
  @js.native
  sealed trait instance extends _Location
  
  @js.native
  sealed trait interfaces extends _DocType
  
  @js.native
  sealed trait internal extends _Visibility
  
  @js.native
  sealed trait locations extends js.Object
  
  @js.native
  sealed trait machine extends ConsumerType
  
  @js.native
  sealed trait maintainability extends RuleType
  
  @js.native
  sealed trait methods extends _DocType
  
  @js.native
  sealed trait namespaces extends _DocType
  
  @js.native
  sealed trait none extends js.Object
  
  @js.native
  sealed trait `null` extends js.Object
  
  @js.native
  sealed trait off extends RuleSeverity
  
  @js.native
  sealed trait overloads extends js.Object
  
  @js.native
  sealed trait privacies extends js.Object
  
  @js.native
  sealed trait `private` extends _Privacy
  
  @js.native
  sealed trait properties extends _DocType
  
  @js.native
  sealed trait `protected` extends _Privacy
  
  @js.native
  sealed trait public extends _Privacy
  
  @js.native
  sealed trait static extends _Location
  
  @js.native
  sealed trait style extends RuleType
  
  @js.native
  sealed trait tags extends js.Object
  
  @js.native
  sealed trait tslintColonrecommended extends js.Object
  
  @js.native
  sealed trait tslintDotjson extends js.Object
  
  @js.native
  sealed trait types extends _DocType
  
  @js.native
  sealed trait typescript extends RuleType
  
  @js.native
  sealed trait undefined extends js.Object
  
  @js.native
  sealed trait utf16be extends Encoding
  
  @js.native
  sealed trait utf16le extends Encoding
  
  @js.native
  sealed trait utf8 extends Encoding
  
  @js.native
  sealed trait `utf8-bom` extends Encoding
  
  @js.native
  sealed trait variables extends _DocType
  
  @js.native
  sealed trait visibilities extends js.Object
  
  @js.native
  sealed trait warn extends js.Object
  
  @js.native
  sealed trait warning extends RuleSeverity
  
  @scala.inline
  def NoInfer: NoInfer = "NoInfer".asInstanceOf[NoInfer]
  @scala.inline
  def `Shorthand method assignments have been disallowedDot`: `Shorthand method assignments have been disallowedDot` = "Shorthand method assignments have been disallowed.".asInstanceOf[`Shorthand method assignments have been disallowedDot`]
  @scala.inline
  def `Shorthand property and method assignments have been disallowedDot`: `Shorthand property and method assignments have been disallowedDot` = "Shorthand property and method assignments have been disallowed.".asInstanceOf[`Shorthand property and method assignments have been disallowedDot`]
  @scala.inline
  def `Shorthand property assignments have been disallowedDot`: `Shorthand property assignments have been disallowedDot` = "Shorthand property assignments have been disallowed.".asInstanceOf[`Shorthand property assignments have been disallowedDot`]
  @scala.inline
  def Tildenil: Tildenil = "~nil".asInstanceOf[Tildenil]
  @scala.inline
  def after: after = "after".asInstanceOf[after]
  @scala.inline
  def all: all = "all".asInstanceOf[all]
  @scala.inline
  def before: before = "before".asInstanceOf[before]
  @scala.inline
  def classes: classes = "classes".asInstanceOf[classes]
  @scala.inline
  def content: content = "content".asInstanceOf[content]
  @scala.inline
  def default: default = "default".asInstanceOf[default]
  @scala.inline
  def `enum-members`: `enum-members` = "enum-members".asInstanceOf[`enum-members`]
  @scala.inline
  def enums: enums = "enums".asInstanceOf[enums]
  @scala.inline
  def error: error = "error".asInstanceOf[error]
  @scala.inline
  def existence: existence = "existence".asInstanceOf[existence]
  @scala.inline
  def exported: exported = "exported".asInstanceOf[exported]
  @scala.inline
  def formatting: formatting = "formatting".asInstanceOf[formatting]
  @scala.inline
  def functionality: functionality = "functionality".asInstanceOf[functionality]
  @scala.inline
  def functions: functions = "functions".asInstanceOf[functions]
  @scala.inline
  def human: human = "human".asInstanceOf[human]
  @scala.inline
  def instance: instance = "instance".asInstanceOf[instance]
  @scala.inline
  def interfaces: interfaces = "interfaces".asInstanceOf[interfaces]
  @scala.inline
  def internal: internal = "internal".asInstanceOf[internal]
  @scala.inline
  def locations: locations = "locations".asInstanceOf[locations]
  @scala.inline
  def machine: machine = "machine".asInstanceOf[machine]
  @scala.inline
  def maintainability: maintainability = "maintainability".asInstanceOf[maintainability]
  @scala.inline
  def methods: methods = "methods".asInstanceOf[methods]
  @scala.inline
  def namespaces: namespaces = "namespaces".asInstanceOf[namespaces]
  @scala.inline
  def none: none = "none".asInstanceOf[none]
  @scala.inline
  def `null`: `null` = "null".asInstanceOf[`null`]
  @scala.inline
  def off: off = "off".asInstanceOf[off]
  @scala.inline
  def overloads: overloads = "overloads".asInstanceOf[overloads]
  @scala.inline
  def privacies: privacies = "privacies".asInstanceOf[privacies]
  @scala.inline
  def `private`: `private` = "private".asInstanceOf[`private`]
  @scala.inline
  def properties: properties = "properties".asInstanceOf[properties]
  @scala.inline
  def `protected`: `protected` = "protected".asInstanceOf[`protected`]
  @scala.inline
  def public: public = "public".asInstanceOf[public]
  @scala.inline
  def static: static = "static".asInstanceOf[static]
  @scala.inline
  def style: style = "style".asInstanceOf[style]
  @scala.inline
  def tags: tags = "tags".asInstanceOf[tags]
  @scala.inline
  def tslintColonrecommended: tslintColonrecommended = "tslint:recommended".asInstanceOf[tslintColonrecommended]
  @scala.inline
  def tslintDotjson: tslintDotjson = "tslint.json".asInstanceOf[tslintDotjson]
  @scala.inline
  def types: types = "types".asInstanceOf[types]
  @scala.inline
  def typescript: typescript = "typescript".asInstanceOf[typescript]
  @scala.inline
  def undefined: undefined = "undefined".asInstanceOf[undefined]
  @scala.inline
  def utf16be: utf16be = "utf16be".asInstanceOf[utf16be]
  @scala.inline
  def utf16le: utf16le = "utf16le".asInstanceOf[utf16le]
  @scala.inline
  def utf8: utf8 = "utf8".asInstanceOf[utf8]
  @scala.inline
  def `utf8-bom`: `utf8-bom` = "utf8-bom".asInstanceOf[`utf8-bom`]
  @scala.inline
  def variables: variables = "variables".asInstanceOf[variables]
  @scala.inline
  def visibilities: visibilities = "visibilities".asInstanceOf[visibilities]
  @scala.inline
  def warn: warn = "warn".asInstanceOf[warn]
  @scala.inline
  def warning: warning = "warning".asInstanceOf[warning]
}

