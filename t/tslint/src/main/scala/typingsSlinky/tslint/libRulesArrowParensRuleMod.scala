package typingsSlinky.tslint

import typingsSlinky.tslint.libLanguageRuleRuleMod.IRuleMetadata
import typingsSlinky.tslint.tslintMod.Rules.AbstractRule
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("tslint/lib/rules/arrowParensRule", JSImport.Namespace)
@js.native
object libRulesArrowParensRuleMod extends js.Object {
  @js.native
  class Rule () extends AbstractRule
  
  /* static members */
  @js.native
  object Rule extends js.Object {
    var FAILURE_STRING_EXISTS: String = js.native
    var FAILURE_STRING_MISSING: String = js.native
    var metadata: IRuleMetadata = js.native
  }
  
}

