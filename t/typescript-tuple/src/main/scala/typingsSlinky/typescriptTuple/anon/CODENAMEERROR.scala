...
object CODENAMEERROR {
  ...
  implicit class CODENAMEERRORMutableBuilder[Self <: CODENAMEERROR] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setCODENAME(value: InfiniteTupleSet with typingsSlinky.typescriptTuple.typescriptTupleStrings.Infinite): Self = StObject.set(x, "CODENAME", value.asInstanceOf[js.Any])
