...
object ESMap {
  ...
  @scala.inline
  implicit class ESMapOps[Self <: ESMap[_, _], K, V] (val x: Self with (ESMap[K, V])) extends AnyVal {
    ...
    @scala.inline
    def setSet(value: (K, V) => ESMap[K, V]): Self = this.set("set", js.Any.fromFunction2(value))
  }
}
