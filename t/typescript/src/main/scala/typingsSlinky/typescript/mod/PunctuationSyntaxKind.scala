package typingsSlinky.typescript.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typingsSlinky.typescript.mod.SyntaxKind.OpenBraceToken
  - typingsSlinky.typescript.mod.SyntaxKind.CloseBraceToken
  - typingsSlinky.typescript.mod.SyntaxKind.OpenParenToken
  - typingsSlinky.typescript.mod.SyntaxKind.CloseParenToken
  - typingsSlinky.typescript.mod.SyntaxKind.OpenBracketToken
  - typingsSlinky.typescript.mod.SyntaxKind.CloseBracketToken
  - typingsSlinky.typescript.mod.SyntaxKind.DotToken
  - typingsSlinky.typescript.mod.SyntaxKind.DotDotDotToken
  - typingsSlinky.typescript.mod.SyntaxKind.SemicolonToken
  - typingsSlinky.typescript.mod.SyntaxKind.CommaToken
  - typingsSlinky.typescript.mod.SyntaxKind.QuestionDotToken
  - typingsSlinky.typescript.mod.SyntaxKind.LessThanToken
  - typingsSlinky.typescript.mod.SyntaxKind.LessThanSlashToken
  - typingsSlinky.typescript.mod.SyntaxKind.GreaterThanToken
  - typingsSlinky.typescript.mod.SyntaxKind.LessThanEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.GreaterThanEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.EqualsEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.ExclamationEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.EqualsEqualsEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.ExclamationEqualsEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.EqualsGreaterThanToken
  - typingsSlinky.typescript.mod.SyntaxKind.PlusToken
  - typingsSlinky.typescript.mod.SyntaxKind.MinusToken
  - typingsSlinky.typescript.mod.SyntaxKind.AsteriskToken
  - typingsSlinky.typescript.mod.SyntaxKind.AsteriskAsteriskToken
  - typingsSlinky.typescript.mod.SyntaxKind.SlashToken
  - typingsSlinky.typescript.mod.SyntaxKind.PercentToken
  - typingsSlinky.typescript.mod.SyntaxKind.PlusPlusToken
  - typingsSlinky.typescript.mod.SyntaxKind.MinusMinusToken
  - typingsSlinky.typescript.mod.SyntaxKind.LessThanLessThanToken
  - typingsSlinky.typescript.mod.SyntaxKind.GreaterThanGreaterThanToken
  - typingsSlinky.typescript.mod.SyntaxKind.GreaterThanGreaterThanGreaterThanToken
  - typingsSlinky.typescript.mod.SyntaxKind.AmpersandToken
  - typingsSlinky.typescript.mod.SyntaxKind.BarToken
  - typingsSlinky.typescript.mod.SyntaxKind.CaretToken
  - typingsSlinky.typescript.mod.SyntaxKind.ExclamationToken
  - typingsSlinky.typescript.mod.SyntaxKind.TildeToken
  - typingsSlinky.typescript.mod.SyntaxKind.AmpersandAmpersandToken
  - typingsSlinky.typescript.mod.SyntaxKind.BarBarToken
  - typingsSlinky.typescript.mod.SyntaxKind.QuestionQuestionToken
  - typingsSlinky.typescript.mod.SyntaxKind.QuestionToken
  - typingsSlinky.typescript.mod.SyntaxKind.ColonToken
  - typingsSlinky.typescript.mod.SyntaxKind.AtToken
  - typingsSlinky.typescript.mod.SyntaxKind.BacktickToken
  - typingsSlinky.typescript.mod.SyntaxKind.EqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.PlusEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.MinusEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.AsteriskEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.AsteriskAsteriskEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.SlashEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.PercentEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.LessThanLessThanEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.GreaterThanGreaterThanEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.GreaterThanGreaterThanGreaterThanEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.AmpersandEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.BarEqualsToken
  - typingsSlinky.typescript.mod.SyntaxKind.CaretEqualsToken
*/
trait PunctuationSyntaxKind extends TokenSyntaxKind
