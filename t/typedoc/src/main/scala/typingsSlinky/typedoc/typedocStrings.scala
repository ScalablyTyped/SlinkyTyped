package typingsSlinky.typedoc

import typingsSlinky.typedoc.typescriptMod.IgnoredTsOptionKeys
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object typedocStrings {
  
  @js.native
  sealed trait BindOption extends StObject
  @scala.inline
  def BindOption: BindOption = "BindOption".asInstanceOf[BindOption]
  
  @js.native
  sealed trait S extends StObject
  @scala.inline
  def S: S = "S".asInstanceOf[S]
  
  @js.native
  sealed trait boolean extends StObject
  @scala.inline
  def boolean: boolean = "boolean".asInstanceOf[boolean]
  
  @js.native
  sealed trait categorizeByGroup extends StObject
  @scala.inline
  def categorizeByGroup: categorizeByGroup = "categorizeByGroup".asInstanceOf[categorizeByGroup]
  
  @js.native
  sealed trait categoryOrder extends StObject
  @scala.inline
  def categoryOrder: categoryOrder = "categoryOrder".asInstanceOf[categoryOrder]
  
  @js.native
  sealed trait declaration extends IgnoredTsOptionKeys
  @scala.inline
  def declaration: declaration = "declaration".asInstanceOf[declaration]
  
  @js.native
  sealed trait declarationDir extends IgnoredTsOptionKeys
  @scala.inline
  def declarationDir: declarationDir = "declarationDir".asInstanceOf[declarationDir]
  
  @js.native
  sealed trait declarationMap extends IgnoredTsOptionKeys
  @scala.inline
  def declarationMap: declarationMap = "declarationMap".asInstanceOf[declarationMap]
  
  @js.native
  sealed trait defaultCategory extends StObject
  @scala.inline
  def defaultCategory: defaultCategory = "defaultCategory".asInstanceOf[defaultCategory]
  
  @js.native
  sealed trait disableOutputCheck extends StObject
  @scala.inline
  def disableOutputCheck: disableOutputCheck = "disableOutputCheck".asInstanceOf[disableOutputCheck]
  
  @js.native
  sealed trait disableSources extends StObject
  @scala.inline
  def disableSources: disableSources = "disableSources".asInstanceOf[disableSources]
  
  @js.native
  sealed trait dts extends StObject
  @scala.inline
  def dts: dts = "dts".asInstanceOf[dts]
  
  @js.native
  sealed trait emitDeclarationOnly extends IgnoredTsOptionKeys
  @scala.inline
  def emitDeclarationOnly: emitDeclarationOnly = "emitDeclarationOnly".asInstanceOf[emitDeclarationOnly]
  
  @js.native
  sealed trait entryPoint extends StObject
  @scala.inline
  def entryPoint: entryPoint = "entryPoint".asInstanceOf[entryPoint]
  
  @js.native
  sealed trait exclude extends StObject
  @scala.inline
  def exclude: exclude = "exclude".asInstanceOf[exclude]
  
  @js.native
  sealed trait excludeExternals extends StObject
  @scala.inline
  def excludeExternals: excludeExternals = "excludeExternals".asInstanceOf[excludeExternals]
  
  @js.native
  sealed trait excludeNotDocumented extends StObject
  @scala.inline
  def excludeNotDocumented: excludeNotDocumented = "excludeNotDocumented".asInstanceOf[excludeNotDocumented]
  
  @js.native
  sealed trait excludeNotExported extends StObject
  @scala.inline
  def excludeNotExported: excludeNotExported = "excludeNotExported".asInstanceOf[excludeNotExported]
  
  @js.native
  sealed trait excludePrivate extends StObject
  @scala.inline
  def excludePrivate: excludePrivate = "excludePrivate".asInstanceOf[excludePrivate]
  
  @js.native
  sealed trait excludeProtected extends StObject
  @scala.inline
  def excludeProtected: excludeProtected = "excludeProtected".asInstanceOf[excludeProtected]
  
  @js.native
  sealed trait excludeTags extends StObject
  @scala.inline
  def excludeTags: excludeTags = "excludeTags".asInstanceOf[excludeTags]
  
  @js.native
  sealed trait externalPattern extends StObject
  @scala.inline
  def externalPattern: externalPattern = "externalPattern".asInstanceOf[externalPattern]
  
  @js.native
  sealed trait function extends StObject
  @scala.inline
  def function: function = "function".asInstanceOf[function]
  
  @js.native
  sealed trait gaID extends StObject
  @scala.inline
  def gaID: gaID = "gaID".asInstanceOf[gaID]
  
  @js.native
  sealed trait gaSite extends StObject
  @scala.inline
  def gaSite: gaSite = "gaSite".asInstanceOf[gaSite]
  
  @js.native
  sealed trait gitRemote extends StObject
  @scala.inline
  def gitRemote: gitRemote = "gitRemote".asInstanceOf[gitRemote]
  
  @js.native
  sealed trait gitRevision extends StObject
  @scala.inline
  def gitRevision: gitRevision = "gitRevision".asInstanceOf[gitRevision]
  
  @js.native
  sealed trait help extends IgnoredTsOptionKeys
  @scala.inline
  def help: help = "help".asInstanceOf[help]
  
  @js.native
  sealed trait hideGenerator extends StObject
  @scala.inline
  def hideGenerator: hideGenerator = "hideGenerator".asInstanceOf[hideGenerator]
  
  @js.native
  sealed trait ignoreCompilerErrors extends StObject
  @scala.inline
  def ignoreCompilerErrors: ignoreCompilerErrors = "ignoreCompilerErrors".asInstanceOf[ignoreCompilerErrors]
  
  @js.native
  sealed trait includeDeclarations extends StObject
  @scala.inline
  def includeDeclarations: includeDeclarations = "includeDeclarations".asInstanceOf[includeDeclarations]
  
  @js.native
  sealed trait includeVersion extends StObject
  @scala.inline
  def includeVersion: includeVersion = "includeVersion".asInstanceOf[includeVersion]
  
  @js.native
  sealed trait includes extends StObject
  @scala.inline
  def includes: includes = "includes".asInstanceOf[includes]
  
  @js.native
  sealed trait incremental extends IgnoredTsOptionKeys
  @scala.inline
  def incremental: incremental = "incremental".asInstanceOf[incremental]
  
  @js.native
  sealed trait inlineSources extends IgnoredTsOptionKeys
  @scala.inline
  def inlineSources: inlineSources = "inlineSources".asInstanceOf[inlineSources]
  
  @js.native
  sealed trait inputFiles extends StObject
  @scala.inline
  def inputFiles: inputFiles = "inputFiles".asInstanceOf[inputFiles]
  
  @js.native
  sealed trait js_ extends StObject
  @scala.inline
  def js_ : js_ = "js".asInstanceOf[js_]
  
  @js.native
  sealed trait json extends StObject
  @scala.inline
  def json: json = "json".asInstanceOf[json]
  
  @js.native
  sealed trait keyof extends StObject
  @scala.inline
  def keyof: keyof = "keyof".asInstanceOf[keyof]
  
  @js.native
  sealed trait list extends StObject
  @scala.inline
  def list: list = "list".asInstanceOf[list]
  
  @js.native
  sealed trait listInvalidSymbolLinks extends StObject
  @scala.inline
  def listInvalidSymbolLinks: listInvalidSymbolLinks = "listInvalidSymbolLinks".asInstanceOf[listInvalidSymbolLinks]
  
  @js.native
  sealed trait logger extends StObject
  @scala.inline
  def logger: logger = "logger".asInstanceOf[logger]
  
  @js.native
  sealed trait mapRoot extends IgnoredTsOptionKeys
  @scala.inline
  def mapRoot: mapRoot = "mapRoot".asInstanceOf[mapRoot]
  
  @js.native
  sealed trait media extends StObject
  @scala.inline
  def media: media = "media".asInstanceOf[media]
  
  @js.native
  sealed trait mode extends StObject
  @scala.inline
  def mode: mode = "mode".asInstanceOf[mode]
  
  @js.native
  sealed trait name extends StObject
  @scala.inline
  def name: name = "name".asInstanceOf[name]
  
  @js.native
  sealed trait number extends StObject
  @scala.inline
  def number: number = "number".asInstanceOf[number]
  
  @js.native
  sealed trait `object` extends StObject
  @scala.inline
  def `object`: `object` = "object".asInstanceOf[`object`]
  
  @js.native
  sealed trait options extends StObject
  @scala.inline
  def options: options = "options".asInstanceOf[options]
  
  @js.native
  sealed trait out extends IgnoredTsOptionKeys
  @scala.inline
  def out: out = "out".asInstanceOf[out]
  
  @js.native
  sealed trait plugin extends StObject
  @scala.inline
  def plugin: plugin = "plugin".asInstanceOf[plugin]
  
  @js.native
  sealed trait quit extends StObject
  @scala.inline
  def quit: quit = "quit".asInstanceOf[quit]
  
  @js.native
  sealed trait readme extends StObject
  @scala.inline
  def readme: readme = "readme".asInstanceOf[readme]
  
  @js.native
  sealed trait readonly extends StObject
  @scala.inline
  def readonly: readonly = "readonly".asInstanceOf[readonly]
  
  @js.native
  sealed trait removeComments extends IgnoredTsOptionKeys
  @scala.inline
  def removeComments: removeComments = "removeComments".asInstanceOf[removeComments]
  
  @js.native
  sealed trait sourceMap extends IgnoredTsOptionKeys
  @scala.inline
  def sourceMap: sourceMap = "sourceMap".asInstanceOf[sourceMap]
  
  @js.native
  sealed trait string extends StObject
  @scala.inline
  def string: string = "string".asInstanceOf[string]
  
  @js.native
  sealed trait theme extends StObject
  @scala.inline
  def theme: theme = "theme".asInstanceOf[theme]
  
  @js.native
  sealed trait toc extends StObject
  @scala.inline
  def toc: toc = "toc".asInstanceOf[toc]
  
  @js.native
  sealed trait tsBuildInfoFile extends IgnoredTsOptionKeys
  @scala.inline
  def tsBuildInfoFile: tsBuildInfoFile = "tsBuildInfoFile".asInstanceOf[tsBuildInfoFile]
  
  @js.native
  sealed trait tsconfig extends StObject
  @scala.inline
  def tsconfig: tsconfig = "tsconfig".asInstanceOf[tsconfig]
  
  @js.native
  sealed trait unique extends StObject
  @scala.inline
  def unique: unique = "unique".asInstanceOf[unique]
  
  @js.native
  sealed trait version extends IgnoredTsOptionKeys
  @scala.inline
  def version: version = "version".asInstanceOf[version]
  
  @js.native
  sealed trait watch extends IgnoredTsOptionKeys
  @scala.inline
  def watch: watch = "watch".asInstanceOf[watch]
}
