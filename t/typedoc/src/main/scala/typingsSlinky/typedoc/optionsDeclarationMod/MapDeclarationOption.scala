...
trait MapDeclarationOption[T] extends DeclarationOptionBase {
  ...
  @JSName("type")
  var type_MapDeclarationOption: typingsSlinky.typedoc.optionsDeclarationMod.ParameterType.Map = js.native
}
object MapDeclarationOption {
  ...
  def apply[T](
    ...
    name: String,
    `type`: typingsSlinky.typedoc.optionsDeclarationMod.ParameterType.Map
  ...
  implicit class MapDeclarationOptionMutableBuilder[Self <: MapDeclarationOption[_], T] (val x: Self with MapDeclarationOption[T]) extends AnyVal {
    ...
    @scala.inline
    def setType(value: typingsSlinky.typedoc.optionsDeclarationMod.ParameterType.Map): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
