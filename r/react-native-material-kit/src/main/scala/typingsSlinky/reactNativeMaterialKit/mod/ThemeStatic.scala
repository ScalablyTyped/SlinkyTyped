package typingsSlinky.reactNativeMaterialKit.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait ThemeStatic extends js.Object {
  var accentColor: js.UndefOr[String] = js.undefined
  var accentColorRGB: js.UndefOr[String] = js.undefined
  var bgDisabled: js.UndefOr[String] = js.undefined
  var bgPlain: js.UndefOr[String] = js.undefined
  var cardActionStyle: js.UndefOr[CardActionStyle] = js.undefined
  var cardContentStyle: js.UndefOr[CardContentStyle] = js.undefined
  var cardImageStyle: js.UndefOr[CardImageStyle] = js.undefined
  var cardMenuStyle: js.UndefOr[CardMenuStyle] = js.undefined
  var cardStyle: js.UndefOr[CardStyle] = js.undefined
  var cardTitleStyle: js.UndefOr[CardTitleStyle] = js.undefined
  var checkboxStyle: js.UndefOr[CheckboxStyle] = js.undefined
  var fontColor: js.UndefOr[String] = js.undefined
  var fontSize: js.UndefOr[Double] = js.undefined
  var iconToggleStyle: js.UndefOr[IconToggleStyle] = js.undefined
  var primaryColor: js.UndefOr[String] = js.undefined
  var primaryColorRGB: js.UndefOr[String] = js.undefined
  var progressStyle: js.UndefOr[ProgressStyle] = js.undefined
  var radioStyle: js.UndefOr[RadioStyle] = js.undefined
  var rippleColor: js.UndefOr[String] = js.undefined
  var sliderStyle: js.UndefOr[SliderStyle] = js.undefined
  var spinnerStyle: js.UndefOr[SpinnerStyle] = js.undefined
  var switchStyle: js.UndefOr[SwitchStyle] = js.undefined
  var textfieldStyle: js.UndefOr[TextFieldStyle] = js.undefined
}

object ThemeStatic {
  @scala.inline
  def apply(
    accentColor: String = null,
    accentColorRGB: String = null,
    bgDisabled: String = null,
    bgPlain: String = null,
    cardActionStyle: CardActionStyle = null,
    cardContentStyle: CardContentStyle = null,
    cardImageStyle: CardImageStyle = null,
    cardMenuStyle: CardMenuStyle = null,
    cardStyle: CardStyle = null,
    cardTitleStyle: CardTitleStyle = null,
    checkboxStyle: CheckboxStyle = null,
    fontColor: String = null,
    fontSize: js.UndefOr[Double] = js.undefined,
    iconToggleStyle: IconToggleStyle = null,
    primaryColor: String = null,
    primaryColorRGB: String = null,
    progressStyle: ProgressStyle = null,
    radioStyle: RadioStyle = null,
    rippleColor: String = null,
    sliderStyle: SliderStyle = null,
    spinnerStyle: SpinnerStyle = null,
    switchStyle: SwitchStyle = null,
    textfieldStyle: TextFieldStyle = null
  ): ThemeStatic = {
    val __obj = js.Dynamic.literal()
    if (accentColor != null) __obj.updateDynamic("accentColor")(accentColor.asInstanceOf[js.Any])
    if (accentColorRGB != null) __obj.updateDynamic("accentColorRGB")(accentColorRGB.asInstanceOf[js.Any])
    if (bgDisabled != null) __obj.updateDynamic("bgDisabled")(bgDisabled.asInstanceOf[js.Any])
    if (bgPlain != null) __obj.updateDynamic("bgPlain")(bgPlain.asInstanceOf[js.Any])
    if (cardActionStyle != null) __obj.updateDynamic("cardActionStyle")(cardActionStyle.asInstanceOf[js.Any])
    if (cardContentStyle != null) __obj.updateDynamic("cardContentStyle")(cardContentStyle.asInstanceOf[js.Any])
    if (cardImageStyle != null) __obj.updateDynamic("cardImageStyle")(cardImageStyle.asInstanceOf[js.Any])
    if (cardMenuStyle != null) __obj.updateDynamic("cardMenuStyle")(cardMenuStyle.asInstanceOf[js.Any])
    if (cardStyle != null) __obj.updateDynamic("cardStyle")(cardStyle.asInstanceOf[js.Any])
    if (cardTitleStyle != null) __obj.updateDynamic("cardTitleStyle")(cardTitleStyle.asInstanceOf[js.Any])
    if (checkboxStyle != null) __obj.updateDynamic("checkboxStyle")(checkboxStyle.asInstanceOf[js.Any])
    if (fontColor != null) __obj.updateDynamic("fontColor")(fontColor.asInstanceOf[js.Any])
    if (!js.isUndefined(fontSize)) __obj.updateDynamic("fontSize")(fontSize.get.asInstanceOf[js.Any])
    if (iconToggleStyle != null) __obj.updateDynamic("iconToggleStyle")(iconToggleStyle.asInstanceOf[js.Any])
    if (primaryColor != null) __obj.updateDynamic("primaryColor")(primaryColor.asInstanceOf[js.Any])
    if (primaryColorRGB != null) __obj.updateDynamic("primaryColorRGB")(primaryColorRGB.asInstanceOf[js.Any])
    if (progressStyle != null) __obj.updateDynamic("progressStyle")(progressStyle.asInstanceOf[js.Any])
    if (radioStyle != null) __obj.updateDynamic("radioStyle")(radioStyle.asInstanceOf[js.Any])
    if (rippleColor != null) __obj.updateDynamic("rippleColor")(rippleColor.asInstanceOf[js.Any])
    if (sliderStyle != null) __obj.updateDynamic("sliderStyle")(sliderStyle.asInstanceOf[js.Any])
    if (spinnerStyle != null) __obj.updateDynamic("spinnerStyle")(spinnerStyle.asInstanceOf[js.Any])
    if (switchStyle != null) __obj.updateDynamic("switchStyle")(switchStyle.asInstanceOf[js.Any])
    if (textfieldStyle != null) __obj.updateDynamic("textfieldStyle")(textfieldStyle.asInstanceOf[js.Any])
    __obj.asInstanceOf[ThemeStatic]
  }
}

