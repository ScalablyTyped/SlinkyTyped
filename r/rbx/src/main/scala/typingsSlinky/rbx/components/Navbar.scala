...
/* The props of this component has an unsupported shape. You can use `set` manually to use it, but with no compiler support :/ . Couldn't find props for typingsSlinky.rbx.anon.`7`[TAsComponent] with typingsSlinky.rbx.anon.PickNavbarContainerPropsf with (typingsSlinky.std.Pick[
...
object Navbar {
  ...
  def apply[TAsComponent <: ReactElement](
    p: `7`[TAsComponent] with PickNavbarContainerPropsf with (Pick[
