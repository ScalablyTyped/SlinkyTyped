...
/* The props of this component has an unsupported shape. You can use `set` manually to use it, but with no compiler support :/ . Couldn't find props for typingsSlinky.rbx.anon.`7`[TAsComponent] with typingsSlinky.rbx.anon.PickNavbarItemContainerPrActive with (typingsSlinky.std.Pick[
...
object NavbarItem {
  ...
  def apply[TAsComponent <: ReactElement](
    p: `7`[TAsComponent] with PickNavbarItemContainerPrActive with (Pick[
  ...
  implicit def make[TAsComponent <: ReactElement](companion: NavbarItem.type): Builder[TAsComponent] = new Builder[TAsComponent](js.Array(this.component, js.Dictionary.empty))()
}
