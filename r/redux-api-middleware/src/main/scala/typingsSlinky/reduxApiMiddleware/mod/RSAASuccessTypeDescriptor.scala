...
object RSAASuccessTypeDescriptor {
  ...
  @scala.inline
  implicit class RSAASuccessTypeDescriptorOps[Self[state, payload, meta] <: RSAASuccessTypeDescriptor[state, payload, meta], State, Payload, Meta] (val x: Self[State, Payload, Meta]) extends AnyVal {
    @scala.inline
