package typingsSlinky.reactNativePaypalLib

import typingsSlinky.reactNativePaypalLib.mod.CURRENCIES
import typingsSlinky.reactNativePaypalLib.mod.ENVIRONMENT
import typingsSlinky.reactNativePaypalLib.mod.INTENT
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object reactNativePaypalLibStrings {
  
  @js.native
  sealed trait AUD extends CURRENCIES
  @scala.inline
  def AUD: AUD = "AUD".asInstanceOf[AUD]
  
  @js.native
  sealed trait AUTHORIZE extends INTENT
  @scala.inline
  def AUTHORIZE: AUTHORIZE = "AUTHORIZE".asInstanceOf[AUTHORIZE]
  
  @js.native
  sealed trait BRL extends CURRENCIES
  @scala.inline
  def BRL: BRL = "BRL".asInstanceOf[BRL]
  
  @js.native
  sealed trait CAD extends CURRENCIES
  @scala.inline
  def CAD: CAD = "CAD".asInstanceOf[CAD]
  
  @js.native
  sealed trait CHF extends CURRENCIES
  @scala.inline
  def CHF: CHF = "CHF".asInstanceOf[CHF]
  
  @js.native
  sealed trait CNY extends CURRENCIES
  @scala.inline
  def CNY: CNY = "CNY".asInstanceOf[CNY]
  
  @js.native
  sealed trait CZK extends CURRENCIES
  @scala.inline
  def CZK: CZK = "CZK".asInstanceOf[CZK]
  
  @js.native
  sealed trait DKK extends CURRENCIES
  @scala.inline
  def DKK: DKK = "DKK".asInstanceOf[DKK]
  
  @js.native
  sealed trait EUR extends CURRENCIES
  @scala.inline
  def EUR: EUR = "EUR".asInstanceOf[EUR]
  
  @js.native
  sealed trait GBP extends CURRENCIES
  @scala.inline
  def GBP: GBP = "GBP".asInstanceOf[GBP]
  
  @js.native
  sealed trait HKD extends CURRENCIES
  @scala.inline
  def HKD: HKD = "HKD".asInstanceOf[HKD]
  
  @js.native
  sealed trait HUF extends CURRENCIES
  @scala.inline
  def HUF: HUF = "HUF".asInstanceOf[HUF]
  
  @js.native
  sealed trait ILS extends CURRENCIES
  @scala.inline
  def ILS: ILS = "ILS".asInstanceOf[ILS]
  
  @js.native
  sealed trait INR extends CURRENCIES
  @scala.inline
  def INR: INR = "INR".asInstanceOf[INR]
  
  @js.native
  sealed trait JPY extends CURRENCIES
  @scala.inline
  def JPY: JPY = "JPY".asInstanceOf[JPY]
  
  @js.native
  sealed trait MXN extends CURRENCIES
  @scala.inline
  def MXN: MXN = "MXN".asInstanceOf[MXN]
  
  @js.native
  sealed trait MYR extends CURRENCIES
  @scala.inline
  def MYR: MYR = "MYR".asInstanceOf[MYR]
  
  @js.native
  sealed trait NOK extends CURRENCIES
  @scala.inline
  def NOK: NOK = "NOK".asInstanceOf[NOK]
  
  @js.native
  sealed trait NO_NETWORK extends ENVIRONMENT
  @scala.inline
  def NO_NETWORK: NO_NETWORK = "NO_NETWORK".asInstanceOf[NO_NETWORK]
  
  @js.native
  sealed trait NZD extends CURRENCIES
  @scala.inline
  def NZD: NZD = "NZD".asInstanceOf[NZD]
  
  @js.native
  sealed trait ORDER extends INTENT
  @scala.inline
  def ORDER: ORDER = "ORDER".asInstanceOf[ORDER]
  
  @js.native
  sealed trait PHP extends CURRENCIES
  @scala.inline
  def PHP: PHP = "PHP".asInstanceOf[PHP]
  
  @js.native
  sealed trait PLN extends CURRENCIES
  @scala.inline
  def PLN: PLN = "PLN".asInstanceOf[PLN]
  
  @js.native
  sealed trait PRODUCTION extends ENVIRONMENT
  @scala.inline
  def PRODUCTION: PRODUCTION = "PRODUCTION".asInstanceOf[PRODUCTION]
  
  @js.native
  sealed trait RUB extends CURRENCIES
  @scala.inline
  def RUB: RUB = "RUB".asInstanceOf[RUB]
  
  @js.native
  sealed trait SALE extends INTENT
  @scala.inline
  def SALE: SALE = "SALE".asInstanceOf[SALE]
  
  @js.native
  sealed trait SANDBOX extends ENVIRONMENT
  @scala.inline
  def SANDBOX: SANDBOX = "SANDBOX".asInstanceOf[SANDBOX]
  
  @js.native
  sealed trait SEK extends CURRENCIES
  @scala.inline
  def SEK: SEK = "SEK".asInstanceOf[SEK]
  
  @js.native
  sealed trait SGD extends CURRENCIES
  @scala.inline
  def SGD: SGD = "SGD".asInstanceOf[SGD]
  
  @js.native
  sealed trait THB extends CURRENCIES
  @scala.inline
  def THB: THB = "THB".asInstanceOf[THB]
  
  @js.native
  sealed trait TWD extends CURRENCIES
  @scala.inline
  def TWD: TWD = "TWD".asInstanceOf[TWD]
  
  @js.native
  sealed trait USD extends CURRENCIES
  @scala.inline
  def USD: USD = "USD".asInstanceOf[USD]
}
