package typingsSlinky.reduxDashAuthDashWrapper

import typingsSlinky.reduxDashAuthDashWrapper.history3LocationHelperMod.LocationHelper
import typingsSlinky.reduxDashAuthDashWrapper.history3LocationHelperMod.LocationHelperConfig
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("redux-auth-wrapper/history4/locationHelper", JSImport.Namespace)
@js.native
object history4LocationHelperMod extends js.Object {
  def default[Props](config: LocationHelperConfig[Props]): LocationHelper[Props] = js.native
}

