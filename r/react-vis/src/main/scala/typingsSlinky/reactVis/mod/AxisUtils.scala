package typingsSlinky.reactVis.mod

import typingsSlinky.reactVis.anon.BOTTOM_
import typingsSlinky.reactVis.anon.HORIZONTAL_
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object AxisUtils {
  
  @JSImport("react-vis", "AxisUtils")
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("react-vis", "AxisUtils.DIRECTION")
  @js.native
  def DIRECTION: HORIZONTAL_ = js.native
  @scala.inline
  def DIRECTION_=(x: HORIZONTAL_): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("DIRECTION")(x.asInstanceOf[js.Any])
  
  @JSImport("react-vis", "AxisUtils.ORIENTATION")
  @js.native
  def ORIENTATION: BOTTOM_ = js.native
  @scala.inline
  def ORIENTATION_=(x: BOTTOM_): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ORIENTATION")(x.asInstanceOf[js.Any])
  
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.UndefOr[scala.Nothing], tickTotal: js.UndefOr[scala.Nothing], tickValues: js.Any): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.UndefOr[scala.Nothing], tickTotal: js.Any): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.UndefOr[scala.Nothing], tickTotal: js.Any, tickValues: js.Any): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.Any): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.Any, tickTotal: js.UndefOr[scala.Nothing], tickValues: js.Any): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.Any, tickTotal: js.Any): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTickValues")
  @js.native
  def getTickValues(scale: js.Any, tickTotal: js.Any, tickValues: js.Any): js.Any = js.native
  
  @JSImport("react-vis", "AxisUtils.getTicksTotalFromSize")
  @js.native
  def getTicksTotalFromSize(): js.Any = js.native
  @JSImport("react-vis", "AxisUtils.getTicksTotalFromSize")
  @js.native
  def getTicksTotalFromSize(size: js.Any): js.Any = js.native
}
