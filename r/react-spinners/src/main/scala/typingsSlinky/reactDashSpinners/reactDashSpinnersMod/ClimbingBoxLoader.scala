package typingsSlinky.reactDashSpinners.reactDashSpinnersMod

import org.scalablytyped.runtime.TopLevel
import typingsSlinky.react.reactMod.Component
import typingsSlinky.react.reactMod.ComponentClass
import typingsSlinky.react.reactMod.ComponentState
import typingsSlinky.reactDashSpinners.interfacesMod.LoaderSizeProps
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("react-spinners", "ClimbingBoxLoader")
@js.native
class ClimbingBoxLoader protected ()
  extends Component[LoaderSizeProps, ComponentState, js.Any] {
  def this(props: LoaderSizeProps) = this()
  def this(props: LoaderSizeProps, context: js.Any) = this()
}

@JSImport("react-spinners", "ClimbingBoxLoader")
@js.native
object ClimbingBoxLoader extends TopLevel[ComponentClass[LoaderSizeProps, ComponentState]]

