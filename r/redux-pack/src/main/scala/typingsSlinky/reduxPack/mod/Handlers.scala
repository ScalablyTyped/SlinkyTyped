...
object Handlers {
  ...
  implicit class HandlersOps[Self <: Handlers[_, _, _, _, _], S, TSuccessPayload, TErrorPayload, TStartPayload, TMetaPayload] (val x: Self with (Handlers[S, TSuccessPayload, TErrorPayload, TStartPayload, TMetaPayload])) extends AnyVal {
    ...
    @scala.inline
    def setAlways(value: (S, typingsSlinky.redux.mod.Action[_]) => S): Self = this.set("always", js.Any.fromFunction2(value))
    ...
    @scala.inline
    def setFinish(value: (S, typingsSlinky.redux.mod.Action[_]) => S): Self = this.set("finish", js.Any.fromFunction2(value))
