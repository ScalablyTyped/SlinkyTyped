package typingsSlinky.reactDashImgpro

import typingsSlinky.reactDashImgpro.reactDashImgproMod.ReactImgpro.Mode
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon_Degree extends js.Object {
  var degree: Double
  var mode: Mode
}

object Anon_Degree {
  @scala.inline
  def apply(degree: Double, mode: Mode): Anon_Degree = {
    val __obj = js.Dynamic.literal(degree = degree.asInstanceOf[js.Any], mode = mode.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon_Degree]
  }
}

