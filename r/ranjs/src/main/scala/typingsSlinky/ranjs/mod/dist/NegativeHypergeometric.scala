package typingsSlinky.ranjs.mod.dist

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("ranjs", "dist.NegativeHypergeometric")
@js.native
class NegativeHypergeometric ()
  extends typingsSlinky.ranjs.mod._dist.Distribution[typingsSlinky.ranjs.ranjsStrings.NegativeHypergeometric] {
  def this(N: Double) = this()
  def this(N: js.UndefOr[scala.Nothing], K: Double) = this()
  def this(N: Double, K: Double) = this()
  def this(N: js.UndefOr[scala.Nothing], K: js.UndefOr[scala.Nothing], r: Double) = this()
  def this(N: js.UndefOr[scala.Nothing], K: Double, r: Double) = this()
  def this(N: Double, K: js.UndefOr[scala.Nothing], r: Double) = this()
  def this(N: Double, K: Double, r: Double) = this()
}

