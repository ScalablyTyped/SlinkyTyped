package typingsSlinky.reactDashNative.reactDashNativeMod.Animated

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - typings.reactDashNative.reactDashNativeStrings.extend
  - typings.reactDashNative.reactDashNativeStrings.identity
  - typings.reactDashNative.reactDashNativeStrings.clamp
*/
trait ExtrapolateType extends js.Object

object ExtrapolateType {
  @scala.inline
  /* private */ def cast[T](in: js.Any): T = in.asInstanceOf[T]
  @scala.inline
  def clamp: typingsSlinky.reactDashNative.reactDashNativeStrings.clamp = this.cast("clamp")
  @scala.inline
  def extend: typingsSlinky.reactDashNative.reactDashNativeStrings.extend = this.cast("extend")
  @scala.inline
  def identity: typingsSlinky.reactDashNative.reactDashNativeStrings.identity = this.cast("identity")
}

