package typingsSlinky.reactWeui.mod

import typingsSlinky.reactWeui.anon.ShowCounter
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("react-weui", "TextArea")
@js.native
class TextArea protected () extends StObject {
  def this(args: js.Any*) = this()
  
  def handleChange(e: js.Any): Unit = js.native
  
  def render(): js.Any = js.native
}
/* static members */
object TextArea {
  
  @JSImport("react-weui", "TextArea")
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("react-weui", "TextArea.defaultProps")
  @js.native
  def defaultProps: ShowCounter = js.native
  @scala.inline
  def defaultProps_=(x: ShowCounter): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("defaultProps")(x.asInstanceOf[js.Any])
  
  object propTypes {
    
    object defaultValue {
      
      @JSImport("react-weui", "TextArea.propTypes.defaultValue")
      @js.native
      def apply(p0: js.Any, p1: js.Any, p2: js.Any, p3: js.Any, p4: js.Any, p5: js.Any): js.Any = js.native
      
      @JSImport("react-weui", "TextArea.propTypes.defaultValue.isRequired")
      @js.native
      def isRequired(p0: js.Any, p1: js.Any, p2: js.Any, p3: js.Any, p4: js.Any, p5: js.Any): js.Any = js.native
    }
    
    object maxLength {
      
      @JSImport("react-weui", "TextArea.propTypes.maxLength")
      @js.native
      def apply(p0: js.Any, p1: js.Any, p2: js.Any, p3: js.Any, p4: js.Any, p5: js.Any): js.Any = js.native
      
      @JSImport("react-weui", "TextArea.propTypes.maxLength.isRequired")
      @js.native
      def isRequired(p0: js.Any, p1: js.Any, p2: js.Any, p3: js.Any, p4: js.Any, p5: js.Any): js.Any = js.native
    }
    
    object showCounter {
      
      @JSImport("react-weui", "TextArea.propTypes.showCounter")
      @js.native
      def apply(p0: js.Any, p1: js.Any, p2: js.Any, p3: js.Any, p4: js.Any, p5: js.Any): js.Any = js.native
      
      @JSImport("react-weui", "TextArea.propTypes.showCounter.isRequired")
      @js.native
      def isRequired(p0: js.Any, p1: js.Any, p2: js.Any, p3: js.Any, p4: js.Any, p5: js.Any): js.Any = js.native
    }
  }
}
