...
trait Props[T /* <: Route */] extends js.Object {
  ...
  def renderLazyPlaceholder(props: AnonRoute[T]): TagMod[Any]
  def renderPager(props: typingsSlinky.reactNativeTabView.pagerMod.Props[T]): TagMod[Any]
  def renderScene(props: SceneRendererProps with AnonRoute[T]): TagMod[Any]
  def renderTabBar(props: SceneRendererProps with AnonNavigationState[T]): TagMod[Any]
}
