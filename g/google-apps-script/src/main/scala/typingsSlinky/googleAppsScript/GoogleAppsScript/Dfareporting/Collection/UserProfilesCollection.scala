...
import typingsSlinky.googleAppsScript.GoogleAppsScript.Dfareporting.Schema.UserProfile
import typingsSlinky.googleAppsScript.GoogleAppsScript.Dfareporting.Schema.UserProfileList
...
object UserProfilesCollection {
  ...
  implicit class UserProfilesCollectionOps[Self <: UserProfilesCollection] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withList(value: () => UserProfileList): Self = {
        val ret = this.duplicate
