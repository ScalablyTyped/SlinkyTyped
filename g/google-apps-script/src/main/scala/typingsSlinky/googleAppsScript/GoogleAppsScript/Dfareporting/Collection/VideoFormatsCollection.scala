...
import typingsSlinky.googleAppsScript.GoogleAppsScript.Dfareporting.Schema.VideoFormat
import typingsSlinky.googleAppsScript.GoogleAppsScript.Dfareporting.Schema.VideoFormatsListResponse
...
object VideoFormatsCollection {
  ...
  implicit class VideoFormatsCollectionOps[Self <: VideoFormatsCollection] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withGet(value: (String, Double) => VideoFormat): Self = {
        val ret = this.duplicate
        ret.asInstanceOf[js.Dynamic].updateDynamic("get")(js.Any.fromFunction2(value))
        ret
    }
    @scala.inline
    def withList(value: String => VideoFormatsListResponse): Self = {
        val ret = this.duplicate
