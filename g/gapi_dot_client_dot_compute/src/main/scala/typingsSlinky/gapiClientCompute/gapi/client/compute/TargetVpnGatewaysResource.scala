...
import typingsSlinky.gapiClientCompute.anon.RegionTargetVpnGateway
...
object TargetVpnGatewaysResource {
  ...
  def apply(
    ...
    delete: typingsSlinky.gapiClientCompute.anon.TargetVpnGateway => Request[Operation],
  ...
  implicit class TargetVpnGatewaysResourceOps[Self <: TargetVpnGatewaysResource] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def setDelete(value: typingsSlinky.gapiClientCompute.anon.TargetVpnGateway => Request[Operation]): Self = this.set("delete", js.Any.fromFunction1(value))
    @scala.inline
