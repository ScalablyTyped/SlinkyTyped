...
import typingsSlinky.gapiClientGamesmanagement.anon.EventId
...
object EventsResource {
  ...
  implicit class EventsResourceOps[Self <: EventsResource] (val x: Self) extends AnyVal {
    ...
    @scala.inline
    def withReset(value: EventId => Request[Unit]): Self = {
    ...
    @scala.inline
    def withResetForAllPlayers(value: EventId => Request[Unit]): Self = {
        val ret = this.duplicate
