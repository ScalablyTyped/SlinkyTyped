...
object GraphQLFieldConfig {
  ...
  @scala.inline
  implicit class GraphQLFieldConfigOps[Self[tsource, tcontext, targs] <: GraphQLFieldConfig[tsource, tcontext, targs], TSource, TContext, TArgs] (val x: Self[TSource, TContext, TArgs]) extends AnyVal {
    @scala.inline
    ...
    @scala.inline
    def withArgs(value: GraphQLFieldConfigArgumentMap): Self[TSource, TContext, TArgs] = {
        val ret = this.duplicate
