package typingsSlinky.googleSpreadsheet

import typingsSlinky.googleSpreadsheet.mod.BorderStyle
import typingsSlinky.googleSpreadsheet.mod.CellErrorType
import typingsSlinky.googleSpreadsheet.mod.CellValueType
import typingsSlinky.googleSpreadsheet.mod.DeveloperMetadataLocationType
import typingsSlinky.googleSpreadsheet.mod.DeveloperMetadataVisibility
import typingsSlinky.googleSpreadsheet.mod.Dimension
import typingsSlinky.googleSpreadsheet.mod.HorizontalAlign
import typingsSlinky.googleSpreadsheet.mod.HyperlinkDisplayType
import typingsSlinky.googleSpreadsheet.mod.NumberFormatType
import typingsSlinky.googleSpreadsheet.mod.RecalculationInterval
import typingsSlinky.googleSpreadsheet.mod.TextDirection
import typingsSlinky.googleSpreadsheet.mod.ThemeColorType
import typingsSlinky.googleSpreadsheet.mod.VerticalAlign
import typingsSlinky.googleSpreadsheet.mod.WorksheetDimension
import typingsSlinky.googleSpreadsheet.mod.WorksheetType
import typingsSlinky.googleSpreadsheet.mod.WrapStrategy
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object googleSpreadsheetStrings {
  @js.native
  sealed trait ACCENT1 extends ThemeColorType
  
  @js.native
  sealed trait ACCENT2 extends ThemeColorType
  
  @js.native
  sealed trait ACCENT3 extends ThemeColorType
  
  @js.native
  sealed trait ACCENT4 extends ThemeColorType
  
  @js.native
  sealed trait ACCENT5 extends ThemeColorType
  
  @js.native
  sealed trait ACCENT6 extends ThemeColorType
  
  @js.native
  sealed trait BACKGROUND extends ThemeColorType
  
  @js.native
  sealed trait BOTTOM extends VerticalAlign
  
  @js.native
  sealed trait CENTER extends HorizontalAlign
  
  @js.native
  sealed trait CLIP extends WrapStrategy
  
  @js.native
  sealed trait COLUMN
    extends DeveloperMetadataLocationType
       with WorksheetDimension
  
  @js.native
  sealed trait COLUMNS extends Dimension
  
  @js.native
  sealed trait CURRENCY extends NumberFormatType
  
  @js.native
  sealed trait DASHED extends BorderStyle
  
  @js.native
  sealed trait DATE extends NumberFormatType
  
  @js.native
  sealed trait DIVIDE_BY_ZERO extends CellErrorType
  
  @js.native
  sealed trait DOCUMENT extends DeveloperMetadataVisibility
  
  @js.native
  sealed trait DOTTED extends BorderStyle
  
  @js.native
  sealed trait DOUBLE extends BorderStyle
  
  @js.native
  sealed trait ERROR extends CellErrorType
  
  @js.native
  sealed trait GRID extends WorksheetType
  
  @js.native
  sealed trait HOUR extends RecalculationInterval
  
  @js.native
  sealed trait LEFT extends HorizontalAlign
  
  @js.native
  sealed trait LEFT_TO_RIGHT extends TextDirection
  
  @js.native
  sealed trait LEGACY_WRAP extends WrapStrategy
  
  @js.native
  sealed trait LINK extends ThemeColorType
  
  @js.native
  sealed trait LINKED extends HyperlinkDisplayType
  
  @js.native
  sealed trait LOADING extends CellErrorType
  
  @js.native
  sealed trait MIDDLE extends VerticalAlign
  
  @js.native
  sealed trait MINUTE extends RecalculationInterval
  
  @js.native
  sealed trait NAME extends CellErrorType
  
  @js.native
  sealed trait NONE extends BorderStyle
  
  @js.native
  sealed trait NULL_VALUE extends CellErrorType
  
  @js.native
  sealed trait NUM extends CellErrorType
  
  @js.native
  sealed trait NUMBER extends NumberFormatType
  
  @js.native
  sealed trait N_A extends CellErrorType
  
  @js.native
  sealed trait OBJECT extends WorksheetType
  
  @js.native
  sealed trait ON_CHANGE extends RecalculationInterval
  
  @js.native
  sealed trait OVERFLOW_CELL extends WrapStrategy
  
  @js.native
  sealed trait PERCENT extends NumberFormatType
  
  @js.native
  sealed trait PLAIN_TEXT extends HyperlinkDisplayType
  
  @js.native
  sealed trait PROJECT extends DeveloperMetadataVisibility
  
  @js.native
  sealed trait REF extends CellErrorType
  
  @js.native
  sealed trait RIGHT extends HorizontalAlign
  
  @js.native
  sealed trait RIGHT_TO_LEFT extends TextDirection
  
  @js.native
  sealed trait ROW
    extends DeveloperMetadataLocationType
       with WorksheetDimension
  
  @js.native
  sealed trait ROWS extends Dimension
  
  @js.native
  sealed trait SCIENTIFIC extends NumberFormatType
  
  @js.native
  sealed trait SHEET extends DeveloperMetadataLocationType
  
  @js.native
  sealed trait SOLID extends BorderStyle
  
  @js.native
  sealed trait SOLID_MEDIUM extends BorderStyle
  
  @js.native
  sealed trait SOLID_THICK extends BorderStyle
  
  @js.native
  sealed trait SPREADSHEET extends DeveloperMetadataLocationType
  
  @js.native
  sealed trait TEXT
    extends NumberFormatType
       with ThemeColorType
  
  @js.native
  sealed trait TIME extends NumberFormatType
  
  @js.native
  sealed trait TOP extends VerticalAlign
  
  @js.native
  sealed trait VALUE extends CellErrorType
  
  @js.native
  sealed trait WRAP extends WrapStrategy
  
  @js.native
  sealed trait boolValue extends CellValueType
  
  @js.native
  sealed trait errorValue extends CellValueType
  
  @js.native
  sealed trait numberValue extends CellValueType
  
  @js.native
  sealed trait stringValue extends CellValueType
  
  @scala.inline
  def ACCENT1: ACCENT1 = "ACCENT1".asInstanceOf[ACCENT1]
  @scala.inline
  def ACCENT2: ACCENT2 = "ACCENT2".asInstanceOf[ACCENT2]
  @scala.inline
  def ACCENT3: ACCENT3 = "ACCENT3".asInstanceOf[ACCENT3]
  @scala.inline
  def ACCENT4: ACCENT4 = "ACCENT4".asInstanceOf[ACCENT4]
  @scala.inline
  def ACCENT5: ACCENT5 = "ACCENT5".asInstanceOf[ACCENT5]
  @scala.inline
  def ACCENT6: ACCENT6 = "ACCENT6".asInstanceOf[ACCENT6]
  @scala.inline
  def BACKGROUND: BACKGROUND = "BACKGROUND".asInstanceOf[BACKGROUND]
  @scala.inline
  def BOTTOM: BOTTOM = "BOTTOM".asInstanceOf[BOTTOM]
  @scala.inline
  def CENTER: CENTER = "CENTER".asInstanceOf[CENTER]
  @scala.inline
  def CLIP: CLIP = "CLIP".asInstanceOf[CLIP]
  @scala.inline
  def COLUMN: COLUMN = "COLUMN".asInstanceOf[COLUMN]
  @scala.inline
  def COLUMNS: COLUMNS = "COLUMNS".asInstanceOf[COLUMNS]
  @scala.inline
  def CURRENCY: CURRENCY = "CURRENCY".asInstanceOf[CURRENCY]
  @scala.inline
  def DASHED: DASHED = "DASHED".asInstanceOf[DASHED]
  @scala.inline
  def DATE: DATE = "DATE".asInstanceOf[DATE]
  @scala.inline
  def DIVIDE_BY_ZERO: DIVIDE_BY_ZERO = "DIVIDE_BY_ZERO".asInstanceOf[DIVIDE_BY_ZERO]
  @scala.inline
  def DOCUMENT: DOCUMENT = "DOCUMENT".asInstanceOf[DOCUMENT]
  @scala.inline
  def DOTTED: DOTTED = "DOTTED".asInstanceOf[DOTTED]
  @scala.inline
  def DOUBLE: DOUBLE = "DOUBLE".asInstanceOf[DOUBLE]
  @scala.inline
  def ERROR: ERROR = "ERROR".asInstanceOf[ERROR]
  @scala.inline
  def GRID: GRID = "GRID".asInstanceOf[GRID]
  @scala.inline
  def HOUR: HOUR = "HOUR".asInstanceOf[HOUR]
  @scala.inline
  def LEFT: LEFT = "LEFT".asInstanceOf[LEFT]
  @scala.inline
  def LEFT_TO_RIGHT: LEFT_TO_RIGHT = "LEFT_TO_RIGHT".asInstanceOf[LEFT_TO_RIGHT]
  @scala.inline
  def LEGACY_WRAP: LEGACY_WRAP = "LEGACY_WRAP".asInstanceOf[LEGACY_WRAP]
  @scala.inline
  def LINK: LINK = "LINK".asInstanceOf[LINK]
  @scala.inline
  def LINKED: LINKED = "LINKED".asInstanceOf[LINKED]
  @scala.inline
  def LOADING: LOADING = "LOADING".asInstanceOf[LOADING]
  @scala.inline
  def MIDDLE: MIDDLE = "MIDDLE".asInstanceOf[MIDDLE]
  @scala.inline
  def MINUTE: MINUTE = "MINUTE".asInstanceOf[MINUTE]
  @scala.inline
  def NAME: NAME = "NAME".asInstanceOf[NAME]
  @scala.inline
  def NONE: NONE = "NONE".asInstanceOf[NONE]
  @scala.inline
  def NULL_VALUE: NULL_VALUE = "NULL_VALUE".asInstanceOf[NULL_VALUE]
  @scala.inline
  def NUM: NUM = "NUM".asInstanceOf[NUM]
  @scala.inline
  def NUMBER: NUMBER = "NUMBER".asInstanceOf[NUMBER]
  @scala.inline
  def N_A: N_A = "N_A".asInstanceOf[N_A]
  @scala.inline
  def OBJECT: OBJECT = "OBJECT".asInstanceOf[OBJECT]
  @scala.inline
  def ON_CHANGE: ON_CHANGE = "ON_CHANGE".asInstanceOf[ON_CHANGE]
  @scala.inline
  def OVERFLOW_CELL: OVERFLOW_CELL = "OVERFLOW_CELL".asInstanceOf[OVERFLOW_CELL]
  @scala.inline
  def PERCENT: PERCENT = "PERCENT".asInstanceOf[PERCENT]
  @scala.inline
  def PLAIN_TEXT: PLAIN_TEXT = "PLAIN_TEXT".asInstanceOf[PLAIN_TEXT]
  @scala.inline
  def PROJECT: PROJECT = "PROJECT".asInstanceOf[PROJECT]
  @scala.inline
  def REF: REF = "REF".asInstanceOf[REF]
  @scala.inline
  def RIGHT: RIGHT = "RIGHT".asInstanceOf[RIGHT]
  @scala.inline
  def RIGHT_TO_LEFT: RIGHT_TO_LEFT = "RIGHT_TO_LEFT".asInstanceOf[RIGHT_TO_LEFT]
  @scala.inline
  def ROW: ROW = "ROW".asInstanceOf[ROW]
  @scala.inline
  def ROWS: ROWS = "ROWS".asInstanceOf[ROWS]
  @scala.inline
  def SCIENTIFIC: SCIENTIFIC = "SCIENTIFIC".asInstanceOf[SCIENTIFIC]
  @scala.inline
  def SHEET: SHEET = "SHEET".asInstanceOf[SHEET]
  @scala.inline
  def SOLID: SOLID = "SOLID".asInstanceOf[SOLID]
  @scala.inline
  def SOLID_MEDIUM: SOLID_MEDIUM = "SOLID_MEDIUM".asInstanceOf[SOLID_MEDIUM]
  @scala.inline
  def SOLID_THICK: SOLID_THICK = "SOLID_THICK".asInstanceOf[SOLID_THICK]
  @scala.inline
  def SPREADSHEET: SPREADSHEET = "SPREADSHEET".asInstanceOf[SPREADSHEET]
  @scala.inline
  def TEXT: TEXT = "TEXT".asInstanceOf[TEXT]
  @scala.inline
  def TIME: TIME = "TIME".asInstanceOf[TIME]
  @scala.inline
  def TOP: TOP = "TOP".asInstanceOf[TOP]
  @scala.inline
  def VALUE: VALUE = "VALUE".asInstanceOf[VALUE]
  @scala.inline
  def WRAP: WRAP = "WRAP".asInstanceOf[WRAP]
  @scala.inline
  def boolValue: boolValue = "boolValue".asInstanceOf[boolValue]
  @scala.inline
  def errorValue: errorValue = "errorValue".asInstanceOf[errorValue]
  @scala.inline
  def numberValue: numberValue = "numberValue".asInstanceOf[numberValue]
  @scala.inline
  def stringValue: stringValue = "stringValue".asInstanceOf[stringValue]
}

