package typingsSlinky.hexoDashFs.hexoDashFsMod

import typingsSlinky.hexoDashFs.Anon_Exclude
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("hexo-fs", "emptyDirSync")
@js.native
object emptyDirSync extends js.Object {
  def apply(path: String): String | js.Array[String] = js.native
  def apply(path: String, options: DirectoryOptions with Anon_Exclude): String | js.Array[String] = js.native
  def apply(path: String, options: DirectoryOptions with Anon_Exclude, parent: String): String | js.Array[String] = js.native
}

