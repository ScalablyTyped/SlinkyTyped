...
object mod {
  ...
  trait HttpTerminatorConfig extends StObject {
    ...
    /**
      ...
      */
    var server: Server | typingsSlinky.node.httpsMod.Server = js.native
  }
  object HttpTerminatorConfig {
    
    @scala.inline
    def apply(server: Server | typingsSlinky.node.httpsMod.Server): HttpTerminatorConfig = {
    ...
    implicit class HttpTerminatorConfigMutableBuilder[Self <: HttpTerminatorConfig] (val x: Self) extends AnyVal {
      ...
      @scala.inline
      def setServer(value: Server | typingsSlinky.node.httpsMod.Server): Self = StObject.set(x, "server", value.asInstanceOf[js.Any])
    }
  }
}
