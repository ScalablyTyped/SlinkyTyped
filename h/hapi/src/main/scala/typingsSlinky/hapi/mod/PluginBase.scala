...
import typingsSlinky.hapi.anon.Hapi
...
trait PluginBase[T] extends js.Object {
  ...
  /**
    ...
    */
  def register(server: Server, options: T): Unit | js.Promise[Unit] = js.native
}

object PluginBase {
  @scala.inline
  def apply[T](register: (Server, T) => Unit | js.Promise[Unit]): PluginBase[T] = {
  ...
  implicit class PluginBaseOps[Self[t] <: PluginBase[t], T] (val x: Self[T]) extends AnyVal {
    ...
    @scala.inline
    def withRegister(value: (Server, T) => Unit | js.Promise[Unit]): Self[T] = {
